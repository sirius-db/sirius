...
mod ffi {
    unsafe extern "C++" {
        ...
        /// arena's free list and coalesces with its free neighbours.
        ...
        /// the caller's job. A zero-row batch is metadata-only: `offset == 0`
        /// with `length == 0` names NO lease, and nothing must be released.
        fn export_packed(
            self: Pin<&mut Fragment>,
            stream_id: u64,
            offset: &mut u64,
