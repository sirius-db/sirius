...
mod ffi {
    unsafe extern "C++" {
        ...
        ///
        /// `source` must have finished [`run`](Fragment::run). `input_stream_id`
        /// must be declared on this fragment, and `source` must have output streams.
        fn relay_from(
            self: Pin<&mut Fragment>,
        ...
        /// True when output stream `stream_id` has ended (every sender closed,
        /// queue empty, no error). False means "not done" — including "nothing
        /// parked right now".
        fn drained(self: Pin<&mut Fragment>, stream_id: u64) -> Result<bool>;
