# Fetch upstream testcontainers-native at a pinned commit, apply the Sirius patch
# (third_party/testcontainers-native.patch — container-command support, error
# propagation from tc_container_run, and a double-append fix), and build the
# Go->C bridge as a static c-archive plus the thin C wrapper into a single
...
FetchContent_Declare(
  ...
  # populates (and runs PATCH_COMMAND) without add_subdirectory()'ing upstream.
...
# -mod=readonly: download into the cache but never mutate the fetched go.mod/sum.
set(TCN_GO_ENV
    "GOPATH=${TCN_OUT}/.gopath"
    "GOCACHE=${TCN_OUT}/.gocache"
...
add_custom_command(
  ...
  DEPENDS
    "${TCN_BRIDGE_DIR}/testcontainers-bridge.go"
    "${TCN_BRIDGE_DIR}/go.mod"
    "${TCN_BRIDGE_DIR}/go.sum"
  WORKING_DIRECTORY "${TCN_BRIDGE_DIR}"
  COMMAND ${CMAKE_COMMAND} -E env ${TCN_GO_ENV}
          "${GO_EXECUTABLE}" build -buildmode=c-archive
          -o "${TCN_BRIDGE_ARCHIVE}" testcontainers-bridge.go
  COMMENT "Building patched testcontainers Go->C bridge (c-archive)"
  VERBATIM)

add_custom_target(testcontainers-bridge-build
                  DEPENDS "${TCN_BRIDGE_ARCHIVE}" "${TCN_BRIDGE_HEADER}")
...
target_include_directories(testcontainers_native
...
target_link_libraries(testcontainers_native
  PUBLIC "${TCN_BRIDGE_ARCHIVE}" Threads::Threads ${CMAKE_DL_LIBS})
