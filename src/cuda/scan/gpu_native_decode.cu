...
//===----------------------------------------------------------------------===//
// GPU-native decode dispatcher for DuckDB-native segments.
//
// Public entry point `gpu_decode_table` walks per-column scan results from
// direct_block_scan, stages every unique block to device in one coalesced
// H2D pass, then dispatches per segment to the matching codec kernel and
// builds a `cudf::table` for the consumer.
//
// As A1 ships, only fixed-width columns (BITPACKING / UNCOMPRESSED /
// CONSTANT) are wired through.  Any column whose codec or DuckDB type is
// not understood here triggers a defensive `viability invariant violated`
// throw — `check_viability` (future PR E) is what owns keeping unsupported
// columns out of this dispatcher.  String / RLE / FSST / ALP arrive in PR
// A2 and PR A3.
//
// Concurrency contract: every CUDA call is enqueued on the supplied
// `stream`.  One `stream.synchronize()` runs near the end so deferred
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  for (auto const& [id, ptr] : seen)
    sorted.push_back({id, ptr});
  std::sort(sorted.begin(), sorted.end(), [](entry const& a, entry const& b) {
  ...
  for (size_t i = 0; i < sorted.size();) {
    size_t run_start = i;
    while (i + 1 < sorted.size() && sorted[i + 1].block_id == sorted[i].block_id + 1 &&
  ...
  uint32_t* d_valid_count_out)
{
  ...
  size_t mask_bytes = cudf::bitmask_allocation_size_bytes(static_cast<cudf::size_type>(total_rows));
                                                        ...
                                                        uint32_t* d_valid_count_out)
{
  ...
  for (auto& seg : col_scan.data.segments) {
    ...
    switch (seg.compression) {
      ...
      case duckdb::CompressionType::COMPRESSION_ALP: {
        // ALP kernels read straight from device staging — they don't have a
        // per-segment H2D fallback path.  Reaching here without a staged
        // block means the bulk-stage step earlier missed this segment.
        if (!d_block) {
          throw std::runtime_error(
      ...
      default:
        ...
        // viability check has fallen out of sync with the kernels we ship.
        throw std::runtime_error(
          "gpu_native_decode: viability invariant violated — unsupported "
          "compression type " +
          std::to_string(static_cast<int>(seg.compression)) + " for fixed-width column");
    }
    row_offset += seg.row_count;
  }
  ...
  if (!bp_segments.empty()) {
    ...
    for (auto const& bp : bp_segments) {
      const uint8_t* d_block = bp.on_device ? bp.block_ptr : d_fb_staging + fb_map[bp.block_ptr];
                                              ...
                                              rmm::device_async_resource_ref mr)
{
  ...
  if (!null_col_indices.empty()) {
    ...
    for (size_t i = 0; i < null_col_indices.size(); ++i) {
      ...
      auto nc = static_cast<cudf::size_type>(col_scans[ci].data.total_rows - h_valid_counts[i]);
