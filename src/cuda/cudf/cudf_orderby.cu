 ...
 */
 ...
 void CustomMultiColumnTopN(
      ...
      // 4. Extract Results
      // [Fix] Must limit output count to actual row count, otherwise reading Sentinel (0xFFFFFFFF) will cause out-of-bounds memory access
      uint64_t limit_clamped = min((uint64_t)num_results, (uint64_t)num_records);
      uint32_t final_count = min(limit_clamped, num_candidates);
      ...
      for (int col_idx = 0; col_idx < num_projections; ++col_idx) {
          ...
          if (kernel_type == KernelColType::STRING) {
              ...
              materialize_determine_lengths<<<gather_grid_size, gather_block_size>>>(
                  d_records, src_offsets, d_new_offsets, final_count
              );
              ...
              materialize_copy_string<<<gather_grid_size, gather_block_size>>>(
                  d_records, src_chars, src_offsets, d_new_chars, d_new_offsets, final_count
              );
              
              CUDA_CHECK_AND_SYNC("Gather String");
  
              auto validity = createNullMask(final_count, cudf::mask_state::ALL_VALID); 
              projection[col_idx] = make_shared_ptr<GPUColumn>(
                  final_count, src_wrapper.type, d_new_chars, d_new_offsets, total_bytes, true, validity
              );
              ...
              auto validity = createNullMask(final_count, cudf::mask_state::ALL_VALID);
              projection[col_idx] = make_shared_ptr<GPUColumn>(
                  final_count, src_wrapper.type, d_new_data, validity
              );
          }
      }
 ...
 void cudf_orderby(vector<shared_ptr<GPUColumn>>& keys, vector<shared_ptr<GPUColumn>>& projection, idx_t num_keys, idx_t num_projections, OrderByType* order_by_type, idx_t num_results) 
 {
      ...
      if (Config::USE_CUSTOM_TOP_N && num_results > 0 && num_results <= MAX_THREAD_TOP_K) {
          ...
          for (size_t col = 0; col < num_keys; col++) {
              ...
              // [Fix] Check for nulls using cudf column view since GPUColumn might not expose null_count directly
              // This is a safe check method that does not rely on internal API
              auto cv = keys[col]->convertToCudfColumn();
              if (cv.null_count() > 0) { 
                  can_use_optimization = false;
                  break;
              }
      ...
      auto sorted_order = cudf::stable_sorted_order(keys_table, orders, null_orders);
      
      // [Fix] Use slice to truncate map early, improving performance and resolving compile errors
      cudf::column_view gather_map = sorted_order->view();
      if (num_results < gather_map.size()) {
           std::vector<cudf::column_view> sliced_map = cudf::slice(
                gather_map, 
                {0, static_cast<cudf::size_type>(num_results)}
           );
           gather_map = sliced_map[0];
      }
      ...
      auto gathered_table = cudf::gather(projection_table, gather_map);
