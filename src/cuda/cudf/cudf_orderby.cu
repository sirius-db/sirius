 ...
 */
 ...
 #define ENABLE_CUDA_DEBUG 1 
     ...
     #define CUDA_CHECK_AND_SYNC(msg) { cudaDeviceSynchronize(); cudaError_t err = cudaGetLastError(); if (err != cudaSuccess) { SIRIUS_LOG_DEBUG("CUDA Error at [{}]: {} - {}", msg, cudaGetErrorName(err), cudaGetErrorString(err)); return; } }
 #else
 ...
 enum class KernelColType : int {
     INT_64 = 0, INT_32 = 1, DOUBLE = 2, STRING = 3, INT_128 = 4, INT_16 = 5, FLOAT_32 = 6, UNKNOWN = 99
 };
 
 struct DeviceKeyColumn {
     int type; uint32_t is_asc; uint8_t* data; uint64_t* offsets; uint32_t* null_mask; 
 };
 ...
 __device__ __forceinline__ T load_unaligned(const void* ptr) {
     T val; const uint8_t* src = reinterpret_cast<const uint8_t*>(ptr); uint8_t* dst = reinterpret_cast<uint8_t*>(&val);
     #pragma unroll
 ...
 __device__ __forceinline__ uint64_t load_primary_key_as_u64(const DeviceKeyColumn& col, uint32_t row_id) {
     ...
     if (type == KernelColType::INT_64) {
         ...
         uint64_t low; int64_t high; load_int128_safe(col.data, row_id, low, high);
         ...
         uint64_t prefix = 0; const uint8_t* ptr = col.data + start;
         uint64_t bytes = min(len, static_cast<uint64_t>(8));
         uint32_t shift = 56;
         for(int i=0; i<bytes; ++i) { prefix |= static_cast<uint64_t>(ptr[i]) << shift; shift -= 8; }
         return prefix;
     }
     return 0;
 }
 ...
 __device__ int compare_rows_multi_col(uint32_t row_a, uint32_t row_b, const DeviceKeyColumn* columns, int num_columns) {
     for (int i = 0; i < num_columns; ++i) {
         ...
         int cmp = 0;
         if (null_a != null_b) cmp = null_a ? 1 : -1;
         else if (null_a) cmp = 0;
         else {
             KernelColType type = static_cast<KernelColType>(col.type);
             if (type == KernelColType::INT_64) {
                 ...
                 uint64_t la, lb; int64_t ha, hb;
                 load_int128_safe(col.data, row_a, la, ha); load_int128_safe(col.data, row_b, lb, hb);
                 if (ha > hb) cmp = 1; else if (ha < hb) cmp = -1; else cmp = (la > lb) - (la < lb);
             } else if (type == KernelColType::STRING) {
                 uint64_t off_a = col.offsets[row_a]; uint64_t len_a = col.offsets[row_a + 1] - off_a; uint8_t* ptr_a = col.data + off_a;
                 uint64_t off_b = col.offsets[row_b]; uint64_t len_b = col.offsets[row_b + 1] - off_b; uint8_t* ptr_b = col.data + off_b;
                 uint64_t min_len = min(len_a, len_b);
                 for(uint64_t k=0; k < min_len; ++k) { if(ptr_a[k] != ptr_b[k]) { cmp = (ptr_a[k] > ptr_b[k]) ? 1 : -1; break; } }
 ...
 struct CustomExactSortComparator { 
    ...
    __host__ __device__ CustomExactSortComparator(DeviceKeyColumn* _d_cols, int _num_cols, bool _is_asc) 
 ...
 __device__ void insert_into_heap(uint32_t row_id, uint64_t pk, uint32_t* top_ids, uint64_t* top_pks, uint32_t& current_k, uint32_t limit, bool asc, DeviceKeyColumn* cols, int num_cols) {
     if (current_k < limit) {
         int pos = current_k; current_k++;
         while(pos > 0) {
             bool swap = false; uint64_t prev_pk = top_pks[pos-1];
             if (pk != prev_pk) { if (asc) { if(pk < prev_pk) swap = true; } else { if(pk > prev_pk) swap = true; } } 
             else { if (compare_rows_multi_col(row_id, top_ids[pos-1], cols, num_cols) < 0) swap = true; }
             if(swap) { top_pks[pos] = top_pks[pos-1]; top_ids[pos] = top_ids[pos-1]; pos--; } else break;
         }
         top_pks[pos] = pk; top_ids[pos] = row_id; return;
     }
     uint64_t worst_pk = top_pks[limit - 1]; bool better = false;
     if (pk != worst_pk) { if (asc) { if(pk < worst_pk) better = true; } else { if(pk > worst_pk) better = true; } } 
     else { if (compare_rows_multi_col(row_id, top_ids[limit-1], cols, num_cols) < 0) better = true; }
     if (better) {
         int pos = limit - 1;
         while(pos > 0) {
             bool swap = false; uint64_t prev_pk = top_pks[pos-1];
         ...
         top_pks[pos] = pk; top_ids[pos] = row_id;
     }
 }
 
 __global__ void per_thread_multi_col_top_k_kernel(DeviceKeyColumn* device_cols, int num_cols, uint32_t num_records, uint32_t limit, str_top_n_record_type* output_candidates) {
     if (num_records == 0) return;
     uint32_t top_ids[MAX_THREAD_TOP_K]; uint64_t top_pks[MAX_THREAD_TOP_K]; uint32_t current_k = 0;
     bool asc = (device_cols[0].is_asc == 1); uint64_t sentinel = asc ? 0xFFFFFFFFFFFFFFFFULL : 0;
     uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x; uint32_t stride = blockDim.x * gridDim.x;
     for (uint32_t row_id = tid; row_id < num_records; row_id += stride) {
         uint64_t curr_pk = load_primary_key_as_u64(device_cols[0], row_id);
         insert_into_heap(row_id, curr_pk, top_ids, top_pks, current_k, limit, asc, device_cols, num_cols);
     }
     uint32_t out_offset = tid * limit;
     for (int i = 0; i < current_k; ++i) output_candidates[out_offset + i] = str_top_n_record_type(top_ids[i], top_pks[i]);
     for (int i = current_k; i < limit; ++i) output_candidates[out_offset + i] = str_top_n_record_type(0xFFFFFFFF, sentinel);
 }
 
 __global__ void per_block_multi_col_top_k_kernel(str_top_n_record_type* input_candidates, uint32_t num_thread_outputs, DeviceKeyColumn* device_cols, int num_cols, uint32_t limit, str_top_n_record_type* block_output, uint32_t input_threads_per_block) {
     if (threadIdx.x != 0) return; 
     uint32_t top_ids[MAX_THREAD_TOP_K]; uint64_t top_pks[MAX_THREAD_TOP_K]; uint32_t current_k = 0;
     bool asc = (device_cols[0].is_asc == 1); uint64_t sentinel = asc ? 0xFFFFFFFFFFFFFFFFULL : 0;
     uint32_t items_per_thread = limit; uint32_t items_per_block_input = input_threads_per_block * items_per_thread;
     uint32_t start_idx = blockIdx.x * items_per_block_input; uint32_t end_idx = min(start_idx + items_per_block_input, num_thread_outputs);
     for (uint32_t i = start_idx; i < end_idx; ++i) {
         uint32_t row_id = input_candidates[i].row_id; if (row_id == 0xFFFFFFFF) continue;
     ...
     for (int i = 0; i < current_k; ++i) block_output[out_offset + i] = str_top_n_record_type(top_ids[i], top_pks[i]);
     for (int i = current_k; i < limit; ++i) block_output[out_offset + i] = str_top_n_record_type(0xFFFFFFFF, sentinel);
 }
 
 __global__ void global_final_multi_col_top_k_kernel(str_top_n_record_type* input_candidates, uint32_t num_candidates, DeviceKeyColumn* device_cols, int num_cols, uint32_t limit, str_top_n_record_type* final_records) {
     ...
     for (int i = 0; i < current_k; ++i) final_records[i] = str_top_n_record_type(top_ids[i], top_pks[i]);
     for (int i = current_k; i < limit; ++i) final_records[i] = str_top_n_record_type(0xFFFFFFFF, sentinel);
 }
 ...
 __global__ void create_radix_keys(uint64_t* key_prefixes, uint32_t* row_ids, DeviceKeyColumn* d_cols, uint32_t num_records) {
 ...
 __global__ void find_candidate_cutoff(uint64_t* sorted_prefixes, uint32_t num_records, uint32_t limit, uint32_t* cutoff_index) {
     if (threadIdx.x == 0 && blockIdx.x == 0) {
         if (limit >= num_records) { *cutoff_index = num_records; return; }
         uint64_t boundary_val = sorted_prefixes[limit - 1];
         uint32_t idx = limit;
         while(idx < num_records && sorted_prefixes[idx] == boundary_val) { idx++; }
         *cutoff_index = idx;
     }
 }
 ...
 __global__ void gather_validity_mask_kernel(str_top_n_record_type* records, const cudf::bitmask_type* src_mask, cudf::bitmask_type* dst_mask, uint32_t num_records) {
     uint32_t idx = threadIdx.x + blockIdx.x * blockDim.x;
 ...
 __global__ void gather_fixed_width_128_kernel(str_top_n_record_type* records, uint8_t* src_data, uint8_t* dst_data, uint32_t num_records) {
     uint32_t idx = threadIdx.x + blockIdx.x * blockDim.x;
     if (idx < num_records) {
         uint32_t row_id = records[idx].row_id;
         if (row_id == 0xFFFFFFFF) return;
         ...
         for(int i=0; i<16; ++i) dst_data[dst_offset + i] = src_data[src_offset + i];
     }
 }
 
 template <typename T>
 __global__ void gather_fixed_width_kernel(str_top_n_record_type* records, uint8_t* src_data, uint8_t* dst_data, uint32_t num_records) {
     uint32_t idx = threadIdx.x + blockIdx.x * blockDim.x;
     if (idx < num_records) {
         uint32_t row_id = records[idx].row_id;
         if (row_id == 0xFFFFFFFF) return;
 ...
 __global__ void materialize_determine_lengths(str_top_n_record_type* ordered_records, uint64_t* src_col_offsets, uint64_t* result_lengths, uint64_t num_records) { 
     ...
     if(curr_record < num_records) { 
         ...
         if (row_id == 0xFFFFFFFF) { result_lengths[curr_record] = 0; return; }
         result_lengths[curr_record] = src_col_offsets[row_id + 1] - src_col_offsets[row_id];
     } else if(curr_record == num_records) result_lengths[curr_record] = 0;
 }
 ...
 __global__ void materialize_copy_string(str_top_n_record_type* ordered_records, uint8_t* src_chars, uint64_t* src_offsets, uint8_t* dst_chars, uint64_t* dst_offsets, uint64_t num_records) { 
     ...
     if(curr_record < num_records) {
         ...
         #pragma unroll
         for(uint64_t i = 0; i < record_length; i++) write_ptr[i] = read_ptr[i];
     }
 }
 ...
 KernelColType MapSiriusTypeToKernelType(GPUColumnTypeId type_id) {
     switch (type_id) {
         case GPUColumnTypeId::INT64: case GPUColumnTypeId::TIMESTAMP_SEC: case GPUColumnTypeId::TIMESTAMP_MS: case GPUColumnTypeId::TIMESTAMP_US: case GPUColumnTypeId::TIMESTAMP_NS: return KernelColType::INT_64;
         case GPUColumnTypeId::INT32: case GPUColumnTypeId::DATE: case GPUColumnTypeId::BOOLEAN: return KernelColType::INT_32;
         ...
         case GPUColumnTypeId::INT128: case GPUColumnTypeId::DECIMAL: return KernelColType::INT_128;
         default: return KernelColType::UNKNOWN;
     }
 }
 
 void PrepareDeviceColumns(vector<shared_ptr<GPUColumn>>& keys, OrderByType* order_by_type, idx_t num_keys, DeviceKeyColumn* d_cols_ptr, GPUBufferManager* gpuBufferManager) {
 ...
 void MaterializeResults(vector<shared_ptr<GPUColumn>>& projection, str_top_n_record_type* d_records, uint32_t final_count, GPUBufferManager* gpuBufferManager, idx_t num_projections) {
     ...
     for (int col_idx = 0; col_idx < num_projections; ++col_idx) {
         ...
         cudf::bitmask_type* d_new_mask = reinterpret_cast<cudf::bitmask_type*>(gpuBufferManager->customCudaMalloc<uint8_t>(mask_bytes, 0, 0));
         cudaMemset(d_new_mask, 0, mask_bytes);
         gather_validity_mask_kernel<<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.validity_mask, d_new_mask, final_count);
         
         if (kernel_type == KernelColType::STRING) {
             uint64_t* d_new_offsets = gpuBufferManager->customCudaMalloc<uint64_t>(final_count + 1, 0, 0);
             materialize_determine_lengths<<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.offset, d_new_offsets, final_count);
             void* d_temp = nullptr; size_t temp_bytes = 0;
             cub::DeviceScan::ExclusiveSum(d_temp, temp_bytes, d_new_offsets, d_new_offsets, final_count + 1);
             ...
             cudaMemcpy(&total_bytes, d_new_offsets + final_count, sizeof(uint64_t), cudaMemcpyDeviceToHost);
             uint8_t* d_new_chars = gpuBufferManager->customCudaMalloc<uint8_t>(total_bytes, 0, 0);
             materialize_copy_string<<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, src_wrapper.offset, d_new_chars, d_new_offsets, final_count);
             projection[col_idx] = make_shared_ptr<GPUColumn>(final_count, src_wrapper.type, d_new_chars, d_new_offsets, total_bytes, true, d_new_mask);
         } else {
             size_t type_size = src_wrapper.getColumnTypeSize();
             if (type_size == 0) {
                 switch(src_wrapper.type.id()) {
                     case GPUColumnTypeId::INT128: case GPUColumnTypeId::DECIMAL: type_size = 16; break;
                     case GPUColumnTypeId::INT64: case GPUColumnTypeId::FLOAT64: case GPUColumnTypeId::TIMESTAMP_SEC: case GPUColumnTypeId::TIMESTAMP_MS: case GPUColumnTypeId::TIMESTAMP_US: case GPUColumnTypeId::TIMESTAMP_NS: type_size = 8; break;
                     case GPUColumnTypeId::INT32: case GPUColumnTypeId::FLOAT32: case GPUColumnTypeId::DATE: type_size = 4; break;
                     case GPUColumnTypeId::INT16: type_size = 2; break; 
                     case GPUColumnTypeId::BOOLEAN: type_size = 1; break; 
                     default: type_size = 0; 
                 }
             }
             uint8_t* d_new_data = gpuBufferManager->customCudaMalloc<uint8_t>(final_count * type_size, 0, 0);
             if (type_size == 16) gather_fixed_width_128_kernel<<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, d_new_data, final_count);
             else if (type_size == 8) gather_fixed_width_kernel<uint64_t><<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, d_new_data, final_count);
             else if (type_size == 4) gather_fixed_width_kernel<uint32_t><<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, d_new_data, final_count);
             else if (type_size == 2) gather_fixed_width_kernel<uint16_t><<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, d_new_data, final_count);
             else if (type_size == 1) gather_fixed_width_kernel<uint8_t><<<gather_grid_size, gather_block_size>>>(d_records, src_wrapper.data, d_new_data, final_count);
             projection[col_idx] = make_shared_ptr<GPUColumn>(final_count, src_wrapper.type, d_new_data, d_new_mask);
         }
     }
 ...
 // Engine A: Heap Based
 void CustomMultiColumnTopN(vector<shared_ptr<GPUColumn>>& keys, vector<shared_ptr<GPUColumn>>& projection, idx_t num_keys, idx_t num_projections, OrderByType* order_by_type, idx_t num_results) {
     ...
     DeviceKeyColumn* d_cols = reinterpret_cast<DeviceKeyColumn*>(gpuBufferManager->customCudaMalloc<uint8_t>(num_keys * sizeof(DeviceKeyColumn), 0, 0));
     ...
     str_top_n_record_type* d_candidates_p1 = reinterpret_cast<str_top_n_record_type*>(gpuBufferManager->customCudaMalloc<uint8_t>(num_candidates_p1 * sizeof(str_top_n_record_type), 0, 0));
     per_thread_multi_col_top_k_kernel<<<num_blocks, num_threads>>>(d_cols, num_keys, num_records, num_results, d_candidates_p1);
     ...
     str_top_n_record_type* d_candidates_p2 = reinterpret_cast<str_top_n_record_type*>(gpuBufferManager->customCudaMalloc<uint8_t>(num_block_candidates * sizeof(str_top_n_record_type), 0, 0));
     per_block_multi_col_top_k_kernel<<<num_blocks, 1>>>(d_candidates_p1, num_candidates_p1, d_cols, num_keys, num_results, d_candidates_p2, num_threads);
     CUDA_CHECK_AND_SYNC("TopK Phase 2");
 
     // P3: Global Merge
     uint32_t final_count = (uint32_t)std::min((uint64_t)num_results, (uint64_t)num_records);
     str_top_n_record_type* d_records = reinterpret_cast<str_top_n_record_type*>(gpuBufferManager->customCudaMalloc<uint8_t>(final_count * sizeof(str_top_n_record_type), 0, 0));
     global_final_multi_col_top_k_kernel<<<1, 1>>>(d_candidates_p2, num_block_candidates, d_cols, num_keys, final_count, d_records);
 ...
 void CustomSingleColumnRadixTopN(vector<shared_ptr<GPUColumn>>& keys, vector<shared_ptr<GPUColumn>>& projection, idx_t num_keys, idx_t num_projections, OrderByType* order_by_type, idx_t num_results) {
     GPUBufferManager* gpuBufferManager = &(GPUBufferManager::GetInstance());
     cudaEvent_t start, stop; cudaEventCreate(&start); cudaEventCreate(&stop);
     cudaEventRecord(start, 0);
     ...
     create_radix_keys<<<num_create_workers, BLOCK_THREADS>>>(d_key_prefixes, d_row_ids, d_cols, num_records);
     ...
     if(order_by_type[0] == OrderByType::ASCENDING) { 
         cub::DeviceRadixSort::SortPairs(d_temp, temp_bytes, d_key_prefixes, d_sorted_prefixes, d_row_ids, d_sorted_row_ids, num_records);
         d_temp = gpuBufferManager->customCudaMalloc<uint8_t>(temp_bytes, 0, 0);
         cub::DeviceRadixSort::SortPairs(d_temp, temp_bytes, d_key_prefixes, d_sorted_prefixes, d_row_ids, d_sorted_row_ids, num_records);
     } else { 
         cub::DeviceRadixSort::SortPairsDescending(d_temp, temp_bytes, d_key_prefixes, d_sorted_prefixes, d_row_ids, d_sorted_row_ids, num_records);
         d_temp = gpuBufferManager->customCudaMalloc<uint8_t>(temp_bytes, 0, 0);
         cub::DeviceRadixSort::SortPairsDescending(d_temp, temp_bytes, d_key_prefixes, d_sorted_prefixes, d_row_ids, d_sorted_row_ids, num_records);
     }
     ...
     str_top_n_record_type* d_records = reinterpret_cast<str_top_n_record_type*>(gpuBufferManager->customCudaMalloc<uint8_t>(cutoff_idx * sizeof(str_top_n_record_type), 0, 0));
     uint32_t combine_workers = (cutoff_idx + BLOCK_THREADS - 1) / BLOCK_THREADS;
     combine_to_records<<<combine_workers, BLOCK_THREADS>>>(d_records, d_sorted_prefixes, d_sorted_row_ids, cutoff_idx);
     ...
     CustomExactSortComparator comparator(d_cols, num_keys, (order_by_type[0] == OrderByType::ASCENDING));
 ...
 void cudf_orderby(vector<shared_ptr<GPUColumn>>& keys, vector<shared_ptr<GPUColumn>>& projection, idx_t num_keys, idx_t num_projections, OrderByType* order_by_type, idx_t num_results) {
    ...
    if (Config::USE_CUSTOM_TOP_N && num_results > 0) {
        ...
        for (size_t col = 0; col < num_keys; col++) {
            if (MapSiriusTypeToKernelType(keys[col]->data_wrapper.type.id()) == KernelColType::UNKNOWN) {
                type_supported = false; break;
            }
        }
        for (size_t col = 0; col < num_projections; col++) {
            if (MapSiriusTypeToKernelType(projection[col]->data_wrapper.type.id()) == KernelColType::UNKNOWN) {
                type_supported = false; break;
            }
        }
        ...
        if (type_supported) {
            ...
            // Rationale: Even for single column, Heap Sort only needs one-pass scan, lower latency than Radix Sort (Multi-Pass).
            // This significantly speeds up queries like Q25, Q11 with LIMIT 10.
            ...
            // Rationale: As limit grows, heap maintenance cost increases, Radix Sort's high throughput advantage shows.
            else if (num_keys == 1) {
                 SIRIUS_LOG_DEBUG("Using Radix Sort Engine (Single Column Large Limit)");
                 CustomSingleColumnRadixTopN(keys, projection, num_keys, num_projections, order_by_type, num_results);
                 return;
            }
        }
    }
    ...
    for (int key = 0; key < num_keys; key++) columns_cudf.push_back(keys[key]->convertToCudfColumn());
    ...
    for (int col = 0; col < num_projections; col++) projection_cudf.push_back(projection[col]->convertToCudfColumn());
    ...
    for (int col = 0; col < num_projections; col++) {
        projection[col]->setFromCudfColumn(gathered_table->get_column(col), projection[col]->is_unique, nullptr, 0, gpuBufferManager);
    }
   }
