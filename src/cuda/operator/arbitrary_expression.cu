...
__device__ bool device_comparison_null(uint32_t validity_bit, CompareType compare_mode)
{
  ...
  if (compare_mode == IS_NULL) {
    ...
    flag = (validity_bit != 0);
  } else if (compare_mode == EQUAL || compare_mode == NOTEQUAL || compare_mode == GREATERTHAN ||
             compare_mode == GREATERTHANOREQUALTO || compare_mode == LESSTHAN ||
             compare_mode == LESSTHANOREQUALTO) {
...
__device__ int device_comparison_string(char* char_data,
                                        uint64_t char_len,
                                        char* compare_chars,
                                        ...
                                        CompareType compare_mode)
{
  ...
  if (compare_mode == EQUAL) {
    ...
    if (!found_answer) {
      is_valid = (is_inclusive && char_len == num_compare_chars) ||
                 (is_greater_check && char_len > num_compare_chars) ||
                 (!is_greater_check && char_len < num_compare_chars);
    }
  }
...
__global__ void table_scan_expression(uint8_t** col,
                                      uint64_t** offset,
                                      uint32_t** bitmask,
    ...
    for (int expr = 0; expr < num_expr; expr++) {
      if (threadIdx.x + ITEM * B < num_tile_items) {
        ...
        uint32_t validity_bit =
          bitmask[expr] ? (bitmask[expr][bitindex] >> bitoffset) & 0x00000001 : 1;
        ...
        if (data_type[expr] == SQLNULL) {
        ...
        } else if (data_type[expr] == INT32 || data_type[expr] == DATE ||
          ...
          selection_flags[ITEM] = device_comparison_string(
            curr_str_chars, curr_str_length, compare_chars, compare_length, compare_mode[expr]);
        } else {
          cudaAssert(0);
        }

        if (!selection_flags[ITEM]) { break; }
      }
    }
                         ...
                         int num_expr)
{
  ...
  cudaMemcpy(
    d_constant_compare, constant_compare, constant_size * sizeof(uint8_t), cudaMemcpyHostToDevice);
  CHECK_ERROR();
  cudaMemcpy(
    d_constant_offset, constant_offset, (num_expr + 1) * sizeof(uint64_t), cudaMemcpyHostToDevice);
  CHECK_ERROR();

  CompareType* d_compare_mode =
    (CompareType*)gpuBufferManager->customCudaMalloc<int>(num_expr, 0, 0);
  cudaMemcpy(d_compare_mode, compare_mode, num_expr * sizeof(int), cudaMemcpyHostToDevice);
  ScanDataType* d_data_type =
    (ScanDataType*)gpuBufferManager->customCudaMalloc<int>(num_expr, 0, 0);
  ...
  table_scan_expression<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(d_col,
                                                           d_offset,
                                                           d_bitmask,
                                                           d_constant_compare,
                                                           d_constant_offset,
