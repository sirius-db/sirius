...
struct MemmoveScanOp {
  __device__ __forceinline__ bool operator()(bool l, bool r) const { return l && r; }
};
                               ...
                               cub::ScanTileState<bool> scan_tile_state)
{
  ...
  using block_load_t          = cub::BlockLoad<std::uint32_t, B, I, cub::BLOCK_LOAD_STRIPED>;
  using block_load_storage_t  = typename block_load_t::TempStorage;
  using block_store_t         = cub::BlockStore<std::uint32_t, B, I, cub::BLOCK_STORE_STRIPED>;
  ...
  if (blockIdx.x == 0) {
    if (threadIdx.x == 0) { scan_tile_state.SetInclusive(blockIdx.x, true); }
  } else {
...
void cudaMemmove(uint8_t* destination, uint8_t* source, size_t num)
{
  ...
  if (destination + num < source) {
    ...
    uint8_t* tile_state_storage =
      gpuBufferManager->customCudaMalloc<uint8_t>(tile_state_storage_bytes, 0, 0);
    ...
    scan_tile_state_init_kernel<<<ceil_div(num_tiles, 256), 256>>>(scan_tile_state, num_tiles);
