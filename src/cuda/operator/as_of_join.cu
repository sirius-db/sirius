                        ...
                        int num_keys)
{
  ...
  SIRIUS_LOG_DEBUG("Launching As Of Join Kernel");
  ...
  for (int ITEM = 0; ITEM < I; ITEM++) {
    if (threadIdx.x + (ITEM * B) < num_tile_items) {
      ...
      while (ht[slot * n_ht_column] != 0xFFFFFFFFFFFFFFFF) {
        ...
        if (local_found) {
          // In an ASOF join, we only care about the latest timestamp, so there can only be one
          // match per slot
          items_count[ITEM] = 1;
        }
  ...
  for (int ITEM = 0; ITEM < I; ITEM++) {
    ...
    if (threadIdx.x + (ITEM * B) < num_tile_items) {
      ...
      while (ht[slot * n_ht_column] != 0xFFFFFFFFFFFFFFFF) {
        ...
        if (local_found) {
          if (ht[slot * n_ht_column + num_keys + 1] >= max_timestamp) {
            max_timestamp = ht[slot * n_ht_column + num_keys + 1];

            row_ids_right[output_offset] = ht[slot * n_ht_column + num_keys];
            row_ids_left[output_offset]  = tile_offset + threadIdx.x + ITEM * B;
            if (is_right)
              ht[slot * n_ht_column + num_keys + 1] = tile_offset + threadIdx.x + ITEM * B;
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ITEM++) {
    if (threadIdx.x + (ITEM * B) < num_tile_items) {
      ...
      while (atomicCAS(&ht[slot * n_ht_column], 0xFFFFFFFFFFFFFFFF, (unsigned long long)item) !=
             0xFFFFFFFFFFFFFFFF) {
        slot = (slot + 65599) % ht_len;
      }
      ...
      ht[slot * n_ht_column + num_keys + 1] = timestamp[tile_offset + threadIdx.x + ITEM * B];
    }
  }
}
                            ...
                            bool is_right)
{
  ...
  if (is_right)
    cudaMemset(ht, 0xFF, ht_len * (num_keys + 2) * sizeof(unsigned long long));
  else
    cudaMemset(ht, 0xFF, ht_len * (num_keys + 1) * sizeof(unsigned long long));
  ...
  gpuBufferManager->customCudaFree(reinterpret_cast<uint8_t*>(timestamps_dev), 0);
}
                            ...
                            bool is_right)
{
  ...
  cudaMemset(count, 0, sizeof(uint64_t));
  uint64_t* offset_each_thread = gpuBufferManager->customCudaMalloc<uint64_t>(
    ((N + tile_items - 1) / tile_items) * BLOCK_THREADS, 0, 0);
  ...
  CHECK_ERROR();
  probe_multikey_count_asof<BLOCK_THREADS, ITEMS_PER_THREAD, T>
  ...
  // gpuBufferManager->gpuProcessingPointer[0] - 1024) / sizeof(uint64_t); size_t openmalloc_half =
  // openmalloc_full / 2; row_ids_left =
  // gpuBufferManager->customCudaMalloc<uint64_t>(openmalloc_half, 0, 0); row_ids_right =
  ...
  row_ids_left  = gpuBufferManager->customCudaMalloc<uint64_t>(h_count[0], 0, 0);
  row_ids_right = gpuBufferManager->customCudaMalloc<uint64_t>(h_count[0], 0, 0);
  probe_multikey_asof<BLOCK_THREADS, ITEMS_PER_THREAD, T>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(keys_dev,
                                                           timestamps_dev,
                                                           ht,
                                                           ht_len,
  ...
  // reinterpret_cast<uint8_t*>(row_ids_right), h_count[0] * sizeof(uint64_t)); CHECK_ERROR();
  // row_ids_right = row_ids_left + h_count[0];
  // gpuBufferManager->gpuProcessingPointer[0] = (reinterpret_cast<uint8_t*>(row_ids_right +
  // h_count[0]) - gpuBufferManager->gpuProcessing[0]);
  gpuBufferManager->customCudaFree(reinterpret_cast<uint8_t*>(offset_each_thread), 0);
