...
__global__ void determine_start_kernel(const IdxT* indices,
                                       IdxT num_strings,
                                       IdxT* worker_start_term,
                                       IdxT num_workers,
                                       IdxT chunk_size,
                                       IdxT last_char)
{
  ...
  IdxT search_start_term = 0;
  IdxT search_end_term   = num_strings;
  ...
  while (search_start_term <= search_end_term) {
    ...
    if (curr_chunk_start >= indices[curr_search_term] &&
        curr_chunk_start < indices[curr_search_term + 1]) {
...
template <typename IdxT>
__global__ void single_term_kmp_kernel(const char* char_data,
                                       const IdxT* indices,
                                       const int* kmp_automato,
                                       const IdxT* worker_start_term,
                                       bool* results,
                                       IdxT pattern_size,
                                       IdxT num_workers,
                                       IdxT chunk_size,
                                       ...
                                       IdxT num_strings)
{
  ...
  uint64_t chunk_id = static_cast<uint64_t>(blockIdx.x);
  ...
  const uint64_t curr_sub_chunk_start =
    min(curr_chunk_start + threadIdx.x * sub_chunk_size, curr_chunk_end);
  const uint64_t curr_sub_chunk_end =
    min(curr_sub_chunk_start + sub_chunk_size + pattern_size, curr_chunk_end);

  // Determine the subchunk that the current string is going to be working on
  uint64_t curr_term = worker_start_term[chunk_id];
  while (curr_term < num_strings && (curr_sub_chunk_start < indices[curr_term] ||
                                     curr_sub_chunk_start >= indices[curr_term + 1])) {
    curr_term++;
  }
...
#pragma unroll
  for (uint64_t i = curr_sub_chunk_start; i <= curr_sub_chunk_end; i++) {
    ...
    curr_idx = (uint64_t)char_data[i] + CHAR_INCREMENT;
...
__global__ void write_matching_rows(bool* results,
                                    uint64_t num_strings,
                    ...
                    int not_equal)
{
  ...
  uint64_t* d_worker_start_term =
    gpuBufferManager->customCudaMalloc<uint64_t>(workers_needed, 0, 0);
  bool* d_answers =
    reinterpret_cast<bool*>(gpuBufferManager->customCudaMalloc<uint8_t>(num_strings, 0, 0));
  ...
  determine_start_kernel<uint64_t><<<preprocess_blocks_needed, THREADS_PER_BLOCK_STRINGS>>>(
    str_indices, num_strings, d_worker_start_term, workers_needed, CHUNK_SIZE, last_char);
  cudaDeviceSynchronize();
  auto preprocessing_end = std::chrono::high_resolution_clock::now();
  int preprocessing_time_us =
    std::chrono::duration_cast<std::chrono::microseconds>(preprocessing_end - preprocessing_start)
      .count();

  auto str_match_start = std::chrono::high_resolution_clock::now();
  uint64_t block_sub_chunk_size =
    (CHUNK_SIZE + THREADS_PER_BLOCK_STRINGS - 1) / THREADS_PER_BLOCK_STRINGS;
  single_term_kmp_kernel<uint64_t>
    <<<workers_needed, THREADS_PER_BLOCK_STRINGS>>>(char_data,
                                                    str_indices,
                                                    d_kmp_automato,
                                                    d_worker_start_term,
                                                    d_answers,
                                                    ...
                                                    workers_needed,
                                                    CHUNK_SIZE,
                                                    block_sub_chunk_size,
                                                    last_char,
                                                    num_strings);
  cudaDeviceSynchronize();
  auto str_match_end = std::chrono::high_resolution_clock::now();
  int str_match_time_us =
    std::chrono::duration_cast<std::chrono::microseconds>(str_match_end - str_match_start).count();
  CHECK_ERROR();

  cudaMemset(count, 0, sizeof(uint64_t));
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(d_answers, row_id, (unsigned long long*)count, num_strings, 1, not_equal);
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(d_answers, row_id, (unsigned long long*)count, num_strings, 0, not_equal);
...
//--------------------------------------------------//
template <typename IdxT>
__global__ void multi_term_kmp_kernel(const char* char_data,
                                      const IdxT* indices,
                                      const int* kmp_automato,
                                      IdxT* worker_start_term,
                                      IdxT* curr_term_answer,
                                      IdxT* prev_term_answer,
                                      bool* found_term,
                                      int pattern_size,
                                      IdxT num_workers,
                                      IdxT chunk_size,
...
#pragma unroll
  for (uint64_t i = curr_sub_chunk_start; i <= curr_sub_chunk_end; i++) {
    ...
    // Record that we have a hit
    if (j >= pattern_size) {
      // Only write the result if we current match index is > than the lowest match index for the
      // previous term
      if (i >= prev_term_answer[curr_term]) {
        found_term[curr_term] = true;
        cuda::atomic_ref<IdxT, cuda::thread_scope_device> curr_term_answer_ref(
          curr_term_answer[curr_term]);
...
__global__ void initialize_term_answers(IdxT* curr_term_answer, IdxT num_chars, IdxT num_strings)
{
  ...
  if (tid < num_strings) { curr_term_answer[tid] = num_chars; }
}
...
template __global__ void initialize_term_answers<uint64_t>(uint64_t* curr_term_answer,
                                                           uint64_t num_chars,
                                                           uint64_t num_strings);
template __global__ void initialize_term_answers<int64_t>(int64_t* curr_term_answer,
                                                          int64_t num_chars,
                                                          int64_t num_strings);
                         ...
                         int not_equal)
{
  ...
  for (int i = 0; i < num_terms; i++) {
    ...
    d_all_automatos[i] =
      gpuBufferManager->customCudaMalloc<int>(kmp_automato_size * sizeof(int), 0, 0);
  }

  // Copy over the necessary data
  cudaMemcpy(
    d_prev_term_answers, str_indices, num_strings * sizeof(uint64_t), cudaMemcpyDeviceToDevice);
  for (int i = 0; i < num_terms; i++) {
    int kmp_automato_size = all_terms[i].size() * CHARS_IN_BYTE;
    cudaMemcpy(d_all_automatos[i],
               all_terms_automato[i],
               kmp_automato_size * sizeof(int),
  ...
  uint64_t last_char = num_chars - 1;
  uint64_t kernel_block_needed =
    (workers_needed + THREADS_PER_BLOCK_STRINGS - 1) / THREADS_PER_BLOCK_STRINGS;
  uint64_t block_sub_chunk_size =
    (CHUNK_SIZE + THREADS_PER_BLOCK_STRINGS - 1) / THREADS_PER_BLOCK_STRINGS;
  determine_start_kernel<uint64_t><<<kernel_block_needed, THREADS_PER_BLOCK_STRINGS>>>(
    str_indices, num_strings, d_worker_start_term, workers_needed, CHUNK_SIZE, last_char);
  CHECK_ERROR();

  // Perform the string matching term by term
  uint64_t preprocess_num_blocks =
    (num_strings + THREADS_PER_BLOCK_STRINGS - 1) / THREADS_PER_BLOCK_STRINGS;
  for (int i = 0; i < num_terms; i++) {
    ...
    cudaMemset(d_found_answer, 0, num_strings * sizeof(bool));
    initialize_term_answers<uint64_t>
      <<<preprocess_num_blocks, THREADS_PER_BLOCK_STRINGS>>>(d_answer_idxs, num_chars, num_strings);
    CHECK_ERROR();

    // Run the search
    multi_term_kmp_kernel<uint64_t>
      <<<workers_needed, THREADS_PER_BLOCK_STRINGS>>>(char_data,
                                                      str_indices,
                                                      curr_term_automato,
                                                      d_worker_start_term,
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(
      d_found_answer, row_id, (unsigned long long*)count, num_strings, 1, not_equal);
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(
      d_found_answer, row_id, (unsigned long long*)count, num_strings, 0, not_equal);
...
//--------------------------------------------------//
template <typename IdxT>
__global__ void prefix_kernel(const char* char_data,
                              IdxT num_chars,
                              const IdxT* str_indices,
                              IdxT num_strings,
                              const char* prefix_chars,
                              IdxT num_prefix_chars,
                              bool* results)
{
                    ...
                    int not_equal)
{
  ...
  cudaMemcpy(
    d_prefix_chars, match_prefix.c_str(), num_prefix_chars * sizeof(char), cudaMemcpyHostToDevice);
  ...
  prefix_kernel<<<num_blocks, BLOCK_THREADS>>>(
    char_data, num_chars, str_indices, num_strings, d_prefix_chars, num_prefix_chars, d_results);
  cudaDeviceSynchronize();
  CHECK_ERROR();
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(d_results, row_id, (unsigned long long*)count, num_strings, 1, not_equal);
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(d_results, row_id, (unsigned long long*)count, num_strings, 0, not_equal);
                                               ...
                                               rmm::cuda_stream_view stream)
{
  ...
  const auto workers_needed =
    cuda::ceil_div((uint64_t)byte_count, static_cast<uint64_t>(CHUNK_SIZE));
  rmm::device_uvector<uint64_t> d_worker_start_term(workers_needed, stream, mr);
  ...
  LAUNCH_KERNEL_DIV(determine_start_kernel,
                    uint64_t,
                    workers_needed,
                    static_cast<uint64_t>(THREADS_PER_BLOCK_STRINGS),
                    stream)
  (input_offsets,
   input_count,
  ...
  LAUNCH_KERNEL_DIRECT(
    single_term_kmp_kernel, uint64_t, workers_needed, THREADS_PER_BLOCK_STRINGS, stream)
  (input_data,
   input_offsets,
   d_kmp_automato.data(),
   d_worker_start_term.data(),
                                                    ...
                                                    rmm::cuda_stream_view stream)
{
  ...
  const auto workers_needed =
    cuda::ceil_div((uint64_t)byte_count, static_cast<uint64_t>(CHUNK_SIZE));
  rmm::device_uvector<uint64_t> d_worker_start_term(workers_needed, stream, mr);
  rmm::device_uvector<uint64_t> d_answer_idxs(input_count, stream, mr);
  rmm::device_uvector<uint64_t> d_prev_answer_idxs(input_count, stream, mr);
  rmm::device_uvector<bool> output(input_count, stream, mr);
  ...
  for (int32_t i = 0; i < match_strings.size(); i++) {
    ...
    CUDF_CUDA_TRY(cudaMemsetAsync(output.data(), 0, input_count * sizeof(bool), stream));
    LAUNCH_KERNEL_DIV(
      initialize_term_answers, uint64_t, input_count, THREADS_PER_BLOCK_STRINGS, stream)
    (answer_idxs_ptr, byte_count, input_count);

    // Launch the KMP kernel for the current term
    LAUNCH_KERNEL_DIRECT(
      multi_term_kmp_kernel, uint64_t, workers_needed, THREADS_PER_BLOCK_STRINGS, stream)
    (input_data,
     input_offsets,
     curr_term_automato,
     d_worker_start_term.data(),
                                               ...
                                               rmm::cuda_stream_view stream)
{
  ...
  CUDF_CUDA_TRY(cudaMemcpyAsync(
    d_match_prefix.data(), match_prefix.data(), prefix_byte_count, cudaMemcpyHostToDevice, stream));
