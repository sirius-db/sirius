...
__global__ void determine_start_kernel(uint64_t* indices, uint64_t num_strings, uint64_t* worker_start_term, uint64_t num_workers, uint64_t chunk_size, uint64_t last_char) {  
  ...
  uint64_t curr_worker_start = -1; uint64_t curr_search_term;
...
__global__ void write_matching_rows(bool* results, uint64_t num_strings, uint64_t* matching_rows, uint64_t* num_match_rows) {
...
__global__ void multi_term_kmp_kernel(char* char_data, int* indices, int* kmp_automato, int* worker_start_term, 
  ...
  int last_char, int num_strings) {
    ...
    int curr_term_end = indices[curr_term + 1];
...
uint64_t* StringMatching(char* char_data, uint64_t* str_indices, std::string match_string, uint64_t* num_match_rows, uint64_t num_chars, uint64_t num_strings) {
  ...
  char* d_match_str = gpuBufferManager->customCudaMalloc<char>(match_string.length() * sizeof(char), 0, 0);
  int* d_kmp_automato = gpuBufferManager->customCudaMalloc<int>(kmp_automato_size * sizeof(int), 0, 0);
  uint64_t* d_worker_start_term = gpuBufferManager->customCudaMalloc<uint64_t>(workers_needed * sizeof(uint64_t), 0, 0);
  bool* d_answers = gpuBufferManager->customCudaMalloc<bool>(num_strings * sizeof(bool), 0, 0);
  uint64_t* d_matching_rows = gpuBufferManager->customCudaMalloc<uint64_t>(num_strings * sizeof(uint64_t), 0, 0);
  uint64_t* d_num_match = gpuBufferManager->customCudaMalloc<uint64_t>(sizeof(uint64_t), 0, 0);
  ...
  int num_match_blocks = max(1, (num_strings + THREADS_PER_BLOCK_STRINGS - 1)/(THREADS_PER_BLOCK_STRINGS * TILE_ITEMS_PER_TILE));
  write_matching_rows<<<num_match_blocks, THREADS_PER_BLOCK_STRINGS>>>(d_answers, num_strings, d_matching_rows, d_num_match);
