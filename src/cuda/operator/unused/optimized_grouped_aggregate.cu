...
struct string_groupby_metadata {
  ...
  __host__ __device__ string_groupby_metadata(uint8_t** _all_keys,
                                              uint64_t** _offsets,
                                              uint64_t _num_keys)
...
struct string_group_by_record {
  ...
  __host__ __device__ string_group_by_record(string_groupby_metadata* _metadata,
                                             uint64_t _row_id,
                                             uint64_t _row_signature)
  ...
  __device__ __forceinline__ bool operator==(const string_group_by_record& other) const
  {
    ...
    if (row_signature != other.row_signature) { return false; }
    ...
    for (uint64_t i = 0; i < group_by_metadata->num_keys; i++) {
      ...
      uint64_t right_length =
        curr_column_offsets[other.row_id + 1] - curr_column_offsets[other.row_id];
    ...
    for (uint64_t i = 0; i < group_by_metadata->num_keys; i++) {
      ...
      while (bytes_remaining > 0) {
        ...
        uint64_t batch_size =
          min(BYTES_IN_INTEGER - max(left_read_idx, right_read_idx), bytes_remaining);
  ...
  {
    // First compare the signature
    if (row_signature != other.row_signature) { return row_signature < other.row_signature; }
...
struct CustomLessGroupByRecord {
  ...
  __device__ __forceinline__ bool operator()(const string_group_by_record& lhs,
                                             const string_group_by_record& rhs)
  {
    return lhs < rhs;
  }
};
...
__global__ void create_metadata_record(string_groupby_metadata* group_by_metadata,
                                       uint8_t** keys,
                                       uint64_t** column_length_offsets,
...
__global__ void create_row_records(string_groupby_metadata* group_by_metadata,
                                   string_group_by_record* row_records,
...
template <typename V>
__global__ void populate_aggregate_buffer(uint8_t** aggregate_input_keys,
                                          V* aggregate_write_buffer,
                                          uint64_t* aggregate_idxs,
                                          string_group_by_record* group_by_row_records,
                                          int* agg_mode,
                                          const uint64_t num_rows,
...
template <typename V>
__global__ void perform_average_reduction(V* aggreate_col_keys,
...
__global__ void populate_original_row_ids(string_group_by_record* grouped_row_records,
                                          uint64_t* original_row_ids,
                                          uint64_t num_groups)
{
  const uint64_t curr_idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (curr_idx < num_groups) { original_row_ids[curr_idx] = grouped_row_records[curr_idx].row_id; }
}

template <typename V>
void optimizedGroupedStringAggregate(uint8_t** keys,
                                     uint8_t** aggregate_keys,
                                     uint64_t** offset,
                                     uint64_t* num_bytes,
                                     uint64_t* count,
                                     uint64_t N,
                                     ...
                                     int* agg_mode)
{
  ...
  uint8_t** d_keys =
    reinterpret_cast<uint8_t**>(gpuBufferManager->customCudaMalloc<void*>(num_keys, 0, 0));
  cudaMemcpy(d_keys, keys, num_keys * sizeof(uint8_t*), cudaMemcpyHostToDevice);
  uint64_t** d_offset =
    reinterpret_cast<uint64_t**>(gpuBufferManager->customCudaMalloc<void*>(num_keys, 0, 0));
  cudaMemcpy(d_offset, offset, num_keys * sizeof(uint64_t*), cudaMemcpyHostToDevice);

  // Create the metadata record
  string_groupby_metadata* d_group_by_metadata = reinterpret_cast<string_groupby_metadata*>(
    gpuBufferManager->customCudaMalloc<string_group_by_metadata_type>(1, 0, 0));
  ...
  string_group_by_record* d_row_records = reinterpret_cast<string_group_by_record*>(
    gpuBufferManager->customCudaMalloc<string_group_by_record_type>(N, 0, 0));
  uint64_t preprocess_items_per_block = BLOCK_THREADS * ITEMS_PER_THREAD;
  uint64_t num_preprocess_blocks =
    (N + preprocess_items_per_block - 1) / preprocess_items_per_block;
  create_row_records<<<num_preprocess_blocks, BLOCK_THREADS>>>(
    d_group_by_metadata, d_row_records, N);
  ...
  cub::DeviceMergeSort::SortKeys(
    sort_temp_storage, sort_temp_storage_bytes, d_row_records, N, custom_less_operator);

  sort_temp_storage = reinterpret_cast<void*>(
    gpuBufferManager->customCudaMalloc<uint8_t>(sort_temp_storage_bytes, 0, 0));
  ...
  // Create a separate buffer storing the aggregate values for each sorted row record
  ...
  uint8_t** d_aggregate_keys =
    reinterpret_cast<uint8_t**>(gpuBufferManager->customCudaMalloc<void*>(num_aggregates, 0, 0));
  cudaMemcpy(
    d_aggregate_keys, aggregate_keys, num_aggregates * sizeof(uint8_t*), cudaMemcpyHostToDevice);
  ...
  populate_aggregate_buffer<<<num_aggregate_blocks, BLOCK_THREADS>>>(d_aggregate_keys,
                                                                     d_aggregate_buffer,
                                                                     d_aggregate_idxs,
                                                                     d_row_records,
                                                                     d_agg_mode,
  ...
  string_group_by_record* d_result_row_records = reinterpret_cast<string_group_by_record*>(
    gpuBufferManager->customCudaMalloc<string_group_by_record_type>(N, 0, 0));
  ...
  cub::DeviceReduce::ReduceByKey(d_group_by_temp_storage,
  ...
  d_group_by_temp_storage =
    gpuBufferManager->customCudaMalloc<uint8_t>(group_by_temp_storage_bytes, 0, 0);
  ...
  // The resulting values of the previous ReduceByKey is an array like 0 100 300 325 450
  // representing the starting index for each group Now we can run DeviceSegmentedReduce for each
  // aggregate column without having to redo the string comparison This reduction will also write
  // the result to the aggregate column directly and thus we don't need to any sort of
  ...
  for (int i = 0; i < num_aggregates; i++) {
    ...
    if (agg_mode[i] == 2) {
      ...
      cub::DeviceSegmentedReduce::Reduce(d_aggregate_temp_storage,
      ...
      d_aggregate_temp_storage =
        gpuBufferManager->customCudaMalloc<uint8_t>(aggregate_temp_storage_bytes, 0, 0);
      ...
      if (agg_mode[i] == 1) {
        // Since this is an average reduction then divide the result by the number of records in the
        // group
        uint64_t num_group_blocks = (num_groups + BLOCK_THREADS - 1) / BLOCK_THREADS;
        perform_average_reduction<<<num_group_blocks, BLOCK_THREADS>>>(
          aggreate_col_keys, d_result_aggregate_idxs, num_groups);
      }
    }
  }
  ...
  populate_original_row_ids<<<populate_num_blocks, BLOCK_THREADS>>>(
    d_result_row_records, d_original_row_ids, num_groups);
  ...
  for (uint64_t i = 0; i < num_keys; i++) {
    ...
    // &result_offset, uint64_t* row_ids, uint64_t* &result_bytes, uint64_t result_len, uint64_t
    // input_size, uint64_t input_bytes)
    materializeString(group_key_chars,
                      group_key_offsets,
                      result,
                      result_offset,
                      d_original_row_ids,
...
template void optimizedGroupedStringAggregate<double>(uint8_t** keys,
                                                      uint8_t** aggregate_keys,
                                                      uint64_t** offset,
                                                      uint64_t* num_bytes,
                                                      uint64_t* count,
...
template void optimizedGroupedStringAggregate<uint64_t>(uint8_t** keys,
                                                        uint8_t** aggregate_keys,
                                                        uint64_t** offset,
                                                        uint64_t* num_bytes,
                                                        uint64_t* count,
...
__global__ void populate_fixed_size_offsets(uint64_t* offsets,
                                            uint64_t record_size,
                                            uint64_t num_records)
{
  uint64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_records) { offsets[idx] = idx * record_size; }
}
...
uint64_t* createFixedSizeOffsets(size_t record_size, uint64_t num_rows)
{
  ...
  populate_fixed_size_offsets<<<num_blocks, BLOCK_THREADS>>>(
    d_offsets, static_cast<uint64_t>(record_size), records_to_populate);
  return d_offsets;
}
