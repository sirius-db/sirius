...
struct OrderByCustomComparator {
  ...
  __host__ __device__ OrderByCustomComparator(uint8_t** _d_col_keys,
                                              uint64_t** _d_col_offsets,
                                              int* _d_sort_orders,
                                              uint64_t _num_cols)
    : d_col_keys(_d_col_keys),
      d_col_offsets(_d_col_offsets),
      d_sort_orders(_d_sort_orders),
      num_cols(_num_cols)
  {
  }
  ...
  __device__ __forceinline__ bool operator()(const uint64_t& left_idx, const uint64_t& right_idx)
  {
    for (uint64_t i = 0; i < num_cols; i++) {
      ...
      while (bytes_remaining > 0) {
        ...
        // If they are not equal then actually perform the comparison character by character
...
__global__ void fill_row_ids_buffer(uint64_t* d_row_ids, uint64_t num_rows)
{
  ...
  if (thread_idx < num_rows) { d_row_ids[thread_idx] = thread_idx; }
}
                   ...
                   uint64_t num_cols)
{
  ...
  uint8_t** d_col_keys =
    reinterpret_cast<uint8_t**>(gpuBufferManager->customCudaMalloc<void*>(num_cols, 0, 0));
  cudaMemcpy(d_col_keys, col_keys, num_cols * sizeof(uint8_t*), cudaMemcpyHostToDevice);
  uint64_t** d_col_offsets =
    reinterpret_cast<uint64_t**>(gpuBufferManager->customCudaMalloc<void*>(num_cols, 0, 0));
  ...
  int* d_sort_orders =
    reinterpret_cast<int*>(gpuBufferManager->customCudaMalloc<int>(num_cols, 0, 0));
  ...
  cub::DeviceMergeSort::SortKeys(
    sort_temp_storage, sort_temp_storage_bytes, d_row_ids, num_rows, order_by_comparator);
  ...
  for (uint64_t i = 0; i < num_cols; i++) {
    ...
    uint8_t* sorted_chars;
    uint64_t* sorted_offsets;
    uint64_t* new_num_bytes;
    materializeString(unsorted_col_chars,
                      unsorted_col_offsets,
                      sorted_chars,
                      sorted_offsets,
                      d_row_ids,
                      new_num_bytes,
