...
__global__ void comparison_expression(const T* a,
                                      const T* b,
                                      uint64_t* row_ids,
...
template <typename T, int B, int I>
__global__ void comparison_constant_expression(const T* a,
                                               const T b,
                                               const T c,
                                               uint64_t* row_ids,
                                               unsigned long long* count,
...
template __global__ void comparison_expression<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const int* a,
  const int* b,
  uint64_t* row_ids,
  unsigned long long* count,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_expression<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint64_t* a,
  const uint64_t* b,
  uint64_t* row_ids,
  unsigned long long* count,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_expression<float, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const float* a,
  const float* b,
  uint64_t* row_ids,
  unsigned long long* count,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_expression<double, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const double* a,
  const double* b,
  uint64_t* row_ids,
  unsigned long long* count,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_expression<uint8_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint8_t* a,
  const uint8_t* b,
  uint64_t* row_ids,
  unsigned long long* count,
...
template __global__ void comparison_constant_expression<int, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const int* a,
  const int b,
  const int c,
  uint64_t* row_ids,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_constant_expression<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint64_t* a,
  const uint64_t b,
  const uint64_t c,
  uint64_t* row_ids,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_constant_expression<float, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const float* a,
  const float b,
  const float c,
  uint64_t* row_ids,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_constant_expression<double, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const double* a,
  const double b,
  const double c,
  uint64_t* row_ids,
  ...
  int compare_mode,
  int is_count);
template __global__ void comparison_constant_expression<uint8_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint8_t* a,
  const uint8_t b,
  const uint8_t c,
  uint64_t* row_ids,
  ...
  T* a, T b, T c, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode)
{
  ...
  // size_t openmalloc_full = (gpuBufferManager->processing_size_per_gpu -
  // gpuBufferManager->gpuProcessingPointer[0] - 1024) / sizeof(uint64_t); row_ids =
  // gpuBufferManager->customCudaMalloc<uint64_t>(openmalloc_full, 0, 0);
  row_ids = gpuBufferManager->customCudaMalloc<uint64_t>(N, 0, 0);
  comparison_constant_expression<T, BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      a, b, c, row_ids, (unsigned long long*)count, N, op_mode, 0);
  CHECK_ERROR();
  cudaDeviceSynchronize();
  uint64_t* h_count = gpuBufferManager->customCudaHostAlloc<uint64_t>(1);
  cudaMemcpy(h_count, count, sizeof(uint64_t), cudaMemcpyDeviceToHost);
  CHECK_ERROR();
  // gpuBufferManager->gpuProcessingPointer[0] = (reinterpret_cast<uint8_t*>(row_ids + h_count[0]) -
...
void comparisonExpression(T* a, T* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode)
{
  ...
  // gpuBufferManager->customCudaMalloc<uint64_t>(openmalloc_full, 0, 0);
  row_ids = gpuBufferManager->customCudaMalloc<uint64_t>(N, 0, 0);
  comparison_expression<T, BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      a, b, row_ids, (unsigned long long*)count, N, op_mode, 0);
  CHECK_ERROR();
  cudaDeviceSynchronize();
  uint64_t* h_count = gpuBufferManager->customCudaHostAlloc<uint64_t>(1);
  cudaMemcpy(h_count, count, sizeof(uint64_t), cudaMemcpyDeviceToHost);
                                             ...
                                             bool* d_results)
{
  ...
  for (uint64_t i = start_idx; i < num_strings; i += tile_size) {
    ...
    if (op_mode == 0 || op_mode == 1) {
      ...
      if (!found_answer) {
        is_valid = (is_inclusive && curr_str_length == num_compare_chars) ||
                   (is_greater_check && curr_str_length > num_compare_chars) ||
                   (!is_greater_check && curr_str_length < num_compare_chars);
      }
    }
                                       ...
                                       uint64_t*& count)
{
  ...
  cudaMemcpy(
    d_lower_chars, lower_string.c_str(), num_lower_chars * sizeof(char), cudaMemcpyHostToDevice);
  ...
  // Perform the upper string comparison
  if (is_upper_inclusive)
    op_mode = 5;
  else
    op_mode = 4;
  string_comparison_expression<<<num_blocks, BLOCK_THREADS>>>(char_data,
  ...
  // size_t openmalloc_full = (gpuBufferManager->processing_size_per_gpu -
  // gpuBufferManager->gpuProcessingPointer[0] - 1024) / sizeof(uint64_t); row_id =
  // gpuBufferManager->customCudaMalloc<uint64_t>(openmalloc_full, 0, 0);
  ...
  compact_valid_rows<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((num_strings + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD)),
       BLOCK_THREADS>>>(d_is_valid, row_id, (unsigned long long*)count, num_strings, 0, 0);
  CHECK_ERROR();
  uint64_t* h_count = gpuBufferManager->customCudaHostAlloc<uint64_t>(1);
  cudaMemcpy(h_count, count, sizeof(uint64_t), cudaMemcpyDeviceToHost);
  CHECK_ERROR();
  // gpuBufferManager->gpuProcessingPointer[0] = (reinterpret_cast<uint8_t*>(row_id + h_count[0]) -
  // gpuBufferManager->gpuProcessing[0]);
  gpuBufferManager->customCudaFree(reinterpret_cast<uint8_t*>(count), 0);
  count = h_count;
                                ...
                                uint64_t*& count)
{
  ...
  cudaMemcpy(d_compare_chars,
             comparison_string.c_str(),
             num_compare_chars * sizeof(char),
...
template void comparisonConstantExpression<int>(
  int* a, int b, int c, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonConstantExpression<uint64_t>(uint64_t* a,
                                                     uint64_t b,
                                                     uint64_t c,
                                                     uint64_t*& row_ids,
                                                     uint64_t*& count,
                                                     uint64_t N,
                                                     int op_mode);
template void comparisonConstantExpression<float>(
  float* a, float b, float c, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonConstantExpression<double>(
  double* a, double b, double c, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonConstantExpression<uint8_t>(
  uint8_t* a, uint8_t b, uint8_t c, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);

template void comparisonExpression<int>(
  int* a, int* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonExpression<uint64_t>(
  uint64_t* a, uint64_t* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonExpression<float>(
  float* a, float* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonExpression<double>(
  double* a, double* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
template void comparisonExpression<uint8_t>(
  uint8_t* a, uint8_t* b, uint64_t*& row_ids, uint64_t*& count, uint64_t N, int op_mode);
