...
__global__ void perform_string_comparsion(char* char_data, uint64_t num_chars, uint64_t* str_indices, uint64_t num_strings, char* compare_chars, uint64_t compare_length, 
...
void comparsionStringExpression(char* char_data, uint64_t num_chars, uint64_t* str_indices, uint64_t num_strings, std::string lower_string, std::string upper_string, 
    ...
    perform_string_comparsion<<<num_blocks, BLOCK_THREADS>>>(char_data, num_chars, str_indices, num_strings, d_lower_chars, num_lower_chars, true, is_lower_inclusive, d_is_valid);
    cudaDeviceSynchronize();
    CHECK_ERROR();

    // Perform the upper string comparsion
    perform_string_comparsion<<<num_blocks, BLOCK_THREADS>>>(char_data, num_chars, str_indices, num_strings, d_upper_chars, num_upper_chars, false, is_upper_inclusive, d_is_valid);
    cudaDeviceSynchronize();
    CHECK_ERROR();
    ...
    row_id = d_valid_idxs;
    count = h_count;
    std::cout << "comparsionStringExpression got count of " << h_count[0] << std::endl;
}
