...
namespace duckdb
{
    ...
    void orderByString(uint8_t** col_keys, uint64_t** col_offsets, int* sort_orders, uint64_t* col_num_bytes, uint64_t num_rows, uint64_t num_cols) {
        ...
        uint8_t** d_col_keys = reinterpret_cast<uint8_t**>(gpuBufferManager->customCudaMalloc<uint64_t*>(num_cols, 0, 0));
        cudaMemcpy(d_col_keys, col_keys, num_cols * sizeof(uint8_t*), cudaMemcpyHostToDevice);
        uint64_t** d_col_offsets = reinterpret_cast<uint64_t**>(gpuBufferManager->customCudaMalloc<uint64_t*>(num_cols, 0, 0));
        ...
        OrderByCustomComparator custom_sort_comparator(d_col_keys, d_col_offsets, d_sort_orders, num_cols);
