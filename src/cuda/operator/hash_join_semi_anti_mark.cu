...
__global__ void probe_single_match(uint64_t **keys, unsigned long long* ht, uint64_t ht_len, uint64_t *row_ids_left, uint64_t *row_ids_right, unsigned long long* count, 
            uint64_t N, int* condition_mode, int num_keys, int join_mode, bool is_count) {
    ...
    for (int ITEM = 0; ITEM < I; ITEM++) {
        if (threadIdx.x + (ITEM * B) < num_tile_items) {
            ...
            else if (num_keys == 2) slot = hash64(keys[0][tile_offset + threadIdx.x + ITEM * B], keys[1][tile_offset + threadIdx.x + ITEM * B]) % ht_len;
            else cudaAssert(0);
...
void probeHashTableSingleMatch(uint64_t **keys, unsigned long long* ht, uint64_t ht_len, uint64_t* &row_ids_left, uint64_t* &row_ids_right, 
    ...
    probe_single_match<BLOCK_THREADS, ITEMS_PER_THREAD><<<(N + tile_items - 1)/tile_items, BLOCK_THREADS>>>(keys_dev, ht, ht_len, row_ids_left, row_ids_right, (unsigned long long*) count, 
            N, condition_mode_dev, num_keys, join_mode, 1);
    CHECK_ERROR();
    ...
    probe_single_match<BLOCK_THREADS, ITEMS_PER_THREAD><<<(N + tile_items - 1)/tile_items, BLOCK_THREADS>>>(keys_dev, ht, ht_len, row_ids_left, row_ids_right, (unsigned long long*) count, 
            N, condition_mode_dev, num_keys, join_mode, 0);
    CHECK_ERROR();
    cudaDeviceSynchronize();
    printf("Count: %lu\n", h_count[0]);
    count = h_count;
