...
/// BIND an @c s3:// object through Sirius's per-connection @c s3_ioctx.
///
...
/// FileSystem (positional reads via @c s3_ioctx::host_read_io); the resulting
/// native @c read_parquet scan is then captured by the transparent optimizer
/// hook and executed on GPU, where the actual column data is read via
/// @c s3_ioctx (NOT through this FileSystem).
///
...
/// @c s3_ioctx, so this FileSystem only ever serves the bind-time footer read; it
/// is never the CPU data path. Read-only: write opens are rejected.
class sirius_s3_filesystem : public duckdb::FileSystem {
 public:
  ...
  /// (no S3 LIST, so wildcards are out of scope).
