...
class task_completion_message_queue {
 public:
    ...
    void EnqueueMessage(sirius::unique_ptr<task_completion_message> message) {
      ...
      * 
     * @return sirius::unique_ptr<task_completion_message> The dequeued message, or nullptr if queue is empty
      */
     ...
     * @return sirius::unique_ptr<task_completion_message> The dequeued message
      */
    sirius::unique_ptr<task_completion_message> PullMessage() {
         return DequeueMessage();
     }
     
 private:
     mutex mutex_;                                                        ///< Mutex for thread-safe queue access
    sirius::queue<sirius::unique_ptr<task_completion_message>> message_queue_;  ///< Underlying message queue
