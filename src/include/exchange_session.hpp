...
class ExchangeSession {
 public:
  ...
  void store_packed(std::shared_ptr<void> data,
                    uintptr_t addr,
                    size_t size,
                    std::unique_ptr<std::vector<uint8_t>> md)
  {
  ...
  void retain(std::shared_ptr<void> data) { retained_data.push_back(std::move(data)); }
};
