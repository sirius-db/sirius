...
class task_creator {
 public:
  ...
  /// this is called the creator reports into its own private publisher, which has
  /// no subscribers and so is a no-op.
  ...
  /// pipeline cannot see.
  void publish_pipeline_closure(std::size_t pipeline_id, std::size_t source_operator_id) noexcept;
 ...
 protected:
  ...
  /// Stamped by prepare_for_query; the publisher loop has no other handle on it.
