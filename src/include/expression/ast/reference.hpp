 ...
 * return_type). The executor normally resolves the carrier from the input
 * table_view at runtime. When that carrier is a strictly narrower compatible
 * integer or same-scale DECIMAL, return_type() provides the native target for
 * an opportunistic restoration; every other mismatch retains the legacy
 * pass-through behavior. The declared type also supports the cuDF-AST
 * translator's decimal-propagation guard.
 */
struct reference {
