...
//! @brief Fill `out[0, rows)` with `out[i] = gen(i)`, vectorizing the 16B-aligned interior with int4
//! stores and covering the unaligned head + ragged tail with scalar stores. Uses the cache-streaming
//! store hint (`STORE_CS`) since the output is write-once decode data.
//!
...
_CCCL_DEVICE _CCCL_FORCEINLINE void vec_fill(T* out, uint32_t rows, uint32_t tid, uint32_t stride, Gen gen)
{
  if constexpr (sizeof(T) <= 8 && 16u % sizeof(T) == 0)
  {
    constexpr uint32_t TPV  = 16u / sizeof(T); // T values per int4
    const uint32_t misalign = static_cast<uint32_t>(reinterpret_cast<::cuda::std::uintptr_t>(out) & 15u);
