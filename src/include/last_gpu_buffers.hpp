...
class LastGPUBuffers {
 public:
  ...
  void EndSession(uint64_t session_id)
  {
    ...
    if (active_session_ && active_session_->session_id == session_id) { active_session_.reset(); }
    // Completed sessions are also tracked for cleanup.
    completed_sessions_.erase(
      std::remove_if(completed_sessions_.begin(),
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) { active_session_->accumulate_partitions(std::move(partitions)); }
  }
                       ...
                       std::unique_ptr<std::vector<uint8_t>> metadata)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) {
      active_session_->store_packed(std::move(gpu_data), gpu_addr, gpu_size, std::move(metadata));
    }
  }
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) { active_session_->retain(std::move(data)); }
  }
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) { return std::move(active_session_->packed_partitions); }
    return {};
  }
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) { active_session_->accumulate_broadcast(std::move(entry)); }
  }

  std::vector<PackedBroadcastEntry> TakePackedBroadcastEntries()
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (active_session_) { return std::move(active_session_->packed_broadcast_entries); }
    return {};
  }
