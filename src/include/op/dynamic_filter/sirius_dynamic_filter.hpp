...
class sirius_dynamic_filter_set {
 public:
   ...
   *
   * @p planned_target_columns contains every consumer output ordinal this producer may publish.
   * An empty vector registers an unscoped producer, requiring consumers to conservatively treat
   * every column as a potential target.
   */
  void register_producer(std::vector<std::size_t> planned_target_columns = {});

  /**
   * @brief Sorted union of all scoped producers' planned consumer output ordinals
   *
   * The result is meaningful only when @ref has_producers is true and
   * @ref has_unscoped_producer is false.
   */
  [[nodiscard]] std::vector<std::size_t> planned_target_columns() const;

  /**
   * @brief True iff any producer registered without declaring its target ordinals
   */
 ...
 private:
  ...
  /**
   * @brief Union of scoped producers' planned consumer output ordinals
   */
  ...
  /**
   * @brief True once an unscoped producer has registered
   */
