...
/// so `QueryEnd()`'s `clear_all_repositories()` cannot touch them. A sender's output therefore
...
class streaming_fragment {
 public:
  ...
  ///
  /// The caller owns the query lifecycle and must bracket `build()` and `run()` in one
  /// `QueryBeginStandalone` / `QueryEnd` pair. A lifecycle opened between them resets the task
