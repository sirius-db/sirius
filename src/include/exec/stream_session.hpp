...
///
/// It is deliberately a *router* and nothing more: it holds no repositories, builds no plan,
/// submits nothing to the scheduler, and owns no teardown. Each call resolves an id to an
/// operator and forwards. The operators are functional on their own — a streaming source's
