...
/// End-of-stream lifecycle for a single stream, kept separate from the queue.
///
/// A `cucascade::shared_data_repository` is the queue; this owns what the repository lacks —
/// who is still producing, whether "no batch right now" means "wait" or "the stream is over",
/// and how a starved consumer gets woken. It holds no repository and no batches: the endpoint
/// passes emptiness in as a snapshot (`repo_empty`), so the two locks are never held together
/// and cannot invert.
///
/// Admission and close share one lock, which is what makes the two central races safe: no batch
/// is admitted after the stream goes terminal, and a batch is registered in the repository
/// before its waker fires.
