...
///
/// The counters live on the per-query manager, so they need no epoch or
/// delta bookkeeping to be read.
...
class readahead_scan_manager : public std::enable_shared_from_this<readahead_scan_manager>,
                               public event::query_event_subscriber {
 public:
  /// Registers for @p publisher's events; the mailbox is only drained once
  /// @ref start runs.
  ///
  ...
  [[nodiscard]] std::vector<event::event_type> has_hooks_for_events() const override
  {
    return {event::event_type::task_deployed,
  ...
  void on_task_deployed(event::event_id_t,
                        event::timestamp_t,
                        query_id_t query_id,
  ...
  void on_task_queue_empty(event::event_id_t, event::timestamp_t) noexcept override;
  ...
  void on_memory_downgrade_for_task(event::event_id_t,
                                    event::timestamp_t,
                                    query_id_t query_id,
                                    std::size_t operator_id,
                                    int gpu_id,
  ...
  void on_wait_for_memory_for_task(event::event_id_t,
                                   event::timestamp_t,
                                   query_id_t query_id,
                                   std::size_t operator_id,
                                   int gpu_id,
