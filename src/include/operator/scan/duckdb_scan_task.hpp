...
struct multiple_blocks_allocation_accessor {
   ...
   */
  template <typename S>
  [[nodiscard]] S get_current_as(const std::unique_ptr<multiple_blocks_allocation>& allocation) const
  {
    assert(block_index < num_blocks);
    assert(allocation != nullptr);
   ...
   */
  void memcpy_to(const std::unique_ptr<multiple_blocks_allocation>& allocation, void* dest, size_t bytes)
  {
    size_t bytes_copied = 0;
