...
{
  registry
    .register_converter<origin_annotated_gpu_table_representation,
  ...
  // The metadata-only wave-capture carrier gets the same annotation-dropping
  // pairs, so an unconsumed capture can never strand a batch at a downgrade
  // or cross-device boundary (typeid-exact converter dispatch).
  registry.register_converter<selection_captured_gpu_table_representation,
                              ::cucascade::host_data_representation>(
    detail::make_annotation_dropping_converter<::cucascade::host_data_representation>(registry));
  registry.register_converter<selection_captured_gpu_table_representation,
                              ::cucascade::gpu_table_representation>(
