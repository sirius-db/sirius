...
uring_io_object::uring_io_object(std::string path) : _path(std::move(path)) {
  ...
  if (!_fd)
    throw std::runtime_error("uring_io_object: open failed: " + _path + ": " +
                             strerror(errno));

  struct stat st{};
  if (::fstat(_fd.get(), &st) < 0)
    throw std::runtime_error("uring_io_object: fstat failed: " +
...
void uring_reactor::cuda_copy_cb(void *p) noexcept {
  ...
  arg->self->_bounce[arg->slot].cuda_done.store(true,
...
uring_reactor::align_to_physical(cudf::io::text::byte_range_info logical,
                                 size_t file_size) {
  ...
  size_t a_end =
      std::min((offset + size + IO_BLOCK_SIZE - 1) & ~(IO_BLOCK_SIZE - 1),
...
size_t uring_reactor::host_read(int fd, size_t offset, size_t size,
                                uint8_t *dst) {
  if (size == 0)
    return 0;
  ssize_t n = ::pread(fd, dst, size, static_cast<off_t>(offset));
  if (n < 0)
    throw std::runtime_error("uring_reactor::host_read pread: " +
                             std::string(strerror(errno)));
...
void uring_reactor::worker_loop() {
  unique_ring ring = [this]() -> unique_ring {
    ...
    int rc = io_uring_queue_init(_ring_entries, r.get(),
    ...
    spdlog::debug("uring_reactor: SINGLE_ISSUER|DEFER_TASKRUN unsupported "
    ...
    if (rc2 < 0)
      throw std::runtime_error("uring_reactor: ring init: " +
                               std::string(strerror(-rc2)));
  ...
  auto submit_pending = [&]() {
    ...
    while (!pending.empty()) {
      ...
      spdlog::trace("reactor submit slot={} fd={} file_off={} io_size={:.2f}MB",
                    si, req.handle, req.file_off, to_mb(req.io_size));
      io_uring_prep_read(sqe, req.handle, _bounce[si].buf.get(),
                         (unsigned)req.io_size, (__u64)req.file_off);
    ...
    while (!pending_host.empty()) {
      ...
      spdlog::trace("reactor host submit fd={} offset={} size={:.2f}MB",
                    req->handle, req->offset, to_mb(req->size));
      io_uring_prep_read(sqe, req->handle, req->dst, (unsigned)req->size,
                         (__u64)req->offset);
      ...
      if (data & HOST_TAG) {
        ...
        if (res < 0) {
          req->ctx->chunk_failed(std::make_exception_ptr(std::runtime_error(
              "reactor host read: " + std::string(strerror(-res)))));
        } else {
          spdlog::trace("reactor host done fd={} offset={} rd={:.2f}MB",
                        req->handle, req->offset, to_mb((size_t)res));
          req->ctx->chunk_done();
        }
        delete req;
      } else {
        ...
        if (res < 0) {
          req.ctx->chunk_failed(std::make_exception_ptr(std::runtime_error(
              "reactor read: " + std::string(strerror(-res)))));
          sinfo = slot_info{};
        } else {
          size_t rd = (size_t)res;
          size_t actual = rd > req.data_off
                              ? std::min(req.data_size, rd - req.data_off)
                              : 0;
          spdlog::trace("reactor done  slot={} file_off={} rd={:.2f}MB "
                        "copy={:.2f}MB",
                        si, req.file_off, to_mb(rd), to_mb(actual));
          if (actual > 0 && !req.ctx->failed.load(std::memory_order_relaxed)) {
            _bounce[si].cuda_done.store(false, std::memory_order_relaxed);
            if (req.device_id >= 0)
              cudaSetDevice(req.device_id);
            cudaMemcpyAsync(req.dst,
                            (uint8_t *)_bounce[si].buf.get() + req.data_off,
                            actual, cudaMemcpyHostToDevice, req.stream);
