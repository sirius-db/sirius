  ...
  : _ring_entries(ring_entries)
{
  for (int i = 0; i < static_cast<int>(NUM_CHUNKS); ++i) {
    void* raw = nullptr;
    CUDA_CHECK(cudaHostAlloc(&raw, bounce_slot_size, cudaHostAllocDefault));
...
void uring_reactor::enqueue(device_read_req req)
{
  _queue.enqueue(std::move(req));
  _wake_seq.fetch_add(1, std::memory_order_release);
  _wake_seq.notify_one();
}

void uring_reactor::enqueue_host(host_read_req req)
{
...
void uring_reactor::worker_loop()
{
  ...
  std::deque<device_read_req> pending;
  std::deque<host_read_req> pending_host;
  int inflight = 0;
  ...
  auto submit_pending = [&]() {
    ...
    while (!pending.empty()) {
      ...
      spdlog::trace("reactor submit slot={} fd={} file_off={} io_size={:.2f}MB",
                    ...
                    to_mb(req.io_size));
      io_uring_prep_read(
        sqe, req.fd_direct, _bounce[si].buf.get(), (unsigned)req.io_size, (__u64)req.file_off);
    ...
    while (!pending_host.empty()) {
      io_uring_sqe* sqe = io_uring_get_sqe(ring.get());
      if (!sqe) break;
      auto* req = new host_read_req(std::move(pending_host.front()));
      pending_host.pop_front();
      spdlog::trace("reactor host submit fd={} offset={} size={:.2f}MB",
                    req->fd,
                    req->offset,
                    to_mb(req->size));
      io_uring_prep_read(sqe, req->fd, req->dst, (unsigned)req->size, (__u64)req->offset);
  ...
  auto reap_cqes = [&]() {
    ...
    for (auto* cqe : std::span{cqes, n}) {
      ...
      if (data & HOST_TAG) {
        ...
        auto* req = reinterpret_cast<host_read_req*>(data & ~HOST_TAG);
        if (res < 0) {
