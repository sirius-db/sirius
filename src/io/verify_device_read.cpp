...
static cudf::io::table_with_metadata read_device(size_t num_rows)
{
  ...
  auto ds     = io_ctx->make_datasource(std::make_unique<sirius::io::uring_io_object>(PATH));
...
static std::vector<uint8_t> to_host(void const* dev_ptr, size_t bytes)
{
  ...
  if (bytes > 0) cudaMemcpy(h.data(), dev_ptr, bytes, cudaMemcpyDeviceToHost);
  return h;
}

// Compare one pair of columns. Returns true if identical.
static bool compare_column(cudf::column_view const& a,
                           cudf::column_view const& b,
                           std::string const& name)
{
  if (a.size() != b.size()) {
    std::cerr << "  FAIL " << name << ": row count differs (" << a.size() << " vs " << b.size()
              << ")\n";
    return false;
  }
  if (a.null_count() != b.null_count()) {
    std::cerr << "  FAIL " << name << ": null_count differs (" << a.null_count() << " vs "
              << b.null_count() << ")\n";
    return false;
  }
  ...
  if (ha != hb) {
    ...
    for (size_t i = 0; i < data_bytes; ++i) {
      if (ha[i] != hb[i]) {
        std::cerr << "  FAIL " << name << ": first diff at byte " << i << " (element "
                  << i / type_sz << "): " << (int)ha[i] << " vs " << (int)hb[i] << "\n";
        return false;
      }
    }
  }
...
int main(int argc, char** argv)
{
  ...
  for (int c = 0; c < ref.tbl->num_columns(); ++c) {
    std::string col_name = (c < (int)COLUMNS.size()) ? COLUMNS[c] : "col" + std::to_string(c);
    bool ok = compare_column(ref.tbl->view().column(c), dev.tbl->view().column(c), col_name);
