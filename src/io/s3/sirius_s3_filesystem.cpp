  ...
  duckdb::optional_ptr<duckdb::FileOpener> opener)
{
  ...
  // Transparent S3 is GPU-only. The GPU scan reads column data through s3_ioctx,
  // so this FileSystem only ever serves the bind-time footer read for the GPU
  // path. If gpu_execution is off there is no GPU consumer, so opening here would
  // be a CPU read of s3:// — which Sirius does not support. Refuse with a clear
  // message instead of silently serving a CPU fallback.
  {
    ...
    if (!gpu_on) {
      throw duckdb::IOException(
        "[sirius_s3_filesystem] reading '" + path +
        "' over S3 requires GPU execution: S3 is GPU-only and has no CPU fallback; "
  ...
  // footer read for the transparent GPU path (the GPU scan then reads via
  // s3_ioctx, not here). An internal query active here means we are on a CPU
  // fallback replay (run_internal_cpu_fallback_query wraps the replay in an
  // InternalQueryGuard) -- e.g. gpu_execution('SELECT ... FROM v_s3') whose view
  // body reads s3:// and whose GPU plan failed. Refuse the read so s3:// data is
  // never served to a CPU plan, even when reached indirectly through a view.
  if (sirius_ctx->is_internal_query_active()) {
    throw duckdb::IOException(
      "[sirius_s3_filesystem] reading '" + path +
      "' on a CPU execution path: S3 CPU fallback is not supported (S3 is GPU-only); "
      "Sirius has no CPU fallback for S3 data sources");
  }
  // Resolve through the scan_manager's datasource factory (the post-cleanup
  // seam): the returned sirius_datasource performs the HEAD and carries the
  // backend; HEAD failures (missing key / auth / network) propagate as
  // exceptions for DuckDB to surface at bind time.
  ...
  return duckdb::make_uniq<sirius_s3_file_handle>(*this, path, flags, std::move(datasource));
}
...
{
  return static_cast<int64_t>(as_s3_handle(handle).datasource_->size());
}
