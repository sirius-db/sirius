...
int main(int argc, char** argv)
{
  ...
  if (paths.size() < 2 * files_per_gpu)
    throw std::runtime_error("need >= " + std::to_string(2 * files_per_gpu) + " files in " +
                             dir.string());
  ...
  auto worker = [&](int gpu) {
    ...
    try {
      ...
      rmm::mr::set_per_device_resource(rmm::cuda_device_id{gpu}, mrs[gpu].get());
      ...
      cuda_check(cudaStreamCreateWithFlags(&raw_stream, cudaStreamNonBlocking), "cudaStreamCreate");
      ...
      for (auto const& path : per_gpu[gpu]) {
        auto io_obj = std::make_unique<sirius::io::uring_io_object>(path.string());
        bytes += io_obj->size();
    ...
    if (raw_stream) cudaStreamDestroy(raw_stream);
  };

  std::array<std::thread, 2> threads{std::thread(worker, 0), std::thread(worker, 1)};
  for (auto& t : threads)
    t.join();
  ...
  for (int g = 0; g < 2; ++g) {
    ...
    std::cout << "GPU " << g << " : " << per_gpu[g].size() << " files, " << rows_read[g].load()
              << " rows, " << gib << " GiB, " << elapsed << " s (" << (gib / elapsed)
              << " GiB/s)\n";
  }
  return rc;
}
