...
void sirius_ioctx::initialize_cache(buffer_pool &pool,
  ...
  _cache =
      std::make_unique<prefetching_cache>(pool, this, inflight_budget_chunks);
}
...
size_t copy_pinned_slices_to_device(
  ...
  if (n_nonempty == 1) {
    ...
    for (auto const &s : slices) {
      if (s.size() == 0)
        continue;
      auto err = cudaMemcpyAsync(dst + copied, s.data(), s.size(),
                                 cudaMemcpyHostToDevice, stream.value());
      if (err != cudaSuccess)
        throw std::runtime_error(
            std::string("sirius_ioctx: cudaMemcpyAsync failed: ") +
  ...
  auto err = cudaMemcpyBatchAsync(dsts.data(), srcs.data(), sizes.data(),
                                  n_nonempty, &attrs, &attrs_idx, 1,
                                  stream.value());
  if (err != cudaSuccess)
    throw std::runtime_error(
        std::string("sirius_ioctx: cudaMemcpyBatchAsync failed at idx ") +
...
size_t sirius_ioctx::device_read(sirius_io_object &obj, size_t offset,
...
sirius_ioctx::device_read(sirius_io_object &obj, size_t offset, size_t size,
                          rmm::cuda_stream_view stream) {
  if (_cache) {
    if (auto view = _cache->read(obj, offset, size); view) {
      auto slices = view.slice(offset, size);
      rmm::device_buffer dbuf(size, stream);
      copy_pinned_slices_to_device(
          slices, static_cast<uint8_t *>(dbuf.data()), stream);
...
void sirius_ioctx::device_read_async(sirius_io_object &obj, size_t offset,
                                     size_t size, uint8_t *dst,
                                     rmm::cuda_stream_view stream,
...
std::unique_ptr<cudf::io::datasource::buffer>
sirius_datasource::host_read(size_t offset, size_t size) {
  return _io_ctx->host_read(*_io_object, offset, size);
}

std::future<size_t>
sirius_datasource::host_read_async(size_t offset, size_t size, uint8_t *dst) {
  auto p = std::make_shared<std::promise<size_t>>();
  auto f = p->get_future();
  _io_ctx->host_read_async(
      *_io_object, offset, size, dst,
...
sirius_datasource::host_read_async(size_t offset, size_t size) {
  size = std::min(size, _io_object->size() > offset
                            ? _io_object->size() - offset
                            : size_t{0});
  auto buf = std::make_shared<std::vector<uint8_t>>(size);
  auto p =
      std::make_shared<std::promise<std::unique_ptr<datasource::buffer>>>();
  auto f = p->get_future();
  _io_ctx->host_read_async(
      *_io_object, offset, size, buf->data(),
      [p, buf](size_t n, std::exception_ptr ep) {
        if (ep) {
          p->set_exception(ep);
          return;
        }
...
std::unique_ptr<cudf::io::datasource::buffer>
sirius_datasource::device_read(size_t offset, size_t size,
                               rmm::cuda_stream_view stream) {
  return _io_ctx->device_read(*_io_object, offset, size, stream);
}

size_t sirius_datasource::device_read(size_t offset, size_t size, uint8_t *dst,
                                      rmm::cuda_stream_view stream) {
  return _io_ctx->device_read(*_io_object, offset, size, dst, stream);
}

std::future<size_t>
sirius_datasource::device_read_async(size_t offset, size_t size, uint8_t *dst,
                                     rmm::cuda_stream_view stream) {
  auto p = std::make_shared<std::promise<size_t>>();
  auto f = p->get_future();
  _io_ctx->device_read_async(
      *_io_object, offset, size, dst, stream,
      [p](size_t n, std::exception_ptr ep) {
        if (ep)
          p->set_exception(ep);
        else
