...
using read_fn_t =
    std::function<std::future<size_t>(size_t file_idx, size_t offset,
                                      size_t size, uint8_t *dst,
                                      rmm::cuda_stream_view stream)>;

using insert_fn_t =
    std::function<void(size_t file_idx, std::vector<Range> const &)>;
...
static std::vector<fs::path> enumerate_paths(fs::path const &dir) {
  ...
  if (paths.size() < N_FILES)
    throw std::runtime_error("need >= " + std::to_string(N_FILES) +
                             " files in " + dir.string());
...
generate_ranges_per_file(std::vector<size_t> const &file_sizes,
                         uint64_t seed) {
  std::mt19937_64 rng(seed);
  std::vector<std::vector<Range>> out(file_sizes.size());
  std::uniform_int_distribution<size_t> size_dist(RANGE_MIN_BYTES,
                                                  RANGE_MAX_BYTES);
  for (size_t fi = 0; fi < file_sizes.size(); ++fi) {
    ...
    for (size_t i = 0; i < RANGES_PER_FILE; ++i) {
      ...
      out[fi].emplace_back(static_cast<int64_t>(off),
...
static run_result
run_generic(std::string const &label,
            std::vector<std::vector<Range>> const &per_file_ranges,
            size_t total_tasks, size_t n_workers, read_fn_t read_fn,
            insert_fn_t insert_fn,
            std::function<void()> post_hook = nullptr) {
  moodycamel::ConcurrentQueue<read_task> queue;
...

      {
        ...
        for (auto const &r : t.ranges) {
          ...
          futs.push_back(read_fn(t.file_idx,
                                 static_cast<size_t>(r.offset()), sz,
  ...
  all_tasks.reserve(total_tasks);
  {
    ...
    std::uniform_int_distribution<size_t> file_dist(
        0, per_file_ranges.size() - 1);
  ...
  std::thread watchdog([&] {
    ...
    while (!done.load(std::memory_order_relaxed)) {
      ...
      std::cerr << "[watchdog] tasks_done=" << cur_done
                << " remaining=" << rem;
  ...
  std::cout << "  bytes        : " << (total_bytes / double(1UL << 30))
            << " GiB\n";
  std::cout << "  throughput   : " << (total_bytes / double(1UL << 30) / elapsed)
            << " GiB/s\n";
...
int main(int argc, char **argv) {
  ...
  if (argc > 1)
    total_tasks = std::stoul(argv[1]);
  if (argc > 2)
    n_workers = std::stoul(argv[2]);
  if (argc > 3)
    pool_max_slabs = static_cast<uint32_t>(std::stoul(argv[3]));
  ...
  std::cout << "stress_prefetching\n"
            ...
            << "  range size   : [" << (RANGE_MIN_BYTES >> 20) << ", "
            << (RANGE_MAX_BYTES >> 20) << "] MiB\n"
            << "  total tasks  : " << total_tasks << "\n"
            << "  workers      : " << n_workers << "\n"
            << "  pool cap     : " << pool_max_slabs << " slabs ("
            << (pool_max_slabs * 500) << " MiB max)\n";
  ...
  // ---- Run 1: CACHE OFF (uring_ioctx, no cache) ----------------------------
  {
    ...
    auto read_fn = [&](size_t fi, size_t off, size_t sz, uint8_t *dst,
                       rmm::cuda_stream_view stream) {
      auto p = std::make_shared<std::promise<size_t>>();
      auto f = p->get_future();
      ioctx->device_read_async(
          *u_objs[fi], off, sz, dst, stream,
          [p](size_t n, std::exception_ptr ep) {
            if (ep)
              p->set_exception(ep);
            else
              p->set_value(n);
    ...
    run_generic("CACHE OFF  (uring, no cache)", per_file_ranges, total_tasks,
                n_workers, read_fn, /*insert_fn=*/nullptr);
    ioctx->shutdown();
  }
  ...
  // ---- Run 2: CACHE ON (uring_ioctx + prefetching_cache) -------------------
  {
    ...
    run_generic("CACHE ON   (uring + prefetching_cache)", per_file_ranges,
                total_tasks, n_workers, read_fn, insert_fn, [&] {
                  std::cout << "  cache        : "
                            << ioctx->cache()->summary() << "\n";
                });
    ioctx->shutdown();
  ...
  // This matches the "no-cache, no GDS" path a typical caller would write.
  {
    ...
    auto read_fn = [&](size_t fi, size_t off, size_t sz, uint8_t *dst,
      ...
      return std::async(
          std::launch::async,
          [&, fi, off, sz, dst, stream]() -> size_t {
            std::vector<uint8_t> host_buf(sz);
            size_t n = ds[fi]->host_read(off, sz, host_buf.data());
            auto err = cudaMemcpyAsync(dst, host_buf.data(), n,
                                       cudaMemcpyHostToDevice, stream.value());
            if (err != cudaSuccess)
              throw std::runtime_error(
                  std::string("cudaMemcpyAsync: ") + cudaGetErrorString(err));
            // Wait on the copy so host_buf is safe to free on return.
            cudaStreamSynchronize(stream.value());
            return n;
          });
    };
    run_generic("CUDF DEFAULT (host_read + cudaMemcpy)", per_file_ranges,
                total_tasks, n_workers, read_fn, /*insert_fn=*/nullptr);
  }
