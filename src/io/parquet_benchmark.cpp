...
int main(int argc, char** argv)
{
  ...
  cudf::io::parquet::FileMetaData file_metadata = [&] {
    ...
    auto base_opts = cudf::io::parquet_reader_options::builder().columns(COLUMNS).build();
    cudf::io::parquet::experimental::hybrid_scan_reader scanner{
      cudf::host_span<uint8_t const>{footer_buf.data(), footer_buf.size()}, base_opts};
  ...
  if (source == DataSource::cudf) {
    ...
    ms           = time_ms([&] {
      auto tbl = cudf::io::read_parquet(std::move(sources), {file_metadata}, read_opts);
      std::cout << "Schema : " << tbl.tbl->num_columns() << " columns, " << tbl.tbl->num_rows()
                << " rows read\n\n";
    });
  } else {
