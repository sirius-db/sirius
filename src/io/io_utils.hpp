 ...
 */
inline std::vector<range> align_ranges(std::span<range const> ranges,
                                       size_t alignment) {
 ...
 */
inline std::vector<range> coalesce_ranges(std::span<range const> ranges,
                                          size_t max_gap) {
  if (ranges.empty())
    return {};
  ...
  for (size_t i = 1; i < sorted.size(); ++i) {
    ...
    if (next_start <= cur_end + max_gap) {
      ...
      out.emplace_back(static_cast<int64_t>(cur_start),
                       static_cast<int64_t>(cur_end - cur_start));
...
inline std::vector<range>
coalesce_sorted_ranges(std::span<range const> sorted_ranges, size_t max_gap) {
 ...
 */
inline std::vector<range>
align_and_coalesce_sorted(std::span<range const> ranges, size_t alignment,
                          size_t max_gap = 0) {
 ...
 */
inline std::vector<range> common_sorted_ranges(std::span<range const> a,
                                               std::span<range const> b) {
  std::vector<range> out;
  size_t i = 0, j = 0;
 ...
 */
inline std::vector<range>
missing_sorted_ranges(std::span<range const> source,
  ...
  for (auto const &s : source) {
    ...
    while (j < coverage.size() &&
           static_cast<size_t>(coverage[j].offset()) +
                   static_cast<size_t>(coverage[j].size()) <=
               cur)
      ++j;
    ...
    while (k < coverage.size() &&
           static_cast<size_t>(coverage[k].offset()) < end) {
      size_t c_start = static_cast<size_t>(coverage[k].offset());
      size_t c_end = c_start + static_cast<size_t>(coverage[k].size());

      if (cur < c_start)
        out.emplace_back(static_cast<int64_t>(cur),
                         static_cast<int64_t>(c_start - cur));
