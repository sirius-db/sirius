...
prefetching_cache::prefetching_cache(buffer_pool& pool,
  ...
  : _pool(pool),
    _io_ctx(io_ctx),
    _inflight_budget(inflight_budget_chunks),
    _evictor_thread([this](std::stop_token st) { evictor_loop(std::move(st)); }),
    _worker_thread([this](std::stop_token st) { worker_loop(std::move(st)); })
{
}
...
void prefetching_cache::insert(const sirius_io_object& obj,
                               ...
                               const std::vector<cudf::io::text::byte_range_info>& ranges)
{
  ...
  auto try_claim = [&](std::shared_ptr<cache_entry> const& e) {
    if (e->state.try_start_queueing()) new_entries.push_back(e);
  };
  ...
  for (auto const& e : new_entries) {
    _pending_chunks.fetch_add(
      (static_cast<size_t>(e->physical_range.size()) + buffer_pool::CHUNK_BYTES - 1) /
  ...
  if (!new_entries.empty()) enqueue_work(prefetch_req{key, &obj, std::move(new_entries)});
}
...
pinned_view prefetching_cache::read(const sirius_io_object& obj, size_t offset, size_t size)
{
  auto const& key = obj.raw_file_cache_id();
...
void prefetching_cache::refresh_cache() { _cache_age.fetch_add(1, std::memory_order_relaxed); }
...
std::string prefetching_cache::summary() const
{
  ...
  return fmt::format(
    ...
    _pool.free_count(),
    _pool.total_chunks(),
    static_cast<double>(_allocated_bytes.load(std::memory_order_relaxed)) / (1024.0 * 1024.0),
...
void prefetching_cache::evictor_loop(std::stop_token stop)
{
  ...
  auto compute_bucket_idx = [&](cache_entry const& e) -> int {
    uint64_t age    = static_cast<uint64_t>(_cache_age.load(std::memory_order_relaxed));
    int64_t raw     = compute_raw_bucket(e);
    int64_t floor_v = e.consumption_ts.load(std::memory_order_relaxed) == age ? 1 : -10;
  ...
  auto evict_to_pool = [this](cache_entry* entry) -> size_t {
    ...
    for (auto* p : entry->chunks)
      _pool.deallocate(p);
    entry->chunks.clear();
    _allocated_bytes.fetch_sub(n * buffer_pool::CHUNK_BYTES, std::memory_order_relaxed);
  ...
  auto drain_candidates = [&]() -> size_t {
    ...
    while (_candidate_queue.try_dequeue(cand)) {
      ...
      if (!entry) continue;
      auto st = entry->state.get_state();
      if (st != entry_state::cached && st != entry_state::in_use) continue;
      if (entry->bucket_idx.load(std::memory_order_relaxed) != -1) continue;
      ...
      // is obvious.
      if (raw < -5 && entry->is_consumed_or_stale(age) && st == entry_state::cached) {
  ...
  while (!stop.stop_requested()) {
    ...
    if (!have_signal) continue;
    if (stop.stop_requested()) break;

    eviction_request req;
    if (!_request_queue.try_dequeue(req)) continue;  // spurious wake (e.g., destructor release)
    ...
    for (size_t b = 0; b < NUM_LRU_BUCKETS && reclaimed < needed; ++b) {
      ...
      while (!bucket.empty() && reclaimed < needed) {
        ...
        if (entry->state.get_state() != entry_state::cached or !entry->is_consumed_or_stale(age)) {
          continue;
        }
    ...
    if (reclaimed >= needed) {
      ...
      req.promise.set_exception(
        std::make_exception_ptr(std::runtime_error("eviction could not free enough chunks")));
    }
  }
}
...
void prefetching_cache::worker_loop(std::stop_token stop)
{
  ...
  while (!stop.stop_requested()) {
    ...
    for (auto const& entry : item.entries) {
      if (pool_exhausted) break;

      if (!entry->state.try_start_loading()) continue;

      auto phys_size = static_cast<size_t>(entry->physical_range.size());
      auto n_chunks  = (phys_size + buffer_pool::CHUNK_BYTES - 1) / buffer_pool::CHUNK_BYTES;
      ...
      if (got < n_chunks) {
        ...
        try {
          ...
          spdlog::warn("prefetching_cache: eviction could not free enough for {}", item.file_key);
          pool_exhausted = true;
          continue;
        }
      ...
      _allocated_bytes.fetch_add(ptrs.size() * buffer_pool::CHUNK_BYTES, std::memory_order_relaxed);
...

    {
      _io_ctx->host_read_ranges_async(
        const_cast<sirius_io_object&>(*item.io_obj),
        io_ranges,
        io_dsts,
        [this,
