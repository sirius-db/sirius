  ...
  op::scan::sirius_gpu_scan_operator& scan, march_side_data& side)
{
  ...
  auto consume =
    [&](tracked_column& col, op::sirius_physical_operator* consumer, bool count_only = false) {
  ...
  for (int hops = 0; hops < 256 && parent != nullptr && !live.empty(); ++hops) {
    switch (parent->type) {
      ...
      case optype::HASH_JOIN: {
        ...
        bool const from_left = !parent->children.empty() && parent->children[0].get() == cur;
        bool const passthrough_type =
          join != nullptr && (join->join_type == duckdb::JoinType::INNER ||
        ...
        bool const side_nullified = join->join_type == duckdb::JoinType::OUTER ||
        ...
        for (auto const& cond : join->conditions) {
          ...
          if (!side) { continue; }
          for (auto const r : expression_references(*side)) {
            key_refs.insert(r);
          }
        }
        ...
        auto const& own =
          from_left ? join->lhs_output_columns.col_idxs : join->rhs_output_columns.col_idxs;
      ...
      case optype::MERGE_GROUP_BY: {
        ...
        if (auto const* agg = dynamic_cast<op::sirius_physical_grouped_aggregate const*>(parent)) {
          ...
          for (std::size_t a = 0; a < agg->cudf_aggregate_idx.size(); ++a) {
            ...
            bool const is_count = a < agg->cudf_aggregates.size() &&
        ...
        for (auto& col : live) {
          auto const key_it =
            std::find(group_idx->begin(), group_idx->end(), static_cast<int>(col.cur_pos));
          if (key_it != group_idx->end()) {
            ...
            col.cur_pos = static_cast<std::size_t>(std::distance(group_idx->begin(), key_it));
            next.push_back(std::move(col));
            continue;
          }
      ...
      case optype::TOP_N: {
        ...
        if (modeled) {
          for (auto const& ord : topn->orders) {
            if (!ord.expression ||
                ord.expression->GetExpressionClass() != duckdb::ExpressionClass::BOUND_REF) {
              modeled = false;
              break;
            }
...
void run_late_mat_plan_pass(op::sirius_physical_operator& root)
{
  ...
  if (late_mat::late_mat_v3_enabled() && !scans.empty()) {
    ...
    std::map<std::pair<op::sirius_physical_operator*, std::size_t>,
