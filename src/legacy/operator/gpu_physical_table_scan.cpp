    ...
    gen_row_id_column(column_ids.back().GetPrimaryIndex() == DConstants::INVALID_INDEX)
{
  ...
  if (scanned_types.empty()) { scanned_types.push_back(LogicalType(LogicalTypeId::UBIGINT)); }
...
SourceResultType GPUPhysicalTableScan::GetDataDuckDBOpt(ExecutionContext& exec_context)
{
  ...
  SIRIUS_LOG_INFO(
    "[GetDataDuckDBOpt] invoked, column_ids.size()={}, gen_row_id={}, names.size()={}",
    column_ids.size(),
    gen_row_id_column,
    names.size());
  ...
  SIRIUS_LOG_INFO(
    "[GetDataDuckDBOpt] checking cache: num_columns={}, table={}", num_columns, table_name);
                                                       ...
                                                       const vector<LogicalType>& returned_types)
{
  ...
  for (auto& [column_index, filter] : filters.filters) {
    ...
    if (column_ids[column_index].IsVirtualColumn()) { continue; }
