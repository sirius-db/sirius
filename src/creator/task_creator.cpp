...
void task_creator::publish_pipeline_closure(std::size_t pipeline_id,
                                            std::size_t source_operator_id) noexcept
{
  _query_event_publisher->publish_pipeline_closed(_query_id, pipeline_id, source_operator_id);
}
...
void task_creator::prepare_for_query(const sirius::planner::query& query)
{
  ...
  // Stashed for the stage hooks: task creation happens deep in the publisher loop
