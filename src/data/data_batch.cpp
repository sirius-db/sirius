...
data_batch::data_batch(uint64_t batch_id, data_repository_manager& data_repo_mgr, sirius::unique_ptr<idata_representation> data, sirius::memory::memory_space& memory_space)
    : _batch_id(batch_id), _data(std::move(data)), _data_repo_mgr(&data_repo_mgr), _memory_space(&memory_space) {}
...
data_batch::data_batch(uint64_t batch_id, data_repository_manager& data_repo_mgr, sirius::unique_ptr<idata_representation> data)
    : _batch_id(batch_id), _data(std::move(data)), _data_repo_mgr(&data_repo_mgr), _memory_space(nullptr) {}
...
void data_batch::decrement_pin_ref_count() {
    ...
    const auto tier = (_memory_space != nullptr) ? _memory_space->get_tier() : _data->get_current_tier();
...
void data_batch::set_data(sirius::unique_ptr<idata_representation> data) {
    std::lock_guard<sirius::mutex> lock(_mutex);
    size_t views = _view_count.load(std::memory_order_relaxed);
    size_t pins = _pin_count.load(std::memory_order_relaxed);
    if (views != 0 || pins != 0) {
...
void data_batch::convert_to_memory_space(const sirius::memory::memory_space* target_memory_space,
    ...
    if (_pin_count.load(std::memory_order_relaxed) != 0) {
