  ...
  rmm::cuda_stream_view stream)
{
  ...
  {
    if (this->get_device_id() == target_memory_space->get_device_id())
    {
      // TODO: implement this for multip gpu support
      //  rmm::device_buffer device_buffer(packed_data.gpu_data->size(),);
    }
  }
  else if (target_memory_space->get_tier() == memory::Tier::HOST)
  {
    ...
    stream.synchronize();
    auto host_table_allocation =
      sirius::make_unique<sirius::memory::host_table_allocation>(std::move(allocation),
  ...
  {
    throw std::runtime_error("Invalid target memory space for "
