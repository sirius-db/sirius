...
void data_repository_manager::add_new_repository(size_t pipeline_id,
...
std::vector<std::unique_ptr<data_batch>> data_repository_manager::get_data_batches_for_downgrade(
  sirius::memory::memory_space_id memory_space_id,
  size_t amount_to_downgrade)
{
  ...
  for (auto& [batch_id, batch] : _data_batches)
  {
    ...
    if (downgrade_amount >= amount_to_downgrade)
    {
      break;
    }
  }
