...
// ---------------------------------------------------------------------------
// initialize_internal — decomposed into focused helper methods
// ---------------------------------------------------------------------------

duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>
sirius_engine::schedule_and_copy_pipelines(pipeline::sirius_meta_pipeline& root_pipeline)
{
...
void sirius_engine::split_table_scan_source(
...
void sirius_engine::split_intermediate_joins(
  duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
...
void sirius_engine::split_join_sink(duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
                                    const sirius::operator_params& op_params)
{
...
void sirius_engine::split_group_aggregate_sink(
  duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
  duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>& copied_scheduled,
  size_t pipeline_idx,
  const sirius::operator_params& op_params)
...
void sirius_engine::split_order_by_sink(
  duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
  duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>& copied_scheduled,
  size_t pipeline_idx,
  const sirius::operator_params& op_params)
...
void sirius_engine::split_top_n_sink(
  duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
  duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>& copied_scheduled,
  size_t pipeline_idx)
{
...
void sirius_engine::split_delim_join_sink(
  duckdb::shared_ptr<pipeline::sirius_pipeline>& current_pipeline,
  duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>& copied_scheduled,
  size_t pipeline_idx,
  const sirius::operator_params& op_params)
{
  ...
  auto partition_distinct_pipeline    = duckdb::make_shared_ptr<pipeline::sirius_pipeline>(*this);
...
void sirius_engine::split_pipelines(
  duckdb::vector<duckdb::shared_ptr<pipeline::sirius_pipeline>>& copied_scheduled,
  const sirius::operator_params& op_params)
{
  for (size_t i = 0; i < copied_scheduled.size(); i++) {
    ...
    split_intermediate_joins(current_pipeline, op_params);
    ...
    if (sink_type == op::SiriusPhysicalOperatorType::HASH_JOIN ||
      ...
      split_join_sink(current_pipeline, op_params);
    } else if (sink_type == op::SiriusPhysicalOperatorType::HASH_GROUP_BY ||
               sink_type == op::SiriusPhysicalOperatorType::UNGROUPED_AGGREGATE) {
      split_group_aggregate_sink(current_pipeline, copied_scheduled, i, op_params);
    } else if (sink_type == op::SiriusPhysicalOperatorType::ORDER_BY) {
      split_order_by_sink(current_pipeline, copied_scheduled, i, op_params);
      ...
      split_delim_join_sink(current_pipeline, copied_scheduled, i, op_params);
    } else {
      new_scheduled.push_back(current_pipeline);
    }
  }
...
void sirius_engine::wire_data_repositories()
{
...
void sirius_engine::setup_pipeline_parents()
{
  for (size_t i = 0; i < new_scheduled.size(); i++) {
    ...
    if (new_scheduled[i]->sink.get()) {
      if (new_scheduled[i]->sink.get()->type == op::SiriusPhysicalOperatorType::RIGHT_DELIM_JOIN) {
        auto& delim_join =
...
void sirius_engine::finalize_pipeline_structure()
{
...
void sirius_engine::link_join_partition_siblings()
{
  for (size_t i = 0; i < new_scheduled.size(); i++) {
...
void sirius_engine::log_pipeline_debug_info() const
{
...
void sirius_engine::initialize_internal(op::sirius_physical_operator& plan)
{
  ...
  // Schedule meta-pipelines in dependency order and deep-copy
  auto copied_scheduled = schedule_and_copy_pipelines(*root_pipeline);

  // Split pipelines: insert PARTITION/CONCAT/MERGE/SORT operators
  split_pipelines(copied_scheduled, op_params);

  // Wire data repositories between pipelines
  wire_data_repositories();

  // Set parent-child pipeline relationships from port wiring
  setup_pipeline_parents();

  // Finalize: merge sink into operators vector, set source alias, wire dependencies
  // AFTER THIS POINT: operators[] contains ALL operators (source through sink).
  // source = &operators[0], sink = operators.back().
  finalize_pipeline_structure();

  // Link build/probe partition siblings for coordinated hashing
  link_join_partition_siblings();

  // Debug logging (reads post-finalization state)
