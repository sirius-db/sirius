...
void gpu_pipeline_task::execute(rmm::cuda_stream_view stream)
{
  ...
  // At this point, all input batches are locked for reading via read_only_pipelineable_operator_data.
  // The locks are held inside local_state._input_data and released once the first operator's
  // execute() call moves and then destroys the read_only_pipelineable_operator_data.
