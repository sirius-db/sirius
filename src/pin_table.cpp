 ...
 public:
  ...
  void observe(cudf::table_view const& batch, rmm::cuda_stream_view stream)
  {
    for (auto& t : _tracks) {
      ...
      bool const sorted = cudf::is_sorted(
        cudf::table_view({col}), {cudf::order::ASCENDING}, {}, stream);
      ...
      auto const distinct = cudf::unique_count(
        col, cudf::null_policy::EXCLUDE, cudf::nan_policy::NAN_IS_VALID);
...
std::vector<std::uint32_t> materialize_pin_batches(op::scan::gpu_ingestible& ingestible,
