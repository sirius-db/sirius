...
#include "instrumentation-bridge/gen/custom_attributes.rs.h"
#include "instrumentation-bridge/gen/operator.rs.h"
#include "instrumentation-bridge/gen/plan.rs.h"
#include "instrumentation-bridge/gen/port.rs.h"
...
telemetry_context::telemetry_context()
  : engine_uuid_(::uuid::now_v7()),
    ...
    context_(::quent::create_context("ndjson", "telemetry_data")),
    ...
    if (sink) {
      for (auto& npi : sink->get_next_port_after_sink()) {
        ...
        port_obs->declaration(npi.pseudo_sink_port_uuid,
                              ::quent::port::Declaration{
                                .operator_id   = pipeline_uuid,
                                .instance_name = ::rust::String(std::string(
                                  npi.next_operator_port_name) + "_sink"),
                              });
