                                         ...
                                         rmm::device_async_resource_ref mr)
{
  ...
  if (req.mask->chunk_offsets == nullptr || req.mask->survivor_count != n_rows) {
    fail("compacted batch rows " + std::to_string(n_rows) +
         " do not match the captured mask (survivors=" + std::to_string(req.mask->survivor_count) +
         ", counted=" + std::string(req.mask->chunk_offsets != nullptr ? "yes" : "no") + ")");
  }
  ...
  rmm::device_buffer local_ids(sizeof(std::int32_t) * static_cast<std::size_t>(n_rows), stream, mr);
  sirius::codegen::mask_to_row_indices(
    *req.mask, static_cast<std::int32_t*>(local_ids.data()), stream);
...
std::unique_ptr<cudf::column> emit_rowid_column(rowid_emission_request const& req,
                                                ...
                                                rmm::device_async_resource_ref mr)
{
  if (n_rows < 0 || req.range.rows < 0 || req.range.start < 0) { fail("negative geometry"); }
