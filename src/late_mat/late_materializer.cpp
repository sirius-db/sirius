...
[[noreturn]] void fail(std::string const& what) { throw std::runtime_error("late_mat: " + what); }
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  // route's exact inputs — go straight through decompress_column's masked
  // selection (zero conversion; the mask cost was paid at scan time). A
  // full-route plan / refusal falls through to full decode + gather via the
  // prepare-built id expansion.
  if (bsel.mask_words) {
    selection_mask mask;
    ...
    simpatico::decode_selection sel;
    sel.mask           = &mask;
    sel.survivor_count = rows.num_survivors;
    sel.route          = simpatico::probe_column(tree).compact_route;
    std::unique_ptr<cudf::column> compacted;
    if (sel.compacted()) {
      compacted = simpatico::decompress_column(tree, stream, mr, &err, /*pred=*/nullptr, &sel);
    }
    ...
    if (full->null_count() != 0) { fail("nullable columns are not supported (v1)"); }
    return retag(
      gather_one(full->view(), int32_map_view(bsel.local_indices, rows.num_survivors), stream, mr));
  }
  ...
  auto const route       = simpatico::probe_column(tree).compact_route;
  bool const chars_route = route == sirius::codegen::decode_route::dict_codes ||
                           route == sirius::codegen::decode_route::str_split;
  if ((bsel.density < dense_sel_threshold() || chars_route) &&
      route != sirius::codegen::decode_route::full) {
    std::int64_t const n  = rows.num_rows;
    std::int64_t const nc = selection_mask::ChunksFor(n);
    rmm::device_buffer mask_words(
      static_cast<std::size_t>(selection_mask::WordsFor(n)) * sizeof(std::uint32_t), stream, mr);
    rmm::device_buffer chunk_offsets(
      static_cast<std::size_t>(nc + 1) * sizeof(std::uint32_t), stream, mr);
    ...
    sel.mask           = &mask;
    sel.survivor_count = rows.num_survivors;
    sel.route          = route;  // != full inside this branch (checked above)
    auto compacted = simpatico::decompress_column(tree, stream, mr, &err, /*pred=*/nullptr, &sel);
    if (compacted) { return retag(std::move(compacted)); }
                                                           ...
                                                           bool allow_raw)
{
  ...
  if (allow_raw &&
      (layout.batch_rows.size() == 1 || (late_mat_v2_enabled() && !layout.batch_rows.empty()))) {
    sel->raw_ids           = ids.ids;
  ...
  for (std::size_t k = 0; k < num_batches; ++k) {
    ...
    sirius::codegen::global_slice_to_local(sorted_ids + begin,
                                           s_k,
                                           layout.batch_row_start[k],
                                           static_cast<std::uint32_t*>(local.data()),
                                           stream);
    ...
    bsel.density = layout.batch_rows[k] > 0 ? static_cast<double>(s_k) / layout.batch_rows[k] : 0.0;
  }
  ...
  for (std::size_t k = 0; k < num_batches; ++k) {
    ...
    switch (in.kind) {
      ...
      case sirius::late_mat::row_selection_kind::id_list: {
        ...
        bsel.local_indices =
          rmm::device_buffer(static_cast<std::size_t>(live) * sizeof(std::int32_t), stream, mr);
        if (cudaMemcpyAsync(bsel.local_indices.data(),
                            in.row_ids->data(),
                                          ...
                                          rmm::device_async_resource_ref mr)
{
  ...
  if (sel.raw_ids != nullptr) {
    ...
    if (origin.batches.size() == 1) {
      ...
      if (src.compressed == nullptr) {
        if (src.uncompressed.null_count() != 0) { fail("nullable columns are not supported (v1)"); }
    ...
    if (all_uncompressed && cudf::is_fixed_width(origin.dtype)) {
      ...
      for (std::size_t k = 0; k < origin.batches.size(); ++k) {
        ...
        if (b.uncompressed.null_count() != 0) { fail("nullable columns are not supported (v1)"); }
  ...
  for (std::size_t k = 0; k < origin.batches.size(); ++k) {
    ...
    if (src.compressed != nullptr) {
      parts.push_back(
        materialize_compressed_batch(*src.compressed, src.column_index, bsel, stream, mr));
    } else {
      ...
      if (bsel.dense || bsel.rows.num_survivors == src.num_rows) {
        ...
        parts.push_back(gather_one(src.uncompressed,
                                   int32_map_view(bsel.local_indices, bsel.rows.num_survivors),
                                   stream,
                                   mr));
      }
    }
  ...
  if (sel.needs_restore() && sel.original_count > 0) {
    return gather_one(
      merged->view(), int32_map_view(sel.restore_rank, sel.original_count), stream, mr);
  }
  return merged;
}
