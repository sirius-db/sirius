...
GPUPhysicalStreamingLimit::Execute(GPUIntermediateRelation &input_relation, GPUIntermediateRelation &output_relation) const {
  ...
  for (int col_idx = 0; col_idx < output_relation.columns.size(); col_idx++) {
    ...
    output_relation.columns[col_idx] = make_shared_ptr<GPUColumn>(
      take, materialize_column->data_wrapper.type, new_data,
      new_offset, materialize_column->data_wrapper.num_bytes,
    ...
    // Correct for both OFFSET=0 (reads offset[take]) and OFFSET>0 (reads offset[skip+take]).
    if (take > 0 && output_relation.columns[col_idx]->data_wrapper.type.id() == GPUColumnTypeId::VARCHAR) {
    ...
    SIRIUS_LOG_DEBUG("Column {} has {} rows (offset={}, take={})", col_idx,
                     output_relation.columns[col_idx]->column_length, skip, take);
  }
