...
vector<LogicalType> GPUPhysicalAsOfJoin::GetJoinTypes() const
{
...
OperatorResultType GPUPhysicalAsOfJoin::ResolveComplexJoin(
  GPUIntermediateRelation& input_relation, GPUIntermediateRelation& output_relation) const
{
  auto start = std::chrono::high_resolution_clock::now();
                                                   ...
                                                   bool build_rhs)
{
  ...
  auto& join_op           = op.Cast<GPUPhysicalAsOfJoin>();
...
{
  GPUPhysicalAsOfJoin::BuildJoinPipelines(current, meta_pipeline, *this);
}
