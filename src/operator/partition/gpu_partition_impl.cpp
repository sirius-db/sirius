...
sirius::vector<sirius::unique_ptr<data_batch>> gpu_partition_impl::hash_partition(
    ...
    auto partition_result = cudf::hash_partition(
        input_table, partition_key_idx, num_partitions, cudf::hash_id::HASH_MURMUR3, cudf::DEFAULT_HASH_SEED,
    ...
    for (int i = 0; i < num_partitions; ++i) {
        ...
        slice_indices.push_back(i == num_partitions - 1 ? input_table.num_rows() : partition_result.second[i + 1]);
    }
    ...
    for (int i = 0; i < num_partitions; ++i) {
        ...
        auto gpu_table_representation = sirius::make_unique<sirius::gpu_table_representation>(
            *output_partition, memory_space);
        output_batches.push_back(sirius::make_unique<sirius::data_batch>(
            data_repository_mgr.get_next_data_batch_id(), data_repository_mgr, std::move(gpu_table_representation)));
    }
