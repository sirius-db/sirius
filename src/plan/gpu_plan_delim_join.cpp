...
// unique_ptr<GPUPhysicalOperator> GPUPhysicalPlanGenerator::PlanDelimJoin(LogicalComparisonJoin
// &op) {
// 	// first create the underlying join
// 	auto plan = PlanComparisonJoin(op);
// 	// this should create a join, not a cross product
...
// 		auto &bound_ref = delim_expr->Cast<BoundReferenceExpression>();
// 		delim_types.push_back(bound_ref.return_type);
// 		distinct_groups.push_back(make_uniq<BoundReferenceExpression>(bound_ref.return_type,
// bound_ref.index));
// 	}
// 	// now create the duplicate eliminated join
// 	unique_ptr<GPUPhysicalDelimJoin> delim_join;
// 	if (op.delim_flipped) {
// 		delim_join =
// 		    make_uniq<GPUPhysicalRightDelimJoin>(op.types, std::move(plan), delim_scans,
// op.estimated_cardinality); 	} else {
// 		// throw NotImplementedException("Left Delim Join not implemented");
// 		delim_join = make_uniq<GPUPhysicalLeftDelimJoin>(op.types, std::move(plan), delim_scans,
// op.estimated_cardinality);
// 	}
// 	// we still have to create the DISTINCT clause that is used to generate the duplicate eliminated
// chunk 	delim_join->distinct = make_uniq<GPUPhysicalGroupedAggregate>(context, delim_types,
// std::move(distinct_expressions), std::move(distinct_groups), op.estimated_cardinality); 	return
// std::move(delim_join);
// }
