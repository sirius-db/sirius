...
// std::move(op.bind_data), op.column_ids,
// 		//                                                   op.estimated_cardinality,
// std::move(op.projected_input));
// 		// node->children.push_back(CreatePlan(std::move(op.children[0])));
// 		// return std::move(node);
// 		throw NotImplementedException("Table producing functions that consume subquery results are not
// supported yet");
// 	}
...
// 		auto node = make_uniq<GPUPhysicalTableScan>(op.returned_types, op.function,
// std::move(op.bind_data), 		                                         op.returned_types,
// op.column_ids, vector<column_t>(), op.names, std::move(table_filters), op.estimated_cardinality,
// op.extra_info);
...
// 		auto projection =
// 		    make_uniq<GPUPhysicalProjection>(std::move(types), std::move(expressions),
// op.estimated_cardinality); 		projection->children.push_back(std::move(node)); 		return
// std::move(projection); 	} else { 		return make_uniq<GPUPhysicalTableScan>(op.types,
// op.function, std::move(op.bind_data), op.returned_types, op.column_ids, op.projection_ids,
// op.names, std::move(table_filters), op.estimated_cardinality, op.extra_info);
// 	}
// }
