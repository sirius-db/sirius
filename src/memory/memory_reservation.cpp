...
reservation::reservation(Tier t, int dev_id, size_t s)
    : tier(t)
    , device_id(dev_id)
    , size(s)
{}
...
memory_reservation_manager& memory_reservation_manager::get_instance()
{
  if (!_instance)
  {
    throw std::runtime_error("memory_reservation_manager not initialized. Call initialize() "
                             "first.");
  }
  return *_instance;
}
...
memory_reservation_manager::request_reservation(const reservation_request& request, size_t size)
{
...
std::vector<const memory_space*>
memory_reservation_manager::get_memory_spaces_for_tier(Tier tier) const
{
...
memory_reservation_manager::select_memory_space_and_make_reservation(
  const reservation_request& request,
  size_t size) const
{
