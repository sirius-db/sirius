...
void GPUBufferManager::registerExternalTablePacked(
    const string& table_name,
    uint8_t* gpu_data,
    size_t gpu_size,
    std::string metadata,
  ...
  if (existing != tables.end()) {
    ...
    cudaDeviceSynchronize();
    {
      ...
      for (size_t i = 0; i < gpu_size; i++) reg_checksum += host_buf[i];
      int32_t first4 = *reinterpret_cast<int32_t*>(host_buf.data());
      bool looks_like_ascii = (first4 > 0x20000000 && first4 < 0x7f7f7f7f);
      SIRIUS_LOG_INFO("[registerExternalTablePacked] GPU data at 0x{:x} size={} checksum={} first4=0x{:08x} ascii={}",
                      reinterpret_cast<uintptr_t>(gpu_data), gpu_size, reg_checksum,
