...
op::sirius_physical_streaming_source& stream_session::resolve_source(stream_id_t id) const
{
  ...
  if (it == _sources.end()) {
    throw sirius::invalid_input_exception("stream_session: no input stream with id " + id_str(id));
  }
  return *it->second;
}
...
const stream_session::sink_output& stream_session::resolve_sink(stream_id_t id) const
{
  ...
  if (it == _sinks.end()) {
    throw sirius::invalid_input_exception("stream_session: no output stream with id " + id_str(id));
  }
  return it->second;
}
