...
bool stream_lifecycle::admit(const std::function<void()>& insert)
{
  std::function<void()> waker;
  {
    ...
    // The insert runs under the lock so close cannot interleave, and so the batch is in the
    // repository before anything below can observe the wake.
    insert();
    waker  = std::move(_waker);
    _waker = nullptr;  // one-shot: the consumer re-arms on its next WAITING classification
  }
  _cv.notify_all();
  // Fired outside the lock: the waker calls back into the scheduler, which must not be able to
  // re-enter this lifecycle while it is held.
  if (waker) { waker(); }
  return true;
}
...
void stream_lifecycle::mark_sender_done(sender_id_t sender)
{
  std::function<void()> hook;
  {
    ...
    // Set insert, not a counter: this is what makes a repeated close idempotent *and* keeps
    // fan-in correct — two closes from sender 0 must not stand in for senders {0, 1}.
...
stream_lifecycle::availability stream_lifecycle::classify(bool repo_empty) const
{
  ...
  // Queued data outranks terminal: end-of-stream is never reported while a batch the stream
  // already accepted is still pullable.
...
{
  std::lock_guard<std::mutex> lock(_mutex);
  // Re-checking the caller's starvation predicate under this lock is what closes the lost-wake
  // race: admit() takes the same lock to insert *and* to take the waker, so a push either
  // happened before this check (arm_if sees the batch) or happens after the arm (and fires it).
...
void stream_lifecycle::set_on_end_of_stream(std::function<void()> hook)
{
  std::function<void()> fire_now;
  {
    ...
    // A hook wired after the stream already ended would otherwise never fire — the same
    // raced-close recheck the channel-based source did explicitly after set_on_close().
