  ...
  vector<string>& names)
{
  ...
  SIRIUS_LOG_INFO("[gpu_processing_substrait] bind: start, input size={}",
                  input.inputs[0].ToString().size());
  string serialized = input.inputs[0].GetValueUnsafe<string>();
  bool is_json      = false;
  // Use the new connection's context to avoid deadlock with the locked caller context.
  SIRIUS_LOG_INFO("[gpu_processing_substrait] bind: transforming Substrait plan ({} bytes)",
                  serialized.size());
  ...
  D_ASSERT(planner.plan);
  SIRIUS_LOG_INFO("[gpu_processing_substrait] bind: DuckDB plan created, {} columns",
                  planner.names.size());
  ...
  SIRIUS_LOG_INFO("[gpu_processing_substrait] bind: done, {} columns, {} types",
                  names.size(),
                  return_types.size());
                                                     ...
                                                     DataChunk& output)
{
  ...
  if (!data.res) {
    SIRIUS_LOG_INFO(
      "[gpu_processing_substrait] exec: starting, buffer_initialized={}, plan_error={}",
    ...
    if (!buffer_is_initialized) {
      ...
      data.res = data.gpu_context->GPUExecuteQuery(context, data.query, data.gpu_prepared, {});
      SIRIUS_LOG_INFO("[gpu_processing_substrait] exec: GPUExecuteQuery returned, hasError={}",
                      data.res->HasError());
      if (data.res->HasError()) {
                                                           ...
                                                           vector<string>& names)
{
  ...
  if (!context.registered_state->Get<SiriusContext>("sirius_state")) {
    ...
    if (sirius_state) { context.registered_state->Insert("sirius_state", sirius_state); }
  }
                                           ...
                                           DataChunk& output)
{
  ...
  if (!data.res) {
    SIRIUS_LOG_INFO(
      "[GPUExecutionFunction] executing: plan_error={}, gpu_prepared={}, sirius_iface={}",
      data.plan_error,
      (void*)data.gpu_prepared.get(),
      (void*)data.sirius_iface.get());
    auto start = std::chrono::high_resolution_clock::now();
    if (data.plan_error) {
      SIRIUS_LOG_WARN("[GPUExecutionFunction] GPU plan failed, falling back to from_substrait");
      if (!data.substrait_blob.empty()) {
        data.res =
          data.conn->TableFunction("from_substrait", {Value::BLOB_RAW(data.substrait_blob)})
            ->Execute();
      } else {
      ...
      SIRIUS_LOG_INFO("[GPUExecutionFunction] sirius_execute_query returned, hasError={}",
                      data.res->HasError());
      if (data.res->HasError()) {
...
unique_ptr<FunctionData> SiriusExtension::GPUExecutionSubstraitBind(
  ClientContext& context,
  TableFunctionBindInput& input,
  vector<LogicalType>& return_types,
  vector<string>& names)
{
  ...
  try {
    ...
    SIRIUS_LOG_INFO(
      "[gpu_execution_substrait] bind: Sirius physical plan generated OK, root type={}",
    ...
    ErrorData error(e);
    SIRIUS_LOG_ERROR("[gpu_execution_substrait] Error in SiriusGeneratePhysicalPlan: {}",
                     error.RawMessage());
    result->plan_error = true;
  }
...
unique_ptr<FunctionData> SiriusExtension::GPUAllocateBuffersBind(ClientContext& context,
                                                                 TableFunctionBindInput& input,
                                                                 vector<LogicalType>& return_types,
                                                                 vector<string>& names)
{
...
void SiriusExtension::GPUAllocateBuffersFunction(ClientContext& context,
                                                 TableFunctionInput& data_p,
                                                 DataChunk& output)
{
...
unique_ptr<FunctionData> SiriusExtension::GetLastGPUBuffersBind(ClientContext& context,
                                                                TableFunctionBindInput& input,
                                                                vector<LogicalType>& return_types,
                                                                vector<string>& names)
{
...
void SiriusExtension::GetLastGPUBuffersFunction(ClientContext& context,
                                                TableFunctionInput& data_p,
                                                DataChunk& output)
{
...
unique_ptr<FunctionData> SiriusExtension::RetainGPUBuffersBind(ClientContext& context,
                                                               TableFunctionBindInput& input,
                                                               vector<LogicalType>& return_types,
                                                               vector<string>& names)
{
...
void SiriusExtension::RetainGPUBuffersFunction(ClientContext& context,
                                               TableFunctionInput& data_p,
                                               DataChunk& output)
{
...
unique_ptr<FunctionData> SiriusExtension::ReleaseGPUBuffersBind(ClientContext& context,
                                                                TableFunctionBindInput& input,
                                                                vector<LogicalType>& return_types,
                                                                vector<string>& names)
{
...
void SiriusExtension::ReleaseGPUBuffersFunction(ClientContext& context,
                                                TableFunctionInput& data_p,
                                                DataChunk& output)
{
...
static unique_ptr<FunctionData> StageGPUBuffersBind(ClientContext& context,
                                                    TableFunctionBindInput& input,
                                                    vector<LogicalType>& return_types,
                                                    vector<string>& names)
{
  return_types = {
    LogicalType::INTEGER, LogicalType::BIGINT, LogicalType::BIGINT, LogicalType::VARCHAR};
  ...
  for (int i = 0; i < static_cast<int>(buffers.size()); i++) {
    ...
    // Data buffer.
    if (buf.len > 0) {
      auto err = cudaMemcpy(reinterpret_cast<void*>(result->staging_ptr + offset),
                            reinterpret_cast<const void*>(buf.addr),
                            buf.len,
                            cudaMemcpyDefault);
      if (err != cudaSuccess) {
        throw IOException(
          "sirius_stage_gpu_buffers: cudaMemcpy data buf %d (addr=0x%lx len=%zu -> staging "
          "0x%lx+%zu) failed: %s",
          i,
          buf.addr,
          buf.len,
...
static void StageGPUBuffersFunction(ClientContext& context,
                                    TableFunctionInput& data_p,
                                    DataChunk& output)
{
...
static unique_ptr<FunctionData> CudaAllocBind(ClientContext& context,
                                              TableFunctionBindInput& input,
                                              vector<LogicalType>& return_types,
                                              vector<string>& names)
{
  ...
  if (err != cudaSuccess) {
    throw IOException(
      "sirius_cuda_alloc: cudaMalloc(%zu) failed: %s", result->size, cudaGetErrorString(err));
  }
...
static void CudaAllocFunction(ClientContext& context, TableFunctionInput& data_p, DataChunk& output)
{
...
unique_ptr<FunctionData> GetPoolInfoBind(ClientContext& context,
                                         TableFunctionBindInput& input,
                                         vector<LogicalType>& return_types,
                                         vector<string>& names)
{
...
void GetPoolInfoFunction(ClientContext& context, TableFunctionInput& data_p, DataChunk& output)
{
...
static unique_ptr<FunctionData> FromSubstraitLocalBind(ClientContext& context,
                                                       TableFunctionBindInput& input,
                                                       vector<LogicalType>& return_types,
                                                       vector<string>& names)
{
...
static unique_ptr<TableRef> FromSubstraitLocalBindReplace(ClientContext& context,
                                                          TableFunctionBindInput& input)
{
  if (input.inputs[0].IsNull()) {
    throw BinderException("from_substrait_local cannot be called with a NULL parameter");
  ...
  if (!plan->IsReadOnly()) { return nullptr; }
  ...
  if (parser.statements.empty()) { return nullptr; }
  auto& parsed = parser.statements[0];
  if (parsed->type != StatementType::SELECT_STATEMENT) { return nullptr; }
  ...
  fprintf(stderr,
          "[from_substrait_local] bind_replace returning SubqueryRef for SQL: %s\n",
          sql.substr(0, 200).c_str());
  return unique_ptr<TableRef>(ref);
}

static void FromSubstraitLocalFunction(ClientContext& context,
                                       TableFunctionInput& data_p,
                                       DataChunk& output)
{
...
void SiriusExtension::RegisterGPUFunctions(DatabaseInstance& instance)
{
  ...
  TableFunction get_last_gpu_buffers(
    "sirius_get_last_gpu_buffers", {}, GetLastGPUBuffersFunction, GetLastGPUBuffersBind);
  ...
  TableFunction get_pool_info("sirius_get_pool_info", {}, GetPoolInfoFunction, GetPoolInfoBind);
  CreateTableFunctionInfo get_pool_info_info(get_pool_info);
  catalog.CreateTableFunction(transaction, get_pool_info_info);

  TableFunction retain_gpu_buffers(
    "sirius_retain_gpu_buffers", {}, RetainGPUBuffersFunction, RetainGPUBuffersBind);
  ...
  {
    struct TakeSessionData : public TableFunctionData {
      bool finished = false;
    };
    auto take_bind = [](ClientContext&,
  ...
  TableFunction release_gpu_buffers(
    "sirius_release_gpu_buffers", {}, ReleaseGPUBuffersFunction, ReleaseGPUBuffersBind);
  ...
  TableFunction cuda_alloc(
    "sirius_cuda_alloc", {LogicalType::BIGINT}, CudaAllocFunction, CudaAllocBind);
  CreateTableFunctionInfo cuda_alloc_info(cuda_alloc);
  catalog.CreateTableFunction(transaction, cuda_alloc_info);

  // sirius_register_packed_table(table_name VARCHAR, gpu_addr BIGINT, gpu_size BIGINT, metadata
  // BLOB) Unpacks a cudf::pack()'d GPU buffer and registers it as a DuckDB table. This skips the
  // PBlock→CPU→Arrow IPC→DuckDB roundtrip entirely.
  {
    ...
    auto bind = [](ClientContext& ctx,
      ...
      if (buffer_is_initialized) {
        auto& mgr = GPUBufferManager::GetInstance();
        mgr.registerExternalTablePacked(
          table_name, gpu_ptr, gpu_size, std::move(md_blob), num_cols, num_rows_out);
      }
          ...
          case cudf::type_id::DECIMAL128: {
            int prec = (col.type().id() == cudf::type_id::DECIMAL32)   ? 9
                       : (col.type().id() == cudf::type_id::DECIMAL64) ? 18
                                                                       : 38;
            dtype =
              "DECIMAL(" + std::to_string(prec) + "," + std::to_string(-col.type().scale()) + ")";
            break;
          }
      ...
      result->create_table_sql =
        "CREATE OR REPLACE TABLE \"" + table_name + "\" (" + col_defs + ")";
    ...
    TableFunction reg_packed(
      "sirius_register_packed_table",
  ...
  {
    struct SetStagingData : public TableFunctionData {
      bool finished = false;
    };
    auto bind = [](ClientContext&,
                   TableFunctionBindInput& input,
                   vector<LogicalType>& return_types,
                   vector<string>& names) -> unique_ptr<FunctionData> {
      return_types = {LogicalType::VARCHAR};
    ...
    TableFunction set_staging(
      "sirius_set_staging_buffer", {LogicalType::BIGINT, LogicalType::BIGINT}, func, bind);
  ...
  {
    struct SetPartData : public TableFunctionData {
      bool finished = false;
    };
    auto bind = [](ClientContext&,
                   TableFunctionBindInput& input,
      ...
      std::vector<int> cols;
      for (auto& c : children) {
        cols.push_back(c.GetValue<int32_t>());
      }
      LastGPUBuffers::GetInstance().SetPartitionConfig(num_parts, std::move(cols));
      SIRIUS_LOG_INFO(
        "[sirius_set_exchange_partition] num_partitions={} cols={}", num_parts, cols.size());
  ...
  // sirius_get_packed_broadcast() — returns per-batch broadcast packed buffers.
  {
    ...
    auto bind = [](ClientContext&,
      ...
      return_types = {LogicalType::INTEGER,
                      ...
                      LogicalType::BIGINT};
      names        = {
        "entry_id", "staging_offset", "packed_size", "metadata", "num_rows", "overflow_gpu_addr"};
  ...
  {
    struct FinalizeData : public TableFunctionData {
      bool finished = false;
    };
    auto bind = [](ClientContext&,
                   TableFunctionBindInput&,
    ...
    auto func = [](ClientContext&, TableFunctionInput& data_p, DataChunk& output) {
      ...
      data.finished = true;
      if (buffer_is_initialized) { GPUBufferManager::GetInstance().finalizeExchangeTables(); }
  ...
  struct GetPackedGPUData : public TableFunctionData {
    bool finished = false;
  };
  auto get_packed_gpu_bind = [](ClientContext& ctx,
                                TableFunctionBindInput& input,
                                vector<LogicalType>& return_types,
                                vector<string>& names) -> unique_ptr<FunctionData> {
  ...
  TableFunction get_packed_gpu(
    "sirius_get_packed_gpu", {}, get_packed_gpu_func, get_packed_gpu_bind);
