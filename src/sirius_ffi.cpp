  ...
  try {
    ...
    if (impl_->is_result()) {
      ...
      if (impl_->broadcast_outputs && impl_->outputs.size() > 1) {
        ...
        // key_cast_types left empty; streaming_fragment::build() derives them from the sink's
        // own output types (integral keys hash as INT64), so no type names cross the FFI and
        // the two ends cannot disagree on a key's width.
