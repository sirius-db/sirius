...
void Context::execute_substrait(const std::string& plan, std::uintptr_t out_stream_addr)
{
  ...
  try {
    ...
    auto physical_plan =
      sirius::planner::sirius_physical_plan_generator(client).create_plan(std::move(lowered.plan));
...
struct Fragment::Impl {
  ...
  void begin_lifecycle()
  {
    ctx.context->QueryBeginStandalone(*ctx.conn->context, kQueryLabel);
    lifecycle_open = true;
  }
  ...
  //! takes the lifecycle mutex that QueryBeginStandalone holds — and *after* the streams are
...
void Fragment::build(const std::string& substrait_plan)
{
  ...
  // must precede QueryBeginStandalone), and the lifecycle must then span planning and execution,
