...
copied_logical_plan copy_logical_plan(duckdb::LogicalOperator& plan, duckdb::ClientContext& context)
{
  copied_logical_plan result;
  result.plan = plan.Copy(context);
  planner::duckdb_join_filter_candidate_adapter::preserve_dynamic_filter_metadata(
    plan, *result.plan, result.counts);
  return result;
}
                           ...
                           duckdb::unique_ptr<duckdb::LogicalOperator>& plan)
{
  ...
  try {
    auto copied = copy_logical_plan(*plan, context);
    if (copied.counts.joins > 0 || copied.counts.gets > 0) {
      SIRIUS_LOG_DEBUG(
        "Transparent execution: preserved dynamic-filter metadata on {} join(s), {} get(s)",
        copied.counts.joins,
        copied.counts.gets);
    }
    ctx->set_captured_logical_plan(std::move(copied.plan));
