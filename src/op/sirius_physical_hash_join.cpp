...
std::unique_ptr<operator_data> sirius_physical_hash_join::get_next_task_input_data_for_build_probe()
{
  ...
  if (_hash_table_build_state == BUILD_HASH_TABLE_STATE::SCHEDULING) {
    ...
    auto probe_batch = probe_port->repo->pop_idle_data_batch();
    auto build_batch = build_port->repo->pop_idle_data_batch();
    ...
    std::vector<std::shared_ptr<cucascade::data_batch>> input_batch;
    auto batch = probe_port->repo->pop_idle_data_batch();
    if (batch) {
                                                                  ...
                                                                  rmm::cuda_stream_view stream)
{
  ...
  if (_join_mode == HASH_JOIN_MODE::BUILD_PROBE) {
    ...
    if (_hash_table_build_state == BUILD_HASH_TABLE_STATE::BUILT) {
      ...
      right_full =
        build_table_ro_holder->get_data()->cast<cucascade::gpu_table_representation>().get_table().view();
