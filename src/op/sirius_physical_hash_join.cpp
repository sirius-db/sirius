                                                                  ...
                                                                  rmm::cuda_stream_view stream)
{
  ...
  if (_join_mode == HASH_JOIN_MODE::BUILD_PROBE) {
    ...
    if (_hash_table_build_state == BUILD_HASH_TABLE_STATE::BUILT) {
      ...
      // Debug: log join dimensions
      SIRIUS_LOG_INFO("[hash_join_debug] id={} {} join, probe={}rows, build={}rows, distinct={}",
                      this->get_operator_id(),
                      duckdb::JoinTypeToString(join_type),
                      probe_keys.num_rows(),
                      _build_table->get_data()
                        ->cast<cucascade::gpu_table_representation>().get_table().view().num_rows(),
                      _distinct_hash_table != nullptr);
