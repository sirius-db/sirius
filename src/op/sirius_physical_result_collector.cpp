                                                  ...
                                                  rmm::cuda_stream_view stream)
{
  ...
  SIRIUS_LOG_INFO("[result_collector] sink called: {} batches, should_retain={}",
                  input_batches.size(),
                  should_retain);
  ...
  for (auto const& input_batch : input_batches) {
    ...
    if (data->get_current_tier() == cucascade::memory::Tier::GPU) {
      // Capture GPU buffer metadata before D2H conversion (for nixl exchange).
      {
        ...
        int device_id =
          input_batch->get_memory_space() ? input_batch->get_memory_space()->get_device_id() : 0;
        ...
        for (cudf::size_type i = 0; i < view.num_columns(); i++) {
          ...
          std::string col_name =
            (static_cast<size_t>(i) < names.size()) ? names[i] : "col_" + std::to_string(i);
        ...
        if (should_retain) {
          ...
          SIRIUS_LOG_INFO(
            "[result_collector] retain path: part_num={} part_cols={} staging_addr=0x{:x}",
            part_num,
            part_cols.size(),
            staging_addr);
          ...
          if (part_num > 0 && !part_cols.empty() && staging_addr != 0) {
            ...
            auto [partitioned_table, offsets] = cudf::hash_partition(view,
                                                                     col_indices,
                                                                     part_num,
            ...
            SIRIUS_LOG_INFO(
              "[result_collector] GPU hash_partition: {} partitions, {} rows → offsets: [{}]",
              part_num,
              view.num_rows(),
              [&]() {
                std::string s;
            ...
            for (cudf::size_type c = 0; c < partitioned_table->view().num_columns(); c++) {
              ...
              if (col.type().id() == cudf::type_id::INT32 && col.size() > 0) {
                ...
                SIRIUS_LOG_INFO(
                  "[result_collector] hash_part col {} INT32 first_val={}", c, first_val);
              }
              if (col.type().id() == cudf::type_id::STRING && col.num_children() > 0 &&
                ...
                SIRIUS_LOG_INFO(
                  "[result_collector] hash_part col {} STRING first_off={} last_off={} offset={}",
                  c,
                  first_off,
                  last_off,
                  col.offset());
            ...
            // The full-table estimate is unreliable because per-partition alignment padding
            // differs.
            ...
            for (int i = 0; i < part_num; i++) {
              ...
              auto psz    = cudf::chunked_pack::create(pslice[0], 1UL << 20, stream)
            ...
            for (int i = 0; i < part_num; i++) {
              ...
              auto total = cudf::chunked_pack::create(slice[0], 1UL << 20, stream)
                             ->get_total_contiguous_size();
              if (staging_offset + total > staging_size) {
                // Overflow: pack into separate rmm::device_buffer instead of dropping data.
                SIRIUS_LOG_WARN(
                  "[result_collector] partition {} ({} bytes) overflows staging, using "
                  "rmm::device_buffer",
                  i,
                  total);
                ...
                pp.overflow_data =
                  std::make_shared<rmm::device_buffer>(std::move(*packed.gpu_data));
                packed_parts.push_back(std::move(pp));
              } else {
                ...
                // Validate: read first 4 bytes of packed buffer — should be INT32 if col 0 is INT32
                {
                  ...
                  cudaMemcpy(&packed_first4,
                             reinterpret_cast<void*>(staging_addr + staging_offset),
                             4,
                             cudaMemcpyDeviceToHost);
                  auto first_col_type = slice[0].column(0).type().id();
                  bool looks_ascii    = (packed_first4 > 0x20000000 && packed_first4 < 0x7f7f7f7f);
                  SIRIUS_LOG_INFO(
                    "[result_collector] partition {} packed first4={} (0x{:08x}) col0_type={} "
                    "ascii={}",
                    i,
                    packed_first4,
                    static_cast<uint32_t>(packed_first4),
                    static_cast<int>(first_col_type),
                    looks_ascii);
                  if (looks_ascii && cudf::is_fixed_width(cudf::data_type{first_col_type})) {
                    ...
                    SIRIUS_LOG_ERROR(
                      "[result_collector] SENDER CORRUPTION: partition {} has ASCII at INT32 "
                      "offset 0!",
                      i);
                    ...
                    SIRIUS_LOG_ERROR("[result_collector] cudf::pack first4={} (0x{:08x}) — {}",
                                     ref_first4,
                                     static_cast<uint32_t>(ref_first4),
                                     (ref_first4 > 0x20000000 && ref_first4 < 0x7f7f7f7f)
                                       ? "ALSO ASCII"
                                       : "OK (INT32)");
                    ...
                    for (cudf::size_type c = 0; c < slice[0].num_columns() && c < 3; c++) {
                      ...
                      SIRIUS_LOG_ERROR(
                        "[result_collector] slice col {} type={} rows={} offset={} data=0x{:x}",
                        c,
                        static_cast<int>(col.type().id()),
                        col.size(),
                        col.offset(),
            ...
            if (actual_total_packed > aligned_estimate) {
              SIRIUS_LOG_ERROR(
                "[result_collector] OVERFLOW: actual_total_packed={} > estimated={} (diff={})",
                actual_total_packed,
                aligned_estimate,
                actual_total_packed - aligned_estimate);
            } else {
              SIRIUS_LOG_INFO(
                "[result_collector] staging estimate OK: actual={} estimated={} (slack={})",
                actual_total_packed,
                aligned_estimate,
                aligned_estimate - actual_total_packed);
            }
            ...
            lgb.StorePackedData(
              nullptr, staging_addr, staging_offset, std::make_unique<std::vector<uint8_t>>());
            ...
            if (staging_offset + total_size <= staging_size) {
              ...
              SIRIUS_LOG_WARN(
                "[result_collector] broadcast batch ({} bytes) overflows staging, using "
                "rmm::device_buffer",
                total_size);
            ...
            lgb.StorePackedData(nullptr,
                                staging_addr,
                                lgb.GetStagingOffset(),
            ...
            auto gpu_size = packed.gpu_data->size();
            SIRIUS_LOG_INFO(
              "[result_collector] cudf::pack (no staging): {} bytes at 0x{:x}", gpu_size, gpu_addr);
            duckdb::PackedBroadcastEntry entry;
            entry.staging_offset    = 0;
            entry.packed_size       = gpu_size;
            entry.metadata          = std::move(packed.metadata);
  ...
  if (should_retain) { duckdb::LastGPUBuffers::GetInstance().SetRetainNext(false); }
}
