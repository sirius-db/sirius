...
host_table_chunk_reader::column_reader::column_reader(
  column_metadata const& cm, std::unique_ptr<multiple_blocks_allocation> const& allocation)
{
  if (allocation == nullptr || allocation->block_size() == 0) {
    throw std::runtime_error(
      "[host_table_chunk_reader::column_reader::column_reader] Invalid allocation.");
  }
  size          = static_cast<size_t>(cm.num_rows);
  null_count    = static_cast<size_t>(cm.null_count);
  cudf_col_type = cudf::data_type(cm.type_id, cm.scale);
  if (!cm.has_null_mask) { null_count = 0; }
  ...
  if (cm.type_id == cudf::type_id::STRING) {
    ...
    use_int64_offsets = (cm.children[0].type_id == cudf::type_id::INT64);
    if (use_int64_offsets) {
      offset_accessor_64.initialize(cm.children[0].data_offset, allocation);
    } else {
      offset_accessor_32.initialize(cm.children[0].data_offset, allocation);
    }
  }
}
