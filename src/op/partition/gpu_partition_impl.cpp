  ...
  cucascade::memory::memory_space& memory_space)
{
  ...
  // Slice the single reordered table into per-partition views (zero-copy). The appended cast
  // columns are dropped via select(). These views reference partition_result.first's device
  // buffers, so it is returned to the caller to keep alive.
...
std::vector<std::shared_ptr<cucascade::data_batch>> gpu_partition_impl::hash_partition(
  ...
  cucascade::memory::memory_space& memory_space)
{
  // Materialize each partition view into its own contiguous, downgradable table. The reordered
  // parent table (held by `reordered`) stays alive until every view has been copied out, then
  // is freed at end of scope.
