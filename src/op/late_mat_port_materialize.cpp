  ...
  rmm::cuda_stream_view stream)
{
  ...
  for (auto const& bundle : directive.bundles) {
    ...
    if (rowid_view.type().id() == cudf::type_id::UINT32) {
      holders.push_back(cudf::cast(rowid_view, cudf::data_type{cudf::type_id::UINT64}, stream, mr));
    ...
    // Review-F1 content check: rowid values must address the pinned table.
    {
      ...
      if (max_id >= static_cast<std::uint64_t>(total_rows)) {
        throw std::runtime_error("[late_mat_apply_port_directive] rowid max " +
                                 std::to_string(max_id) + " out of range for pinned table with " +
  ...
  for (auto const& pb : prepared) {
    for (std::size_t i = 0; i < pb.bundle->positions.size(); ++i) {
      ...
      placeholders.push_back(std::exchange(cols[pb.bundle->positions[i]], std::move(materialized)));
    }
  }
