...
duckdb::ExtraOperatorInfo copy_extra_info_iceberg_scan(const duckdb::ExtraOperatorInfo& src)
{
...
sirius_physical_iceberg_scan::sirius_physical_iceberg_scan(sirius_physical_table_scan* table_scan,
  ...
  : sirius_physical_iceberg_scan(
      ...
      copy_extra_info_iceberg_scan(table_scan->extra_info),
...
sirius_physical_iceberg_scan::sirius_physical_iceberg_scan(
  ...
  : sirius_physical_operator(
      SiriusPhysicalOperatorType::ICEBERG_SCAN, std::move(types), estimated_cardinality),
  ...
  if (table_filters && !table_filters->filters.empty()) {
    ...
    if (duckdb_expression) {
      ...
      if (!all_translations_ok) {
        ...
        SIRIUS_LOG_INFO(
          "[sirius_physical_iceberg_scan] Failed to translate filter expression for pushdown. "
...
std::size_t sirius_physical_iceberg_scan::no_history_peak_memory_estimate(
