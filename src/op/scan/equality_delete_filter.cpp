...
equality_delete_filter::equality_delete_filter(
...
std::unique_ptr<cudf::table> equality_delete_filter::apply(
  ...
  rmm::cuda_stream_view stream)
{
  ...
  for (auto idx : _data_key_indices) {
    if (idx >= static_cast<cudf::size_type>(tbl->num_columns())) {
      SIRIUS_LOG_WARN(
        "[equality_delete_filter] Key column index {} >= num_columns {}; skipping.",
        idx, tbl->num_columns());
      return tbl;
    }
  }
  ...
  // or JoinNoMatch if no match.
  auto build_indices = _hash_join->left_join(data_key_view, stream);

  // Build anti-join mask entirely on GPU (no host roundtrip).
