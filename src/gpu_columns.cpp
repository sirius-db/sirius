 ...
 */
 ...
 size_t DataWrapper::getColumnTypeSize() const {
     switch (type.id()) {
         ...
         default:
             throw duckdb::InternalException("Unsupported sirius column type size: %d", static_cast<int>(type.id()));
     }
 }
 
 GPUColumn::GPUColumn(size_t _column_length, GPUColumnType type, uint8_t* data, cudf::bitmask_type* validity_mask) {
 ...
 cudf::column_view GPUColumn::convertToCudfColumn() {
     ...
     if (data_wrapper.type.id() == GPUColumnTypeId::INT64) {
         return cudf::column_view(cudf::data_type(cudf::type_id::INT64), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::INT32) {
         return cudf::column_view(cudf::data_type(cudf::type_id::INT32), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
         ...
         return cudf::column_view(cudf::data_type(cudf::type_id::INT16), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::FLOAT32) {
         return cudf::column_view(cudf::data_type(cudf::type_id::FLOAT32), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::FLOAT64) {
         return cudf::column_view(cudf::data_type(cudf::type_id::FLOAT64), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::BOOLEAN) {
         return cudf::column_view(cudf::data_type(cudf::type_id::BOOL8), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::DATE) {
         return cudf::column_view(cudf::data_type(cudf::type_id::TIMESTAMP_DAYS), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::TIMESTAMP_SEC) {
         return cudf::column_view(cudf::data_type(cudf::type_id::TIMESTAMP_SECONDS), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::TIMESTAMP_MS) {
         return cudf::column_view(cudf::data_type(cudf::type_id::TIMESTAMP_MILLISECONDS), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::TIMESTAMP_US) {
         return cudf::column_view(cudf::data_type(cudf::type_id::TIMESTAMP_MICROSECONDS), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::TIMESTAMP_NS) {
         return cudf::column_view(cudf::data_type(cudf::type_id::TIMESTAMP_NANOSECONDS), size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     } else if (data_wrapper.type.id() == GPUColumnTypeId::VARCHAR) {
         auto offsets_col = cudf::column_view(cudf::data_type{cudf::type_id::INT64}, size + 1, data_wrapper.offset, nullptr, 0);
         std::vector<cudf::column_view> children;
         children.push_back(offsets_col);
         return cudf::column_view(cudf::data_type{cudf::type_id::STRING}, size, data_wrapper.data, data_wrapper.validity_mask, null_count, 0, std::move(children));
     } else if (data_wrapper.type.id() == GPUColumnTypeId::DECIMAL) {
         cudf::data_type cudf_type;
         switch (data_wrapper.getColumnTypeSize()) {
             case sizeof(int32_t): cudf_type = cudf::data_type(cudf::type_id::DECIMAL32, -data_wrapper.type.GetDecimalTypeInfo()->scale_); break;
             case sizeof(int64_t): cudf_type = cudf::data_type(cudf::type_id::DECIMAL64, -data_wrapper.type.GetDecimalTypeInfo()->scale_); break;
             case sizeof(__int128_t): cudf_type = cudf::data_type(cudf::type_id::DECIMAL128, -data_wrapper.type.GetDecimalTypeInfo()->scale_); break;
             default: throw duckdb::InternalException("Unsupported decimal size");
         }
         return cudf::column_view(cudf_type, size, data_wrapper.data, data_wrapper.validity_mask, null_count);
     }
     throw duckdb::InternalException("Unsupported sirius column type: %d", data_wrapper.type.id());
 }
 
 void GPUColumn::setFromCudfColumn(cudf::column& cudf_column, bool _is_unique, int32_t* _row_ids, uint64_t _row_id_count, GPUBufferManager* gpuBufferManager) {
     ...
     if (col_type == cudf::data_type(cudf::type_id::STRING)) {
         if (cont.children[0]->type().id() == cudf::type_id::INT32) {
             ...
             convertCudfOffsetToSiriusOffset(reinterpret_cast<int32_t*>(gpuBufferManager->rmm_stored_buffers.back()->data()));
         ...
         throw NotImplementedException("Unsupported cudf data type in `setFromCudfColumn`: %d", static_cast<int>(col_type.id()));
     }
 ...
 void GPUColumn::setFromCudfScalar(cudf::scalar& cudf_scalar, GPUBufferManager* gpuBufferManager) {
     ...
     if (scalar_type == cudf::data_type(cudf::type_id::INT64)) {
         auto& s = static_cast<cudf::numeric_scalar<uint64_t>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 8;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::INT64);
     } else if (scalar_type == cudf::data_type(cudf::type_id::INT32)) {
         auto& s = static_cast<cudf::numeric_scalar<int32_t>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 4;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::INT32);
     } else if (scalar_type == cudf::data_type(cudf::type_id::INT16)) { // [Fix] Add INT16 scalar support
         auto& s = static_cast<cudf::numeric_scalar<int16_t>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 2;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::INT16);
     } else if (scalar_type == cudf::data_type(cudf::type_id::FLOAT32)) {
         auto& s = static_cast<cudf::numeric_scalar<float>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 4;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::FLOAT32);
     } else if (scalar_type == cudf::data_type(cudf::type_id::FLOAT64)) {
         auto& s = static_cast<cudf::numeric_scalar<double>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 8;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::FLOAT64);
     } else if (scalar_type == cudf::data_type(cudf::type_id::BOOL8)) {
         auto& s = static_cast<cudf::numeric_scalar<bool>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 1;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::BOOLEAN);
     } else if (scalar_type.id() == cudf::type_id::DECIMAL32) {
         auto& s = static_cast<cudf::fixed_point_scalar<numeric::decimal32>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 4;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::DECIMAL);
         data_wrapper.type.SetDecimalTypeInfo(Decimal::MAX_WIDTH_INT32, -s.type().scale());
     } else if (scalar_type.id() == cudf::type_id::DECIMAL64) {
         auto& s = static_cast<cudf::fixed_point_scalar<numeric::decimal64>&>(cudf_scalar); scalar_ptr = s.data(); scalar_size = 8;
         data_wrapper.type = GPUColumnType(GPUColumnTypeId::DECIMAL);
         ...
         throw NotImplementedException("Unsupported scalar type: %d", static_cast<int>(scalar_type.id()));
     }
 
     data_wrapper.data = gpuBufferManager->customCudaMalloc<uint8_t>(scalar_size, 0, 0);
     callCudaMemcpyDeviceToDevice<uint8_t>(data_wrapper.data, reinterpret_cast<uint8_t*>(scalar_ptr), scalar_size, 0);
 ...
 GPUIntermediateRelation::GPUIntermediateRelation(size_t column_count) : column_count(column_count) {
     ...
     for (int i = 0; i < column_count; i++) columns[i] = nullptr;
 }
 
 bool GPUIntermediateRelation::checkLateMaterialization(size_t idx) {
     SIRIUS_LOG_DEBUG("Checking late mat idx {}", idx);
     if (columns[idx] == nullptr) return false;
 ...
 int16_t* GPUColumn::GetDataInt16() { 
     return reinterpret_cast<int16_t*>(data_wrapper.data); 
 }
