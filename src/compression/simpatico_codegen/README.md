...
Use the `cuda12` environment on systems that require CUDA 12:

```bash
pixi run -e cuda12 make
pixi run -e cuda12 build/release/extension/sirius/test/cpp/sirius_unittest "[compression]"
```

## Standalone CLI and tests

The standalone build produces the `simpatico` CLI and the dedicated Simpatico CTest suite.
It uses the Sirius repository's Pixi environment.
...
pixi run ./build/simpatico explore --input lineitem.tbl --beam-width 200 --max-depth 8
...
```bash
pixi run ./build/simpatico compress --input data.parquet --plan plans/example_plans.txt --out data.hpln
# --verify decompresses in-memory and checks the round-trip matches the input
pixi run ./build/simpatico compress --input data.parquet --plan plans/example_plans.txt --out data.hpln --verify
```
...
```bash
pixi run ./build/simpatico benchmark --input data.parquet --plan plans/intraday_balanced.txt \
    --warmup 5 --iters 20
