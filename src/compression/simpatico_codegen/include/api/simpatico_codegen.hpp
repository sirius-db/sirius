...
struct compressed_column {
  ...
  /// Compressed payload together with the plan tree that describes its layout.
  std::unique_ptr<simpatico::plan_compound> compound;
};
...
std::unique_ptr<cudf::column> decompress(
  const simpatico::plan_compound& compound,
  rmm::cuda_stream_view stream      = cudf::get_default_stream(),
  rmm::device_async_resource_ref mr = rmm::mr::get_current_device_resource_ref());
