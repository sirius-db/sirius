...
/// JIT-encode the fusable subtree rooted at PlanTree node ``start_node`` from a
/// contiguous int32/int64 column. Writes the region's reps into ``builder``
/// (NodeId-keyed in builder.leaves; raw passthrough reps in
/// builder.raw_passthrough_leaves); entropy tails on the region's boundary
/// outputs are scheduled by the caller's compress walk. Returns true on
/// success. If ``head_out`` is non-null it receives the extracted CodegenHead so
/// the caller can inspect covered_nodes without a second extract_fusable_subtree
/// call.
