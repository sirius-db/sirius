                                            ...
                                            std::int32_t* __restrict__ out)
{
  ...
  std::int32_t const chunk_base =
    static_cast<std::int32_t>(chunk_ids[blockIdx.x]) * 1024;
                                    ...
                                    std::uint32_t* __restrict__ mask_words)
{
  ...
  if (threadIdx.x < kWordsPerChunk) {
    mask_words[static_cast<std::int64_t>(chunk_ids[blockIdx.x]) * kWordsPerChunk +
               threadIdx.x] = words[threadIdx.x];
  }
}
                                            ...
                                            rmm::device_async_resource_ref mr)
{
  ...
  if (count > num_rows) {
    throw std::runtime_error("latemat_rowset: bucket_sorted_local_ids: count > num_rows "
  ...
  std::size_t temp_bytes = 0;
  throw_on_cuda(cub::DeviceRunLengthEncode::Encode(
                  nullptr, temp_bytes, chunk_it,
  ...
  throw_on_cuda(cudaMemcpyAsync(&num_touched, num_runs_dev.data(), sizeof(std::int64_t),
  ...
  set.chunk_ids = rmm::device_buffer(
    static_cast<std::size_t>(num_touched) * sizeof(std::uint32_t), stream, mr);
  throw_on_cuda(cudaMemcpyAsync(set.chunk_ids.data(), unique_chunks.data(),
  ...
  device_prefix_sum<std::uint32_t>(static_cast<std::uint32_t*>(run_lengths.data()), offs,
                                   num_touched, /*inclusive=*/false, stream, mr,
                                   "offsets scan");
  ...
  set.in_chunk_offsets = rmm::device_buffer(
    static_cast<std::size_t>(count) * sizeof(std::uint16_t), stream, mr);
                                         ...
                                         rmm::device_async_resource_ref mr)
{
  ...
  throw_on_cuda(cudaMemcpyAsync(keys_a.data(), ids,
  ...
  std::size_t temp_bytes = 0;
  throw_on_cuda(cub::DeviceRadixSort::SortPairs(nullptr, temp_bytes, d_keys, d_vals, count,
                                                0, 64, stream.value()),
                "radix sort size query");
  rmm::device_buffer temp(temp_bytes, stream, mr);
  throw_on_cuda(cub::DeviceRadixSort::SortPairs(temp.data(), temp_bytes, d_keys, d_vals, count,
                                                0, 64, stream.value()),
                "radix sort");
  ...
  device_prefix_sum<std::int32_t>(static_cast<std::int32_t*>(flags.data()),
                                  static_cast<std::int32_t*>(rank_incl.data()), count,
                                  /*inclusive=*/true, stream, mr, "rank scan");
  ...
  out.ids =
    rmm::device_buffer(static_cast<std::size_t>(count) * sizeof(std::uint64_t), stream, mr);
  out.restore_rank =
  ...
  scatter_unique_and_rank_kernel<<<grid_for(count, kBlock), kBlock, 0, stream.value()>>>(
    sorted, perm, static_cast<std::int32_t const*>(rank_incl.data()), count,
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  rmm::device_buffer bounds(host_bounds.data(),
                            host_bounds.size() * sizeof(std::uint64_t), stream, mr);
  ...
  batch_starts_kernel<<<grid_for(num_bounds, kBlock), kBlock, 0, stream.value()>>>(
    sorted_ids, max_count, count_dev, static_cast<std::uint64_t const*>(bounds.data()),
    num_bounds, static_cast<std::int64_t*>(starts_dev.data()));
  ...
  throw_on_cuda(cudaMemcpyAsync(starts.data(), starts_dev.data(),
                                host_bounds.size() * sizeof(std::int64_t),
                                cudaMemcpyDeviceToHost, stream.value()),
                "batch starts D2H");
  if (count_dev != nullptr) {
    throw_on_cuda(cudaMemcpyAsync(&actual_count32, count_dev, sizeof(std::int32_t),
                                  cudaMemcpyDeviceToHost, stream.value()),
                  "count D2H");
  }
                              ...
                              rmm::cuda_stream_view stream)
{
  ...
  expand_local_indices_kernel<<<static_cast<unsigned>(set.num_touched), kBlock, 0,
                                stream.value()>>>(set.chunk_ids, set.chunk_out_offsets,
                                                  set.in_chunk_offsets, out);
...
void row_set_to_mask(chunk_row_set const& set,
                     ...
                     rmm::device_async_resource_ref mr)
{
  if (mask_words == nullptr || all_chunk_offsets == nullptr || !set.valid() ||
      set.num_rows <= 0) {
  ...
  throw_on_cuda(cudaMemsetAsync(mask_words, 0,
                                static_cast<std::size_t>(num_words) * sizeof(std::uint32_t),
                                stream.value()),
                "mask zero");
  if (set.num_survivors > 0) {
    scatter_mask_kernel<<<static_cast<unsigned>(set.num_touched), kBlock, 0,
                          stream.value()>>>(set.chunk_ids, set.chunk_out_offsets,
                                            set.in_chunk_offsets, mask_words);
  ...
  if (set.num_survivors > 0) {
    scatter_chunk_counts_kernel<<<grid_for(set.num_touched, kBlock), kBlock, 0,
                                  stream.value()>>>(set.chunk_ids, set.chunk_out_offsets,
                                                    set.num_touched,
  ...
  device_prefix_sum<std::uint32_t>(static_cast<std::uint32_t*>(dense_counts.data()),
                                   all_chunk_offsets, nc, /*inclusive=*/false, stream, mr,
                                   "chunk offsets scan");
