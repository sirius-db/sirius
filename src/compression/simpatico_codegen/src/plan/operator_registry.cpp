...
std::vector<OperatorInfo> const& operator_registry()
{
  ...
  static const std::vector<OperatorInfo> kTable = {
    ...
    // index into it. `indices` is the row-indexed channel. Validity is stripped upstream of the
    // plan (compress_column), so a nullable dictionary column carries no null_mask channel here.
