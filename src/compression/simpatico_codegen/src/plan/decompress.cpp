...
std::unique_ptr<cudf::column> rows_decode_fused_root(PlanTree const& tree,
...
std::unique_ptr<cudf::column> rows_decode_dict_fast_path(PlanTree const& tree,
                                                         sirius::codegen::chunk_row_set const& rows,
                                                         rmm::cuda_stream_view stream,
                                                         rmm::device_async_resource_ref mr,
                                                         std::string* error_out)
...
std::unique_ptr<cudf::column> rows_decode_str_k6(PlanTree const& tree,
                                                 sirius::codegen::chunk_row_set const& rows,
                                                 rmm::cuda_stream_view stream,
                                                 rmm::device_async_resource_ref mr,
                                                 std::string* error_out)
...
std::unique_ptr<cudf::column> decompress_column_rows(PlanTree const& tree,
                                                     sirius::codegen::chunk_row_set const& rows,
                                                     rmm::cuda_stream_view stream,
                                                     rmm::device_async_resource_ref mr,
                                                     std::string* error_out)
{
  ...
  // Route on the same one-probe classifier the masked decode uses
  // (probe_column), so a plan's sparse route can never drift from its masked
  // route: bitpack/delta roots take the composable fused kernel, dict and
  // str_split their dedicated fast paths, and `full` (no compacted decode)
  // reports the full-decode + gather fallback to the caller.
  switch (probe_column(tree).compact_route) {
    case sirius::codegen::decode_route::bitpack_mask:
    case sirius::codegen::decode_route::delta_mask:
      return rows_decode_fused_root(tree, rows, stream, mr, error_out);
    case sirius::codegen::decode_route::dict_codes: {
      auto col = rows_decode_dict_fast_path(tree, rows, stream, mr, error_out);
      if (!col && error_out && error_out->empty()) {
        *error_out =
          "decompress_rows: dict plan outside the sparse fast path "
          "(compressed/variable-width keys) — fall back to the mask route";
      }
      return col;
    }
    case sirius::codegen::decode_route::str_split:
      return rows_decode_str_k6(tree, rows, stream, mr, error_out);
    case sirius::codegen::decode_route::full:
    default:
      if (error_out) {
        *error_out =
          "decompress_rows: plan has no compacted decode route — "
