...
void lz4_compressed_representation::decompress(decode_frame& frame, decode_column_slot output) const
{
  detail::nvcomp_decompress_impl(
    lz4_ops(), payload_data(), payload_size(), original_type, num_rows, frame, output);
}
