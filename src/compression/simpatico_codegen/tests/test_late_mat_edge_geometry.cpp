...
std::vector<std::pair<std::string, std::vector<std::uint32_t>>> edge_selections(std::int64_t n)
{
  ...
  out.emplace_back("last_row", std::vector<std::uint32_t>{static_cast<std::uint32_t>(n - 1)});
  {
    ...
    for (std::int64_t i = 0; i < n; ++i)
      full[static_cast<std::size_t>(i)] = static_cast<std::uint32_t>(i);
  ...
  }
  {
    ...
    for (std::int64_t i = 0; i < n; i += 2)
      parity.push_back(static_cast<std::uint32_t>(i));
...
bool run_geometry_csr(std::int64_t n)
{
  ...
  for (auto const& [name, ids] : edge_selections(n)) {
    ...
    if (!ids.empty()) {
      d_ids = reinterpret_cast<const std::uint32_t*>(enc.upload_bytes(ids.data(), ids.size() * 4));
    }
    ...
    try {
      ...
      REQUIRE_MSG(false,
                  "[n=%lld/%s] bucket_sorted_local_ids threw: %s",
                  static_cast<long long>(n),
                  name.c_str(),
                  e.what());
    }
    REQUIRE_MSG(built.num_rows == n,
                "[n=%lld/%s] num_rows not preserved",
                static_cast<long long>(n),
                name.c_str());
    ...
    if (!ids.empty()) {
      ...
      REQUIRE_MSG(got_chunks == ref.chunks && got_offsets == ref.offsets && got_in == ref.in_chunk,
                  "[n=%lld/%s] device CSR != host CSR",
                  static_cast<long long>(n),
                  name.c_str());
      ...
      cudaMemcpy(got_exp.data(),
                 reinterpret_cast<const void*>(d_exp),
                 ids.size() * 4,
                 cudaMemcpyDeviceToHost);
      bool exp_ok = true;
      for (std::size_t i = 0; i < ids.size(); ++i)
      ...
      REQUIRE_MSG(
        exp_ok, "[n=%lld/%s] int32 expansion mismatch", static_cast<long long>(n), name.c_str());
...
bool run_k8_edges(std::int64_t n, bool delta_root, int arch)
{
  ...
  GpuEncoded enc                  = codegen_test::gpu_encode_tree<Element>(
    *tree, delta_root ? "int64_t" : "int32_t", data.data(), n, arch);
  ...
  for (auto const& [name, ids] : edge_selections(n)) {
    ...
    const std::uint32_t* d_ids =
      reinterpret_cast<const std::uint32_t*>(enc.upload_bytes(ids.data(), ids.size() * 4));
    ...
    REQUIRE_MSG(
      simpatico::launch_decode_fused_tree_sparse_rows(
        *tree, enc.buffers, dtype, n, built.view(), reinterpret_cast<void*>(d_out), stream),
      "[n=%lld/%s/%s] sparse_rows launch failed",
      static_cast<long long>(n),
    ...
    REQUIRE_MSG(got == expect,
                "[n=%lld/%s/%s] K8 output != host gather (%zu rows)",
                static_cast<long long>(n),
                delta_root ? "delta" : "bitpack",
                name.c_str(),
                ids.size());
...
bool run_batch_split()
{
  ...
  auto starts = sirius::codegen::split_sorted_ids_by_batch(static_cast<std::uint64_t const*>(d_ids),
  ...
  // Empty id list.
  auto empty_starts =
    sirius::codegen::split_sorted_ids_by_batch(static_cast<std::uint64_t const*>(d_ids),
                                               0,
                                               /*count_dev=*/nullptr,
                                               row_start,
                                               /*count_out=*/nullptr,
                                               stream,
                                               mr);
  REQUIRE_MSG(!empty_starts.empty() && empty_starts.back() == 0, "empty split must end at 0");
  ...
  starts = sirius::codegen::split_sorted_ids_by_batch(static_cast<std::uint64_t const*>(d_ids),
                                                      3,
                                                      /*count_dev=*/nullptr,
                                                      row_start,
                                                      /*count_out=*/nullptr,
  ...
  sirius::codegen::global_slice_to_local(static_cast<std::uint64_t const*>(d_ids),
                                         ...
                                         3048,
                                         static_cast<std::uint32_t*>(d_local),
                                         stream);
  cudaStreamSynchronize(stream.value());
  std::vector<std::uint32_t> got(g.size());
  cudaMemcpy(got.data(), d_local, got.size() * 4, cudaMemcpyDeviceToHost);
  REQUIRE_MSG((got == std::vector<std::uint32_t>{0, 1, 1024}), "global_slice_to_local mismatch");
  cudaFree(d_ids);
  cudaFree(d_local);
...
int main()
{
  ...
  try {
    for (std::int64_t n : {std::int64_t{1023},
                           std::int64_t{1024},
                           std::int64_t{1025},
                           std::int64_t{2048},
    ...
    for (std::int64_t n :
         {std::int64_t{1023}, std::int64_t{1024}, std::int64_t{1025}, std::int64_t{7 * 1024 + 1}}) {
