...
WRONG_VALUES = {
    ...
    "q14",
    "q19",
}  # FP64 decimal defect; marked with a dagger
ARMS = (2, 4, 8)
...
core = [
    q
    for q in sorted(set(data[4]) & set(data[8]))
...
for q in core:
    t = {
        n: (
            warm_median(data[n][q]) / 1000.0
            if q in data[n] and passed(data[n][q])
            else None
        )
        for n in ARMS
    }
    rows.append(
        {"q": q, "t": t, "s48": t[4] / t[8], "s24": (t[2] / t[4]) if t[2] else None}
    )
...
geo24 = math.exp(sum(math.log(r["s24"]) for r in have2) / len(have2)) if have2 else None
tot = {n: sum(r["t"][n] for r in rows if r["t"][n]) for n in ARMS}

fig, (axA, axB) = plt.subplots(
    1,
    2,
    figsize=(14.0, 0.50 * len(rows) + 3.6),
...
for i, r in enumerate(rows):
    ...
    for val, n in pts:
        axA.plot(
            val, i, "o", ms=9.5, color=RAMP[n], zorder=2 + n, mec=SURFACE, mew=2
...
axB.legend(
    handles=[
        Patch(facecolor=RAMP[4], label="2 → 4 GPUs"),
...
# ---- titles & provenance -----------------------------------------------------------
fig.suptitle(
    "Sirius GPU scale-out · TPC-H SF500 · 2 → 4 → 8 × A100 80 GB",
    fontsize=15.5,
    color=INK,
    x=0.052,
    ha="left",
...
sub1 = (
    f"{len(rows)} queries · 4→8 GPUs {geo48:.2f}× geomean "
    f"({geo48 / 2 * 100:.0f}% of linear)"
)
if geo24:
    sub1 += f" · 2→4 GPUs {geo24:.2f}× over the {len(have2)} that run at 2 GPUs"
sub2 = (
    "one memory config across all arms (16 GiB arena / 62 GiB pool) so the arms are "
