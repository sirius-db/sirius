...
**Treat the formula as retired.** STATUS.md (2026-08-20) showed arena occupancy is a pool-pressure
gauge, not independent demand — do not size from `96 GiB × SF/500 / N`. Use a measured config for
this box (`STATUS.md`, `CONFIGURATIONS.md`, `cn-tuning` skill). Left below as the A100 campaign's
original floor, which underestimated even then (SF100, N=2: formula 12 GiB, 16 GiB exhausted).

```
STAGING(N, SF) ≈ 96 GiB × (SF / 500) / N     rounded up to the next 4 GiB   # RETIRED
```
