#!/usr/bin/env bash
# Baseline TPC-H SF1000 arm: input tables pinned to the GPU tier, spill
# compression OFF. Run from the repo root:
#   pixi run bash bench/compress-spills-v2/run-baseline.sh
#
# Pairs with run-spill-compress.sh. The only difference between the two arms is
# the config file; both run at memory.gpu.usage_limit_fraction 0.908, so the
# query pool is the same size in each and the 4 GiB compression arena in the
# other arm is a partition of the device rather than extra memory.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"

DATA="${DATA:-/mnt/datasets/tpch/sf1000}"
CFG="${CFG:-$HERE/sirius-rtx6000.yaml}"
PLANS="${PLANS:-$REPO/src/compression/simpatico_codegen/plans/tpch_sf1000}"
NAME="${NAME:-baseline_no_spill_compress}"
...
# debug level is what emits the downgrade_executor "request done: ... to_disk:"
# summaries the spill analysis reads. Both arms run at the same level so the
# logging overhead cancels in the comparison.
export SIRIUS_LOG_LEVEL="${SIRIUS_LOG_LEVEL:-debug}"

# Decompression pushdown: fused filter inside the compressed-scan decode.
export SIRIUS_EXP_FUSED_SCAN_FILTER=1

# cuDF's JIT expression evaluator rather than the interpreted AST walker.
