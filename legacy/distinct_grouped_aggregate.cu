...
__global__ void distinct_bound(T* result, sort_keys_type* sort_keys, uint64_t N)
{
...
struct CustomSum {
  __device__ uint64_t operator()(const uint64_t& lhs, const uint64_t& rhs) { return lhs + rhs; }
};

template __global__ void distinct_bound<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  uint64_t* result, sort_keys_type* sort_keys, uint64_t N);
...
template __global__ void new_modify<BLOCK_THREADS, ITEMS_PER_THREAD>(sort_keys_type* sort_keys,
                                                                     uint64_t N,
                                                                     uint64_t new_num_keys);

template <typename T, typename V>
void groupedDistinctAggregate(uint8_t** keys,
                              ...
                              int* distinct_mode)
{
  ...
  cudaMemcpy(
    keys_dev, keys_row_id, (num_keys + num_aggregates) * sizeof(T*), cudaMemcpyHostToDevice);

  int tile_items = BLOCK_THREADS * ITEMS_PER_THREAD;
  columns_to_rows<T, BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      keys_dev, row_keys, materialized_temp, N, num_keys + num_aggregates);
  CHECK_ERROR();
  ...
  cub::DeviceMergeSort::SortKeys(
    d_temp_storage, temp_storage_bytes, materialized_temp, N, custom_op);
  ...
  for (int agg = 0; agg < num_aggregates; agg++) {
    ...
    if (distinct_mode[agg] == 0) {  // count distinct
      ...
      distinct_bound<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>
        <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
          distinct_boundary, materialized_temp, N);

      CHECK_ERROR();

      new_modify<BLOCK_THREADS, ITEMS_PER_THREAD>
        <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(materialized_temp, N, num_keys);
  ...
  rows_to_columns<T, BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(h_count[0] + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      group_by_rows, keys_dev_result, h_count[0], num_keys);
  CHECK_ERROR();
...
template void groupedDistinctAggregate<uint64_t, uint64_t>(uint8_t** keys,
                                                           uint8_t** aggregate_keys,
                                                           uint64_t* count,
                                                           uint64_t N,
                                                           uint64_t num_keys,
