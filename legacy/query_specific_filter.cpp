...
bool GPUExpressionExecutor::HandlingSpecificFilter(GPUIntermediateRelation& input_relation,
                                                   ...
                                                   Expression& expression)
{
  ...
  if (expression.type == ExpressionType::CONJUNCTION_OR) {
    ...
    if (expr.children.size() == 3 && expr.children[2]->type == ExpressionType::CONJUNCTION_AND) {
      ...
      if (expr2.children.size() == 5 && expr2.children[0]->type == ExpressionType::COMPARE_EQUAL &&
          expr2.children[1]->type == ExpressionType::COMPARE_LESSTHANOREQUALTO &&
          expr2.children[2]->type == ExpressionType::COMPARE_GREATERTHANOREQUALTO &&
          expr2.children[3]->type == ExpressionType::COMPARE_LESSTHANOREQUALTO &&
          expr2.children[4]->type == ExpressionType::COMPARE_IN) {
        ...
        if (expr3.children.size() == 5) {
          ...
          // string t = "(((P_BRAND = 12) AND (L_QUANTITY <= 11) AND (P_SIZE <= 5) AND (P_CONTAINER
          // IN (0, 1, 4, 5))) OR ((P_BRAND = 23) AND (L_QUANTITY >= 10) AND (L_QUANTITY <= 20) AND
          // (P_SIZE <= 10) AND (P_CONTAINER IN (17, 18, 20, 21))) OR ((P_BRAND = 34) AND
          // (L_QUANTITY >= 20) AND (L_QUANTITY <= 30) AND (P_SIZE <= 15) AND (P_CONTAINER IN (8, 9,
          // 12, 13))))";
          string t =
            "(((P_BRAND = 12) AND (P_SIZE <= 5) AND (L_QUANTITY <= 11.0) AND (P_CONTAINER IN (0, "
            "1, 4, 5))) OR ((P_BRAND = 23) AND (P_SIZE <= 10) AND (L_QUANTITY >= 10.0) AND "
            "(L_QUANTITY <= 20.0) AND (P_CONTAINER IN (17, 18, 20, 21))) OR ((P_BRAND = 34) AND "
            "(P_SIZE <= 15) AND (L_QUANTITY >= 20.0) AND (L_QUANTITY <= 30.0) AND (P_CONTAINER IN "
            "(8, 9, 12, 13))))";
          if (!expression.ToString().compare(t)) {
            ...
            BoundComparisonExpression& second =
              expr2.children[1]->Cast<BoundComparisonExpression>();
            ...
            shared_ptr<GPUColumn> materialized_brand =
              HandleMaterializeExpression(input_relation.columns[p_brand],
                                          first.left->Cast<BoundReferenceExpression>(),
                                          gpuBufferManager);
            shared_ptr<GPUColumn> materialized_size =
              HandleMaterializeExpression(input_relation.columns[p_size],
                                          second.left->Cast<BoundReferenceExpression>(),
                                          gpuBufferManager);
            shared_ptr<GPUColumn> materialized_quantity =
              HandleMaterializeExpression(input_relation.columns[l_quantity],
                                          third.left->Cast<BoundReferenceExpression>(),
                                          gpuBufferManager);
            shared_ptr<GPUColumn> materialized_container =
              HandleMaterializeExpression(input_relation.columns[p_container],
                                          fourth.children[0]->Cast<BoundReferenceExpression>(),
                                          gpuBufferManager);
    ...
    // Q7 HACK!!!
    if (expr.children.size() == 2 && expr.type == ExpressionType::CONJUNCTION_OR &&
        expr.children[0]->type == ExpressionType::COMPARE_EQUAL &&
        expr.children[1]->type == ExpressionType::COMPARE_EQUAL) {
      ...
      val[0] = constant1;
      val[1] = constant2;
      shared_ptr<GPUColumn> materialized_nkey1 =
        HandleMaterializeExpression(input_relation.columns[n_nationkey1],
                                    first.left->Cast<BoundReferenceExpression>(),
                                    gpuBufferManager);
      shared_ptr<GPUColumn> materialized_nkey2 =
        HandleMaterializeExpression(input_relation.columns[n_nationkey2],
                                    second.left->Cast<BoundReferenceExpression>(),
                                    gpuBufferManager);
    ...
    // Q7 HACK!!!
    if (expr.children.size() == 2 && expr.children[0]->type == ExpressionType::CONJUNCTION_AND &&
        expr.children[1]->type == ExpressionType::CONJUNCTION_AND) {
      auto& expr2 = expr.children[0]->Cast<BoundConjunctionExpression>();
      if (expr2.children.size() == 2 && expr2.children[0]->type == ExpressionType::COMPARE_EQUAL &&
          expr2.children[1]->type == ExpressionType::COMPARE_EQUAL) {
        SIRIUS_LOG_DEBUG("Filter expression of Q7");
        string t =
          "(((N_NATIONKEY = 6) AND (N_NATIONKEY = 7)) OR ((N_NATIONKEY = 7) AND (N_NATIONKEY = "
          "6)))";
        if (!expression.ToString().compare(t)) {
          BoundComparisonExpression& first  = expr2.children[0]->Cast<BoundComparisonExpression>();
          auto n_nationkey1                 = first.left->Cast<BoundReferenceExpression>().index;
    ...
    auto& expr = expression.Cast<BoundConjunctionExpression>();
    // Q16 HACK!!!
    //  if (expr.children.size() == 3 && expr.children[0]->type == ExpressionType::COMPARE_NOTEQUAL
    //  && expr.children[1]->type == ExpressionType::CONJUNCTION_OR && expr.children[2]->type ==
    //  ExpressionType::COMPARE_IN) {
    //      SIRIUS_LOG_DEBUG("Filter expression of Q16");
    //      string t = "((P_BRAND != 45) AND ((P_TYPE < 65) OR (P_TYPE >= 70)) AND (P_SIZE IN (49,
    //      14, 23, 45, 19, 3, 36, 9)))"; if (!expression.ToString().compare(t)) {
    //          BoundComparisonExpression& first =
    //          expr.children[0]->Cast<BoundComparisonExpression>(); auto p_brand =
    //          first.left->Cast<BoundReferenceExpression>().index; BoundConjunctionExpression&
    //          second = expr.children[1]->Cast<BoundConjunctionExpression>();
    //          BoundComparisonExpression& second_temp =
    //          second.children[0]->Cast<BoundComparisonExpression>(); auto p_type =
    //          second_temp.left->Cast<BoundReferenceExpression>().index; BoundOperatorExpression&
    //          third = expr.children[2]->Cast<BoundOperatorExpression>(); auto p_size =
    //          third.children[0]->Cast<BoundReferenceExpression>().index;
    ...
    //         shared_ptr<GPUColumn> materialized_brand =
    //         HandleMaterializeExpression(input_relation.columns[p_brand],
    //         first.left->Cast<BoundReferenceExpression>(), gpuBufferManager);
    //         shared_ptr<GPUColumn> materialized_type =
    //         HandleMaterializeExpression(input_relation.columns[p_type],
    //         second_temp.left->Cast<BoundReferenceExpression>(), gpuBufferManager);
    //         shared_ptr<GPUColumn> materialized_size =
    //         HandleMaterializeExpression(input_relation.columns[p_size],
    //         third.children[0]->Cast<BoundReferenceExpression>(), gpuBufferManager);
    ...
    //         size_t size = materialized_brand->column_length;
    //         q16FilterExpression(a, b, c, p_brand_val, p_type_val[0], p_type_val[1], p_size_val,
    //         comparison_idx, count, size);
    //     }

    // }

    // Q16 HACK!!!
    if (expr.children.size() == 2 && expr.children[0]->type == ExpressionType::CONJUNCTION_OR &&
        expr.children[1]->type == ExpressionType::COMPARE_IN) {
      SIRIUS_LOG_DEBUG("Filter expression of Q16");
      string t =
        "(((P_TYPE < 65) OR (P_TYPE >= 70)) AND (P_SIZE IN (49, 14, 23, 45, 19, 3, 36, 9)))";
      if (!expression.ToString().compare(t)) {
        BoundConjunctionExpression& second = expr.children[0]->Cast<BoundConjunctionExpression>();
        BoundComparisonExpression& second_temp =
          second.children[0]->Cast<BoundComparisonExpression>();
        ...
        shared_ptr<GPUColumn> materialized_type =
          HandleMaterializeExpression(input_relation.columns[p_type],
                                      second_temp.left->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        shared_ptr<GPUColumn> materialized_size =
          HandleMaterializeExpression(input_relation.columns[p_size],
                                      third.children[0]->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        ...
        q16FilterExpression(
          a, b, p_type_val[0], p_type_val[1], p_size_val, comparison_idx, count, size);
      }
    }

    // Q12 HACK!!!
    if (expr.children.size() == 3 && expr.children[0]->type == ExpressionType::COMPARE_LESSTHAN &&
        expr.children[1]->type == ExpressionType::COMPARE_LESSTHAN &&
        expr.children[2]->type == ExpressionType::COMPARE_IN) {
      SIRIUS_LOG_DEBUG("Filter expression of Q12");
      string t =
        "((L_COMMITDATE < L_RECEIPTDATE) AND (L_SHIPDATE < L_COMMITDATE) AND (L_SHIPMODE IN (4, "
        "6)))";
      if (!expression.ToString().compare(t)) {
        ...
        shared_ptr<GPUColumn> materialized_commitdate =
          HandleMaterializeExpression(input_relation.columns[l_commitdate],
                                      first.left->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        shared_ptr<GPUColumn> materialized_receiptdate =
          HandleMaterializeExpression(input_relation.columns[l_receiptdate],
                                      first.right->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        shared_ptr<GPUColumn> materialized_shipdate =
          HandleMaterializeExpression(input_relation.columns[l_shipdate],
                                      second.left->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        shared_ptr<GPUColumn> materialized_shipmode =
          HandleMaterializeExpression(input_relation.columns[l_shipmode],
                                      third.children[0]->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        ...
        q12FilterExpression(
          a, b, c, d, l_shipmode_val[0], l_shipmode_val[1], comparison_idx, count, size);
      }
    }
  }
  ...
  if (expression.type == ExpressionType::COMPARE_EQUAL) {
    ...
    if (expr.left->type == ExpressionType::BOUND_FUNCTION &&
        expr.right->type == ExpressionType::VALUE_CONSTANT) {
        ...
        if (expr.left->type == ExpressionType::BOUND_FUNCTION) {
          ...
          if (function_expr.function.name.compare("%") == 0) {
            ...
            if (left_function_expr.function.name.compare("+") == 0) {
              ...
              shared_ptr<GPUColumn> materialized_type = HandleMaterializeExpression(
                input_relation.columns[p_type],
                left_function_expr.children[0]->Cast<BoundReferenceExpression>(),
                gpuBufferManager);
  ...
  if (expression.type == ExpressionType::COMPARE_IN) {
    ...
    if (in_expr.children.size() == 8 || in_expr.children.size() == 4) {
      ...
      if (!expression.ToString().compare(t)) {
        ...
        uint64_t length                    = 2;
        shared_ptr<GPUColumn> materialized_column =
          HandleMaterializeExpression(input_column, bound_ref, gpuBufferManager);
        ...
        count                  = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
        q22FilterExpression(
          a, offset, start_idx, length, c_phone_val_str, comparison_idx, count, size, 8);
      }
  ...
  if (count && comparison_idx) {
    ...
    HandleMaterializeRowIDs(
      input_relation, output_relation, count[0], comparison_idx, gpuBufferManager, true);
    return true;
  }
