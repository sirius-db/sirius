...
struct sort_keys_type_string {
  ...
  __host__ __device__ sort_keys_type_string(uint64_t* _keys, uint64_t _num_key)
    : keys(_keys), num_key(_num_key)
  {
  }
...
struct CustomLessString {
  __device__ bool operator()(const sort_keys_type_string& lhs, const sort_keys_type_string& rhs)
  {
    for (uint64_t i = 0; i < lhs.num_key; i++) {
      if (lhs.keys[i] != rhs.keys[i]) { return lhs.keys[i] < rhs.keys[i]; }
    }
    return true;
  }
};
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) {
      offset[tile_offset + threadIdx.x + ITEM * B] =
        sizeof(T) * (tile_offset + threadIdx.x + ITEM * B);
    }
  }
}

template <int B, int I>
__global__ void columns_to_rows_string(uint8_t** a,
                                       uint8_t* result,
                                       uint64_t** input_offset,
                                       uint64_t* key_length,
                                       sort_keys_type_string* temp,
                                       uint64_t N,
                                       uint64_t num_keys)
{
  uint64_t tile_size   = B * I;
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) {
      ...
      // copy the row ids
      memcpy(result + (offset * total_length_bytes) + ((meta_num_keys - 1) * sizeof(uint64_t)),
             a[num_keys - 1] + (offset * sizeof(uint64_t)),
             sizeof(uint64_t));
      temp[offset] = sort_keys_type_string(
        reinterpret_cast<uint64_t*>(&result[offset * total_length_bytes]), meta_num_keys);
    }
  }
}

template <int B, int I>
__global__ void compact_string_offset(uint64_t* group_idx,
...
template <int B, int I>
__global__ void rows_to_columns_string(uint64_t* group_idx,
                                       sort_keys_type_string* row_keys,
                                       uint8_t** col_keys,
                                       uint64_t** group_byte_offset,
                                       uint64_t* key_length,
                                       uint64_t N,
    ...
    if (threadIdx.x + ITEM * B < num_tile_items) {
      ...
      if (group_idx[offset] != group_idx[offset + 1]) {
        ...
        for (uint64_t key = 0; key < num_keys; key++) {
          ...
          uint64_t actual_key_length =
            group_byte_offset[key][offset + 1] - group_byte_offset[key][offset];
          uint8_t* ptr = reinterpret_cast<uint8_t*>(row_keys[out_idx].keys);
          memcpy(col_keys[key] + out_offset,
                 ptr + key_length_bytes,
                 actual_key_length * sizeof(uint8_t));
...
template <int B, int I>
__global__ void distinct_string(uint64_t* distinct_mark,
                                uint64_t* distinct_len,
                                uint64_t* len,
                                sort_keys_type_string* sort_keys,
                                uint64_t N)
{
  uint64_t tile_size   = B * I;
  uint64_t tile_offset = blockIdx.x * tile_size;
...
template <typename T, int B, int I>
__global__ void gather_and_modify(
  const T* a, T* result, sort_keys_type_string* sort_keys, uint64_t N, uint64_t meta_num_keys)
{
...
__global__ void gather(
  const T* a, T* result, sort_keys_type_string* sort_keys, uint64_t N, uint64_t num_keys)
{
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) { a[tile_offset + threadIdx.x + ITEM * B] = b; }
  }
}

template __global__ void gather_and_modify<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint64_t* a,
  uint64_t* result,
  sort_keys_type_string* sort_keys,
  uint64_t N,
  uint64_t meta_num_keys);
template __global__ void gather_and_modify<double, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const double* a,
  double* result,
  sort_keys_type_string* sort_keys,
  uint64_t N,
  uint64_t meta_num_keys);
template __global__ void gather<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  const uint64_t* a,
  uint64_t* result,
  sort_keys_type_string* sort_keys,
  uint64_t N,
...
template <typename V>
void groupedStringAggregate(uint8_t** keys,
                            ...
                            int* agg_mode)
{
  ...
  uint64_t* key_length = gpuBufferManager->customCudaMalloc<uint64_t>(
    num_keys, 0, 0);  // store the maximum length of each key
  ...
  for (int key = 0; key < num_keys; key++) {
    ...
    if (offset[key] == nullptr) {
      ...
      fill_offset<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>
        <<<(N + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD),
           BLOCK_THREADS>>>(offset[key], N + 1);
      CHECK_ERROR();
    }
    cudaMemcpy(original_bytes + key, offset[key] + N, sizeof(uint64_t), cudaMemcpyDeviceToHost);

    get_len<BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + BLOCK_THREADS * ITEMS_PER_THREAD - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD),
         BLOCK_THREADS>>>(offset[key], len[key], N);
    CHECK_ERROR();
    ...
    if (offset[key] == nullptr) {
      ...
      cub::DeviceReduce::Max(d_temp_storage, temp_storage_bytes, len[key], key_length + key, N);
  ...
  sort_keys_type_string* materialized_temp = reinterpret_cast<sort_keys_type_string*>(
  ...
  sequence<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(row_sequence, N);
  ...
  columns_to_rows_string<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      keys_dev, row_keys, offset_dev, key_length, materialized_temp, N, num_keys + 1);
  CHECK_ERROR();
  ...
  cub::DeviceMergeSort::SortKeys(
    d_temp_storage, temp_storage_bytes, materialized_temp, N, custom_less);
  ...
  for (uint64_t key = 0; key < num_keys; key++) {
    ...
    gather_and_modify<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
        len[key], temp, materialized_temp, N, meta_num_keys);
    CHECK_ERROR();
    distinct_string<BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
        distinct_bound, temp, temp, materialized_temp, N);
    CHECK_ERROR();
    // cub scan
    d_temp_storage     = nullptr;
    temp_storage_bytes = 0;
    cub::DeviceScan::ExclusiveSum(
      d_temp_storage, temp_storage_bytes, temp, group_byte_offset[key], N + 1);
    ...
    cudaMemcpy(
      d_num_bytes + key, group_byte_offset[key] + N, sizeof(uint64_t), cudaMemcpyDeviceToDevice);
  ...
  ;
  cudaMemcpy(
    group_byte_offset_dev, group_byte_offset, num_keys * sizeof(uint64_t*), cudaMemcpyHostToDevice);

  // cub scan
  d_temp_storage     = nullptr;
  temp_storage_bytes = 0;
  cub::DeviceScan::ExclusiveSum(
    d_temp_storage, temp_storage_bytes, distinct_bound, group_idx, N + 1);
  ...
  sort_keys_type_string* group_by_rows = reinterpret_cast<sort_keys_type_string*>(
    gpuBufferManager->customCudaMalloc<pointer_and_key>(N, 0, 0));
  ...
  for (int agg = 0; agg < num_aggregates; agg++) {
    ...
    if (agg_mode[agg] == 4 || agg_mode[agg] == 5) {  // count_star or count(null) or sum(null)
      ...
      if (agg_mode[agg] == 4) {
        fill_n<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>
          <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(aggregate_star_temp[agg], 1, N);
      } else if (agg_mode[agg] == 5) {
        cudaMemset(aggregate_star_temp[agg], 0, N * sizeof(double));
      }

      modify<BLOCK_THREADS, ITEMS_PER_THREAD>
        <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(materialized_temp, N, meta_num_keys);
      CHECK_ERROR();
      ...
      gather_and_modify<V, BLOCK_THREADS, ITEMS_PER_THREAD>
        <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
          temp, aggregate_keys_temp[agg], materialized_temp, N, meta_num_keys);
      CHECK_ERROR();
      ...
      if (agg_mode[agg] == 0) {
        ...
        divide<V, BLOCK_THREADS, ITEMS_PER_THREAD>
          <<<(count[0] + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
            agg_out, agg_star_out, output, count[0]);
  ...
  compact_string_offset<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<((N + 1) + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
  ...
  rows_to_columns_string<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
...
//     add_offset<<<((N_b + 1) + BLOCK_THREADS - 1)/(BLOCK_THREADS), BLOCK_THREADS>>>(offset_c +
//     N_a, offset_b, num_bytes_a, N_b + 1); CHECK_ERROR(); cudaDeviceSynchronize();
// }

template void groupedStringAggregate<double>(uint8_t** keys,
                                             uint8_t** aggregate_keys,
                                             uint64_t** offset,
                                             uint64_t* num_bytes,
                                             uint64_t* count,
...
template void groupedStringAggregate<uint64_t>(uint8_t** keys,
                                               uint8_t** aggregate_keys,
                                               uint64_t** offset,
                                               uint64_t* num_bytes,
                                               uint64_t* count,
