...
__global__ void hash_groupby_gmem(T** group_key,
                                  V** aggregate,
                                  ...
                                  bool need_count)
{
  ...
  int n_ht_column;
  if (need_count)
    n_ht_column = num_keys + num_aggregates + 1;
  else
    n_ht_column = num_keys + num_aggregates;
  ...
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) {
      ...
      while (!can_break) {
        ...
        if (atomic) {
          old = atomicCAS(&ht[slot * n_ht_column + n],
                          0xFFFFFFFFFFFFFFFF,
                          (unsigned long long)item);  // compete for atomic
          if (old != 0xFFFFFFFFFFFFFFFF)
            atomic = false;  // lose the atomic, no longer do the atomic for the next keys
          else
            atomic = true;
        } else {
        ...
        if ((atomic &&
             old == 0xFFFFFFFFFFFFFFFF)) {  // atomic succeeds, try atomic on the next keys
          if (n == num_keys - 1) {
            can_break        = true;
            final_slot[ITEM] = slot;
          } else
            n++;
        } else if (!atomic && old == 0xFFFFFFFFFFFFFFFF) {  // not the thread with the atomic and
                                                            // need to wait until the atomic is done
          continue;
...
template <typename T, typename V, int B, int I>
__global__ void hash_groupby_smem(T** group_key,
                                  V** aggregate,
                                  T* max,
                                  T* min,
                                  unsigned long long* ht,
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) {
      ...
      for (int n = 0; n < num_keys; n++) {
        uint64_t item = group_key[n][offset];
        atomicCAS(&local_ht[final_slot[ITEM] * n_ht_column + n],
                  0xFFFFFFFFFFFFFFFF,
...
#pragma unroll
  for (int ITEM = 0; ITEM < I; ++ITEM) {
    if (threadIdx.x + ITEM * B < num_tile_items) {
      ...
      if (need_count) {  // need count to count average
        atomicAdd(&local_ht[final_slot[ITEM] * n_ht_column + num_keys + num_aggregates],
                  1LLU);  // count
      }
    }
  }
  ...
  if (threadIdx.x < ht_len) {
    ...
    for (int n = 0; n < num_aggregates; n++) {
      ...
      if (agg_mode[n] == 0) {
        ...
        if (local_ht[threadIdx.x * n_ht_column + num_keys + n] != 0)
          atomicAdd(&ht[threadIdx.x * n_ht_column + num_keys + n],
                    local_ht[threadIdx.x * n_ht_column + num_keys + n]);  // count
      } else if (agg_mode[n] == 5) {
        // should be zero, so do nothing
      } else
        cudaAssert(false);
    ...
    if (need_count) {  // need count to count average
      atomicAdd(&ht[threadIdx.x * n_ht_column + num_keys + num_aggregates],
                local_ht[threadIdx.x * n_ht_column + num_keys + num_aggregates]);  // count
    }
  }
}

template <typename T, typename V, int B, int I>
__global__ void scan_hash_group(unsigned long long* ht,
                                uint8_t** group,
                                uint8_t** aggregate,
      ...
      if (selection_flags[ITEM]) {
        ...
        for (int n = 0; n < num_aggregates; n++) {
          ...
          if (agg_mode[n] == 0 || agg_mode[n] == 2 || agg_mode[n] == 3) {
            ...
            uint64_t* ptr_cnt =
              reinterpret_cast<uint64_t*>(ht + (slot * n_ht_column + num_keys + num_aggregates));
            aggregate_ptr[offset] = ptr[0] / ptr_cnt[0];
          } else if (agg_mode[n] == 4) {
            uint64_t* aggregate_cnt_ptr = reinterpret_cast<uint64_t*>(aggregate[n]);
            uint64_t* ptr_cnt =
              reinterpret_cast<uint64_t*>(ht + (slot * n_ht_column + num_keys + n));
...
template <typename V>
__global__ void set_group_ht(unsigned long long* ht,
                             V** aggregate,
                             uint64_t num_keys,
                             uint64_t num_aggregates,
                             uint64_t ht_len,
...
#pragma unroll
  ...
  if (threadIdx.x == 0) { res_atomic_max.fetch_max(block_reduce_max); }

  __syncthreads();
  T block_reduce_min = BlockReduce<T, BLOCK_THREADS, ITEMS_PER_THREAD>(local_min, (T*)buffer, 3);
  __syncthreads();

  if (threadIdx.x == 0) { res_atomic_min.fetch_min(block_reduce_min); }
}

template <typename T, typename V>
void hashGroupedAggregate(uint8_t** keys,
                          ...
                          int* agg_mode)
{
  ...
  cudaMemcpy(aggregate_keys_dev,
             reinterpret_cast<V*>(aggregate_keys),
             num_aggregates * sizeof(V*),
  ...
  for (int i = 0; i < num_keys; i++) {
    ...
    get_min_max<T, BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
        reinterpret_cast<T*>(keys[i]), max_key_ptr, min_key_ptr, N);
    CHECK_ERROR();
  }
  ...
  if (C < BLOCK_THREADS) {
    ...
    size_t smem_size;
    if (need_count) {
      ht = (unsigned long long*)gpuBufferManager->customCudaMalloc<uint64_t>(
        ht_len * (num_keys + num_aggregates + 1), 0, 0);
      smem_size = ht_len * (num_keys + num_aggregates + 1) * sizeof(unsigned long long);
    } else {
      ht = (unsigned long long*)gpuBufferManager->customCudaMalloc<uint64_t>(
        ht_len * (num_keys + num_aggregates), 0, 0);
      smem_size = ht_len * (num_keys + num_aggregates) * sizeof(unsigned long long);
    }
    set_group_ht<<<(ht_len + BLOCK_THREADS - 1) / BLOCK_THREADS, BLOCK_THREADS>>>(
      ht,
      aggregate_keys_dev,
      num_keys,
      num_aggregates,
      ...
      need_count);
    CHECK_ERROR();
    hash_groupby_smem<T, V, BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS, smem_size>>>(keys_dev,
                                                                        aggregate_keys_dev,
                                                                        max_key,
                                                                        min_key,
                                                                        ht,
    ...
    set_group_ht<<<(ht_len + BLOCK_THREADS - 1) / BLOCK_THREADS, BLOCK_THREADS>>>(
      ...
      need_count);
    CHECK_ERROR();
    hash_groupby_gmem<T, V, BLOCK_THREADS, ITEMS_PER_THREAD>
      <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(keys_dev,
                                                             aggregate_keys_dev,
                                                             ht,
                                                             ht_len,
                                                             N,
  ...
  uint8_t** aggregate_keys_dev_result =
    gpuBufferManager->customCudaMalloc<uint8_t*>(num_aggregates, 0, 0);
  uint64_t* d_count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
  cudaMemset(d_count, 0, sizeof(uint64_t));
  scan_hash_group<T, V, BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(ht_len + tile_items - 1) / tile_items, BLOCK_THREADS>>>(ht,
                                                                nullptr,
                                                                nullptr,
  ...
  for (int i = 0; i < num_aggregates; i++) {
    if (agg_mode[i] == 0 || agg_mode[i] == 1 || agg_mode[i] == 2 || agg_mode[i] == 3) {
      aggregate_keys_result[i] =
        gpuBufferManager->customCudaMalloc<uint8_t>(h_count[0] * sizeof(V), 0, 0);
    } else if (agg_mode[i] == 4 || agg_mode[i] == 5) {
      aggregate_keys_result[i] =
        gpuBufferManager->customCudaMalloc<uint8_t>(h_count[0] * sizeof(uint64_t), 0, 0);
    } else {
      assert(false);
    }
  }
  ...
  cudaMemcpy(aggregate_keys_dev_result,
             aggregate_keys_result,
             num_aggregates * sizeof(uint8_t*),
             cudaMemcpyHostToDevice);
  cudaMemset(d_count, 0, sizeof(uint64_t));
  CHECK_ERROR();
...
template void hashGroupedAggregate<uint64_t, uint64_t>(uint8_t** keys,
                                                       uint8_t** aggregate_keys,
                                                       uint64_t* count,
                                                       uint64_t N,
                                                       uint64_t num_keys,
...
template void hashGroupedAggregate<uint64_t, double>(uint8_t** keys,
                                                     uint8_t** aggregate_keys,
                                                     uint64_t* count,
                                                     uint64_t N,
                                                     uint64_t num_keys,
