...
template __global__ void print_gpu_column<uint64_t>(uint64_t* a, int32_t N);
template __global__ void print_gpu_column<double>(double* a, int32_t N);
template __global__ void print_gpu_column<int>(int* a, int32_t N);
template __global__ void print_gpu_column<float>(float* a, int32_t N);
template __global__ void print_gpu_column<uint8_t>(uint8_t* a, int32_t N);
...
int main()
{
  ...
  uint64_t* a =
    new uint64_t[25]{0, 0, 3, 1, 3, 2, 1, 1, 0, 1, 4, 3, 4, 2, 2, 3, 3, 2, 0, 4, 4, 2, 1, 4};
  uint64_t* b = new uint64_t[25]{16, 15, 5, 6,  1,  22, 8, 2,  17, 14, 3,  4, 7,
                                 11, 21, 9, 23, 19, 18, 0, 13, 10, 12, 24, 20};
  ...
  auto cudf_column_a   = cudf::column_view(
    cudf::data_type(cudf::type_id::UINT64), 25, reinterpret_cast<void*>(a_gpu), nullptr, 0);
  columns_cudf.push_back(cudf_column_a);

  auto cudf_column_b = cudf::column_view(
    cudf::data_type(cudf::type_id::UINT64), 25, reinterpret_cast<void*>(b_gpu), nullptr, 0);
  ...
  //         gpuBufferManager->customCudaMalloc<uint64_t>(projection[projection_idx]->column_length,
  //         0, false); uint8_t* temp_column =
  //         gpuBufferManager->customCudaMalloc<uint8_t>(projection[projection_idx]->data_wrapper.num_bytes,
  //         0, false); callCudaMemcpyDeviceToDevice<uint64_t>(temp_offset,
  //         projection[projection_idx]->data_wrapper.offset,
  //         projection[projection_idx]->column_length, 0);
  //         callCudaMemcpyDeviceToDevice<uint8_t>(temp_column,
  //         projection[projection_idx]->data_wrapper.data,
  //         projection[projection_idx]->data_wrapper.num_bytes, 0);
  //         projection_columns[projection_idx] = new
  //         GPUColumn(projection[projection_idx]->column_length,
  //         projection[projection_idx]->data_wrapper.type, temp_column, temp_offset,
  //         projection[projection_idx]->data_wrapper.num_bytes, true);
  //     } else {
  //         uint8_t* temp_column =
  //         gpuBufferManager->customCudaMalloc<uint8_t>(projection[projection_idx]->data_wrapper.num_bytes,
  //         0, false); callCudaMemcpyDeviceToDevice<uint8_t>(temp_column,
  //         projection[projection_idx]->data_wrapper.data,
  //         projection[projection_idx]->data_wrapper.num_bytes, 0);
  //         projection_columns[projection_idx] = new
  //         GPUColumn(projection[projection_idx]->column_length,
  //         projection[projection_idx]->data_wrapper.type, temp_column);
  //     }
  // }
