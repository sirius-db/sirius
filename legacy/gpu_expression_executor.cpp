...
template <typename T>
shared_ptr<GPUColumn> GPUExpressionExecutor::ResolveTypeBinaryConstantExpression(
  ...
  string function_name)
{
  ...
  shared_ptr<GPUColumn> result =
    make_shared_ptr<GPUColumn>(size, column->data_wrapper.type, reinterpret_cast<uint8_t*>(c));
  return result;
}

shared_ptr<GPUColumn> GPUExpressionExecutor::HandleBinaryConstantExpression(
  shared_ptr<GPUColumn> column,
  ...
  string function_name)
{
  switch (column->data_wrapper.type.id()) {
    case GPUColumnTypeId::INT32: {
      ...
      return ResolveTypeBinaryConstantExpression<int>(
        column, constant, gpuBufferManager, function_name);
    }
    case GPUColumnTypeId::INT64: {
      uint64_t constant = expr.value.GetValue<uint64_t>();
      return ResolveTypeBinaryConstantExpression<uint64_t>(
        column, constant, gpuBufferManager, function_name);
    }
    case GPUColumnTypeId::FLOAT32: {
      float constant = expr.value.GetValue<float>();
      return ResolveTypeBinaryConstantExpression<float>(
        column, constant, gpuBufferManager, function_name);
    }
    case GPUColumnTypeId::FLOAT64: {
      double constant = expr.value.GetValue<double>();
      return ResolveTypeBinaryConstantExpression<double>(
        column, constant, gpuBufferManager, function_name);
    }
    default:
      throw NotImplementedException(
        "Unsupported sirius column type in `HandleBinaryConstantExpression`: %d",
        static_cast<int>(column->data_wrapper.type.id()));
  }
}

template <typename T>
shared_ptr<GPUColumn> GPUExpressionExecutor::ResolveTypeBinaryExpression(
  shared_ptr<GPUColumn> column1,
  ...
  string function_name)
{
  ...
  shared_ptr<GPUColumn> result =
    make_shared_ptr<GPUColumn>(size, column1->data_wrapper.type, reinterpret_cast<uint8_t*>(c));
  return result;
}

shared_ptr<GPUColumn> GPUExpressionExecutor::HandleBinaryExpression(
  shared_ptr<GPUColumn> column1,
  shared_ptr<GPUColumn> column2,
  GPUBufferManager* gpuBufferManager,
  string function_name)
{
  switch (column1->data_wrapper.type.id()) {
    ...
    case GPUColumnTypeId::INT64:
      return ResolveTypeBinaryExpression<uint64_t>(
        column1, column2, gpuBufferManager, function_name);
    case GPUColumnTypeId::FLOAT32:
    ...
    default:
      throw NotImplementedException(
        "Unsupported sirius column type in `HandleBinaryExpression`: %d",
        static_cast<int>(column1->data_wrapper.type.id()));
  }
}

template <typename T>
void GPUExpressionExecutor::ResolveTypeComparisonConstantExpression(shared_ptr<GPUColumn> column,
                                                                    BoundConstantExpression& expr1,
                                                                    BoundConstantExpression& expr2,
                                                                    uint64_t*& count,
                                                                    uint64_t*& row_ids,
...
void GPUExpressionExecutor::HandleComparisonConstantExpression(shared_ptr<GPUColumn> column,
                                                               BoundConstantExpression& expr1,
                                                               BoundConstantExpression& expr2,
                                                               uint64_t*& count,
                                                               uint64_t*& row_ids,
  ...
  switch (column->data_wrapper.type.id()) {
    case GPUColumnTypeId::INT32:
      ResolveTypeComparisonConstantExpression<int>(
        column, expr1, expr2, count, row_ids, expression_type);
      break;
    case GPUColumnTypeId::INT64:
    ...
    case GPUColumnTypeId::BOOLEAN:
      ResolveTypeComparisonConstantExpression<uint8_t>(
        column, expr1, expr2, count, row_ids, expression_type);
      break;
    default:
      throw NotImplementedException(
...
template <typename T>
void GPUExpressionExecutor::ResolveTypeComparisonExpression(shared_ptr<GPUColumn> column1,
                                                            shared_ptr<GPUColumn> column2,
                                                            uint64_t*& count,
                                                            uint64_t*& row_ids,
                                                            ExpressionType expression_type)
...
void GPUExpressionExecutor::HandleComparisonExpression(shared_ptr<GPUColumn> column1,
                                                       shared_ptr<GPUColumn> column2,
                                                       uint64_t*& count,
                                                       uint64_t*& row_ids,
                                                       ExpressionType expression_type)
{
  switch (column1->data_wrapper.type.id()) {
    ...
    default:
      throw NotImplementedException(
        "Unsupported sirius column type in `HandleComparisonExpression`: %d",
        static_cast<int>(column1->data_wrapper.type.id()));
  }
}

shared_ptr<GPUColumn> GPUExpressionExecutor::HandleRoundExpression(shared_ptr<GPUColumn> column,
                                                                   int decimal_places)
{
...
void GPUExpressionExecutor::FilterRecursiveExpression(GPUIntermediateRelation& input_relation,
                                                      ...
                                                      int depth)
{
  ...
  switch (expr.expression_class) {
    case ExpressionClass::BOUND_BETWEEN: {
      ...
      // FilterRecursiveExpression(input_relation, output_relation, *(bound_between.input), depth +
      // 1); FilterRecursiveExpression(input_relation, output_relation, *(bound_between.lower),
      // depth + 1); FilterRecursiveExpression(input_relation, output_relation,
      // *(bound_between.upper), depth + 1);
      ...
      shared_ptr<GPUColumn> materialized_column = HandleMaterializeExpression(
        input_relation.columns[bound_ref.index], bound_ref, gpuBufferManager);
      // count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
      HandleComparisonConstantExpression(
        materialized_column, bound_lower, bound_upper, count, comparison_idx, bound_between.type);
    ...
    case ExpressionClass::BOUND_REF: {
      ...
      output_relation.columns[bound_ref.index]->row_ids =
        gpuBufferManager->customCudaHostAlloc<uint64_t>(1);
    ...
    case ExpressionClass::BOUND_CASE: {
      ...
      for (idx_t i = 0; i < bound_case.case_checks.size(); i++) {
        FilterRecursiveExpression(
          input_relation, output_relation, *(bound_case.case_checks[i].when_expr), depth + 1);
        FilterRecursiveExpression(
          input_relation, output_relation, *(bound_case.case_checks[i].then_expr), depth + 1);
      }
      FilterRecursiveExpression(
        input_relation, output_relation, *(bound_case.else_expr), depth + 1);
      break;
    }
    ...
    case ExpressionClass::BOUND_COMPARISON: {
      ...
      // FilterRecursiveExpression(input_relation, output_relation, *(bound_comparison.left), depth
      // + 1); FilterRecursiveExpression(input_relation, output_relation, *(bound_comparison.right),
      // depth + 1);
      ...
      if (bound_comparison.right->expression_class == ExpressionClass::BOUND_CONSTANT) {
        ...
        if (input_relation.columns[bound_ref1.index]->data_wrapper.data == nullptr) {
          ...
          shared_ptr<GPUColumn> materialized_column = HandleMaterializeExpression(
            input_relation.columns[bound_ref1.index], bound_ref1, gpuBufferManager);
          // count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
          HandleComparisonConstantExpression(materialized_column,
                                             bound_ref2,
                                             bound_ref2,
                                             count,
                                             comparison_idx,
                                             bound_comparison.type);
          // if (count[0] == 0) throw NotImplementedException("No match found");
        }
        ...
        if (input_relation.columns[bound_ref1.index]->data_wrapper.data == nullptr ||
            input_relation.columns[bound_ref2.index]->data_wrapper.data == nullptr) {
          SIRIUS_LOG_DEBUG("Column is null");
          count    = gpuBufferManager->customCudaHostAlloc<uint64_t>(1);
          count[0] = 0;
        } else {
          shared_ptr<GPUColumn> materialized_column1 = HandleMaterializeExpression(
            input_relation.columns[bound_ref1.index], bound_ref1, gpuBufferManager);
          shared_ptr<GPUColumn> materialized_column2 = HandleMaterializeExpression(
            input_relation.columns[bound_ref2.index], bound_ref2, gpuBufferManager);
          // count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
          HandleComparisonExpression(materialized_column1,
                                     materialized_column2,
                                     count,
                                     comparison_idx,
                                     bound_comparison.type);
          // if (count[0] == 0) throw NotImplementedException("No match found");
    ...
    case ExpressionClass::BOUND_OPERATOR: {
      ...
      if (bound_operator.type == ExpressionType::OPERATOR_NOT) {
        ...
        } else if (bound_operator.children[0]->expression_class ==
...
void GPUExpressionExecutor::ProjectionRecursiveExpression(GPUIntermediateRelation& input_relation,
                                                          GPUIntermediateRelation& output_relation,
                                                          Expression& expr,
                                                          int output_idx,
                                                          int depth)
{
  bool is_specific_projection =
    HandlingSpecificProjection(input_relation, output_relation, expr, output_idx);
  ...
  switch (expr.expression_class) {
    case ExpressionClass::BOUND_BETWEEN: {
      ...
      auto& bound_between = expr.Cast<BoundBetweenExpression>();
      SIRIUS_LOG_DEBUG("Executing between expression");
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_between.input), output_idx, depth + 1);
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_between.lower), output_idx, depth + 1);
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_between.upper), output_idx, depth + 1);
      break;
    }
    case ExpressionClass::BOUND_REF: {
      auto& bound_ref = expr.Cast<BoundReferenceExpression>();
      ...
      SIRIUS_LOG_DEBUG("Reading column index {}", bound_ref.index);
      input_relation.checkLateMaterialization(bound_ref.index);
      if (depth == 0) { projected_columns.push_back(bound_ref.index); }
      break;
    }
    case ExpressionClass::BOUND_CASE: {
      throw NotImplementedException("Case expression not supported");
      ...
      SIRIUS_LOG_DEBUG("Executing case expression");
      for (idx_t i = 0; i < bound_case.case_checks.size(); i++) {
        ProjectionRecursiveExpression(input_relation,
                                      output_relation,
                                      *(bound_case.case_checks[i].when_expr),
                                      output_idx,
                                      depth + 1);
        ProjectionRecursiveExpression(input_relation,
                                      output_relation,
                                      *(bound_case.case_checks[i].then_expr),
                                      output_idx,
                                      depth + 1);
      }
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_case.else_expr), output_idx, depth + 1);
      break;
    }
    case ExpressionClass::BOUND_CAST: {
      throw NotImplementedException("Cast expression not supported");
      ...
      SIRIUS_LOG_DEBUG("Executing cast expression");
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_cast.child), output_idx, depth + 1);
      break;
    }
    case ExpressionClass::BOUND_COMPARISON: {
      throw NotImplementedException("Comparison expression not supported");
      ...
      SIRIUS_LOG_DEBUG("Executing comparison expression");
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_comparison.left), output_idx, depth + 1);
      ProjectionRecursiveExpression(
        input_relation, output_relation, *(bound_comparison.right), output_idx, depth + 1);
      break;
    }
    case ExpressionClass::BOUND_CONJUNCTION: {
      throw NotImplementedException("Conjunction expression not supported");
      ...
      for (auto& child : bound_conjunction.children) {
        ProjectionRecursiveExpression(
          input_relation, output_relation, *child, output_idx, depth + 1);
      }
      break;
    }
    case ExpressionClass::BOUND_CONSTANT: {
    ...
    case ExpressionClass::BOUND_FUNCTION: {
      ...
      if ((bound_function.ToString().find("substr") != std::string::npos) ||
          (bound_function.ToString().find("substring") != std::string::npos)) {
        ...
        shared_ptr<GPUColumn> materialized_column =
          HandleMaterializeExpression(input_column, bound_ref1, gpuBufferManager);
        ...
        shared_ptr<GPUColumn> materialized_column = HandleMaterializeExpression(
          input_relation.columns[bound_ref1.index], bound_ref1, gpuBufferManager);
        result = HandleBinaryConstantExpression(
          materialized_column, bound_ref2, gpuBufferManager, bound_function.function.name);
        ...
        shared_ptr<GPUColumn> materialized_column2 = HandleMaterializeExpression(
          input_relation.columns[bound_ref2.index], bound_ref2, gpuBufferManager);
        result = HandleBinaryExpression(materialized_column1,
                                        materialized_column2,
                                        gpuBufferManager,
                                        bound_function.function.name);
      } else {
  ...
  if (depth == 0) {
    if (expr.expression_class == ExpressionClass::BOUND_REF) {
      output_relation.columns[output_idx] =
        input_relation.columns[expr.Cast<BoundReferenceExpression>().index];
    } else {
      if (result) {
        ...
        output_relation.columns[output_idx] =
          make_shared_ptr<GPUColumn>(0, GPUColumnType(GPUColumnTypeId::INT32), nullptr);
      }
    }
  }
}
