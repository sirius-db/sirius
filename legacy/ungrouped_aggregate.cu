...
#pragma unroll
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if (threadIdx.x + ITEM * BLOCK_THREADS < num_tile_items) {
      if (op_mode == 0)
        local += a[tile_offset + threadIdx.x + ITEM * B];
      else if (op_mode == 1)
        local += a[tile_offset + threadIdx.x + ITEM * B];
      else if (op_mode == 2)
        local = max(local, a[tile_offset + threadIdx.x + ITEM * B]);
      else if (op_mode == 3)
        local = min(local, a[tile_offset + threadIdx.x + ITEM * B]);
      else
        cudaAssert(0);
    }
  }
...
template __global__ void ungrouped_aggregate<int, BLOCK_THREADS, ITEMS_PER_THREAD>(int* a,
                                                                                   int* result,
                                                                                   uint64_t N,
                                                                                   int op_mode);
template __global__ void ungrouped_aggregate<unsigned long long, BLOCK_THREADS, ITEMS_PER_THREAD>(
  unsigned long long* a, unsigned long long* result, uint64_t N, int op_mode);
template __global__ void ungrouped_aggregate<uint64_t, BLOCK_THREADS, ITEMS_PER_THREAD>(
  uint64_t* a, uint64_t* result, uint64_t N, int op_mode);
template __global__ void ungrouped_aggregate<double, BLOCK_THREADS, ITEMS_PER_THREAD>(
  double* a, double* result, uint64_t N, int op_mode);

// Define the host function that launches the CUDA kernel
template <typename T>
void ungroupedAggregate(
  uint8_t** a, uint8_t** result, uint64_t N, int* agg_mode, int num_aggregates)
{
  CHECK_ERROR();
  if (N == 0) {
  ...
  for (int agg = 0; agg < num_aggregates; agg++) {
    if (agg_mode[agg] == 4) {
      ...
      if (agg_mode[agg] == 0)
        cudaMemset(result_temp, 0, sizeof(T));
      else if (agg_mode[agg] == 1)
        cudaMemset(result_temp, 0, sizeof(T));
      else if (agg_mode[agg] == 2)
        cudaMemcpy(result_temp, a_temp, sizeof(T), cudaMemcpyDeviceToDevice);
      else if (agg_mode[agg] == 3)
        cudaMemcpy(result_temp, a_temp, sizeof(T), cudaMemcpyDeviceToDevice);

      ungrouped_aggregate<T, BLOCK_THREADS, ITEMS_PER_THREAD>
...
template void ungroupedAggregate<int>(
  uint8_t** a, uint8_t** result, uint64_t N, int* agg_mode, int num_aggregates);
template void ungroupedAggregate<uint64_t>(
  uint8_t** a, uint8_t** result, uint64_t N, int* agg_mode, int num_aggregates);
template void ungroupedAggregate<double>(
  uint8_t** a, uint8_t** result, uint64_t N, int* agg_mode, int num_aggregates);
