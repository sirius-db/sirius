...
bool GPUExpressionExecutor::HandlingSpecificProjection(GPUIntermediateRelation& input_relation,
                                                       ...
                                                       int output_idx)
{
  ...
  if (expression.type == ExpressionType::CASE_EXPR) {
    ...
    if (expr.case_checks[0].then_expr->type == ExpressionType::BOUND_REF) {
      ...
      auto materialized_nation =
        HandleMaterializeExpression(input_relation.columns[nation],
                                    when_expr.left->Cast<BoundReferenceExpression>(),
                                    gpuBufferManager);
      auto materialized_volume =
        HandleMaterializeExpression(input_relation.columns[volume], then_expr, gpuBufferManager);
      ...
      result = make_shared_ptr<GPUColumn>(
        size, GPUColumnType(GPUColumnTypeId::FLOAT64), reinterpret_cast<uint8_t*>(out));
      ...
      if (then_expr.function.name.compare("error") == 0) {
        // Q11 & Q22 HACK!!!
        // CASE  WHEN ((#1 > 1)) THEN (error('More than one row returned by a subquery used as an
        // expression - scalar subqueries can only return a single row. Use "SET
        // scalar_subquery_error_on_multiple_rows=false" to revert to previous behavior of returning
        // a random row.')) ELSE #0 END
        ...
        if (input_relation.columns[else_expr]->column_length != 1) {
          throw(
            "Error: More than one row returned by a subquery used as an expression - scalar "
            "subqueries can only return a single row.\n");
        }
        result = make_shared_ptr<GPUColumn>(input_relation.columns[else_expr]->column_length,
                                            input_relation.columns[else_expr]->data_wrapper.type,
                                            input_relation.columns[else_expr]->data_wrapper.data);
        ...
        auto materialized_type =
          HandleMaterializeExpression(input_relation.columns[p_type],
                                      when_expr.left->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        auto materialized_extendedprice =
          HandleMaterializeExpression(input_relation.columns[l_extendedprice],
                                      then_expr.children[0]->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
        auto materialized_discount =
          HandleMaterializeExpression(input_relation.columns[l_discount],
                                      substract_expr.children[1]->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
      ...
      if (when_expr.type == ExpressionType::CONJUNCTION_AND) {
        ...
        auto materialized_orderpriority =
          HandleMaterializeExpression(input_relation.columns[o_orderpriority],
                                      compare_expr.left->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
  ...
  if (expression.expression_class == ExpressionClass::BOUND_FUNCTION) {
    ...
    if (expr.children[0]->type == ExpressionType::OPERATOR_CAST) {
      ...
      auto materialized_date =
        HandleMaterializeExpression(input_relation.columns[date],
                                    cast_expr.child->Cast<BoundReferenceExpression>(),
                                    gpuBufferManager);
      ...
      result = make_shared_ptr<GPUColumn>(
        size, GPUColumnType(GPUColumnTypeId::INT64), reinterpret_cast<uint8_t*>(out));
      ...
      if (expr.children[0]->type == ExpressionType::BOUND_REF) {
        ...
        if (function_expr.function.name.compare("+") == 0) {
          ...
          auto materialize_4 = HandleMaterializeExpression(
            input_relation.columns[l_extendedprice_or_4], ref_expr, gpuBufferManager);
          auto materialize_tax =
            HandleMaterializeExpression(input_relation.columns[l_tax],
                                        function_expr.children[1]->Cast<BoundReferenceExpression>(),
                                        gpuBufferManager);
          ...
          auto materialize_extendedprice = HandleMaterializeExpression(
            input_relation.columns[l_extendedprice_or_4], ref_expr, gpuBufferManager);
          auto materialize_discount =
            HandleMaterializeExpression(input_relation.columns[l_discount],
                                        function_expr.children[1]->Cast<BoundReferenceExpression>(),
                                        gpuBufferManager);
        ...
        auto l_extendedprice =
          left_function_expr.children[0]->Cast<BoundReferenceExpression>().index;
        auto& left_right_function_expr =
          left_function_expr.children[1]->Cast<BoundFunctionExpression>();
        auto l_discount =
          left_right_function_expr.children[1]->Cast<BoundReferenceExpression>().index;

        auto& right_function_expr = expr.children[1]->Cast<BoundFunctionExpression>();
        auto ps_supplycost =
          right_function_expr.children[0]->Cast<BoundReferenceExpression>().index;
        auto l_quantity = right_function_expr.children[1]->Cast<BoundReferenceExpression>().index;

        auto materialize_extendedprice = HandleMaterializeExpression(
          input_relation.columns[l_extendedprice],
          left_function_expr.children[0]->Cast<BoundReferenceExpression>(),
          gpuBufferManager);
        auto materialize_discount = HandleMaterializeExpression(
          input_relation.columns[l_discount],
          left_right_function_expr.children[1]->Cast<BoundReferenceExpression>(),
          gpuBufferManager);
        auto materialize_supplycost = HandleMaterializeExpression(
          input_relation.columns[ps_supplycost],
          right_function_expr.children[0]->Cast<BoundReferenceExpression>(),
          gpuBufferManager);
        auto materialize_quantity = HandleMaterializeExpression(
          input_relation.columns[l_quantity],
          right_function_expr.children[1]->Cast<BoundReferenceExpression>(),
          gpuBufferManager);
      ...
      if (expr.children[0]->type == ExpressionType::BOUND_FUNCTION) {
        // Q14 HACK!!!
        //((sum(CASE  WHEN (((P_TYPE >= CAST(125 AS BIGINT)) AND (P_TYPE < CAST(150 AS BIGINT))))
        // THEN ((L_EXTENDEDPRICE * (CAST(1 AS DOUBLE) - L_DISCOUNT))) ELSE CAST(0.0 AS DOUBLE) END)
        //* 100.0) / sum((L_EXTENDEDPRICE * (CAST(1 AS DOUBLE) - L_DISCOUNT))))
        ...
        auto value =
          left_function_expr.children[1]->Cast<BoundConstantExpression>().value.GetValue<double>();
        auto sum = expr.children[1]->Cast<BoundReferenceExpression>().index;

        auto materialize_a = HandleMaterializeExpression(
          input_relation.columns[sum_case],
          left_function_expr.children[0]->Cast<BoundReferenceExpression>(),
          gpuBufferManager);
        auto materialize_b =
          HandleMaterializeExpression(input_relation.columns[sum],
                                      expr.children[1]->Cast<BoundReferenceExpression>(),
                                      gpuBufferManager);
