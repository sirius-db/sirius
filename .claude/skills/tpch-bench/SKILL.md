...
Runs from the repo root `/home/prestouser/aocsa/sirius` unless noted. This is the **single-host 4× GB200 ops** skill (StarRocks CN cluster + A-vs-B). Siblings: `build-sirius-starrocks` (fresh-box build), `tpch-cn-sweep` (portable 22-query sweep), `cn-tuning` (GPU_MEM/STAGING/operator budgets), `tpch-2host` (two physical GB200 hosts). The `benchmark` skill is Sirius-vs-DuckDB in-process and does not apply.

**Current refs**: [`notes/OPEN.md`](../../../notes/OPEN.md) — living work queue, read first. [`bench/gb200-4gpu/HARDWARE.md`](../../../bench/gb200-4gpu/HARDWARE.md) — box facts. `experimental/starrocks/benchmarks/8GPU-NVLINK-RUNBOOK.md` — multi-GPU bring-up, §5 data generation, §8 transport verification.
**History (do not copy numbers or commands):** [`notes/2026-08-09-gb200-sf100/`](../../../notes/2026-08-09-gb200-sf100/) (SF100 audit), [`notes/2026-08-07-sf1-l4/`](../../../notes/2026-08-07-sf1-l4/) (SF1 / 2-CN / L4). The pixi `cluster2`/`cluster` tasks are SF1 on one 23 GiB L4 with 2 CNs sharing a GPU.
...
**StarRocks submodule**: the gitlink is `14b7e3fa` (upstream `origin/branch-4.1.1`), and the three nixl proto RPCs are **not** in it — they arrive as a working-tree patch, so `git submodule update` alone leaves you with a checkout that cannot build the CN. Always follow it with `pixi run --manifest-path $SR/pixi.toml apply-starrocks-patches` (idempotent; re-run it after any `--force` update, which resets the checkout), then `fe-build` (Maven, ~40 min). The submodule is dirty by design; `.gitmodules` sets `ignore = dirty` so that dirt stays out of `git status` — **never `git add` the submodule after patching**, that re-records an unpushable local commit as the gitlink and breaks `submodule update` for everyone (it is what `04cd3136`, now gone, was). Run `git submodule update --init --recursive` at the repo root too — `substrait`, `vcpkg` and the root `duckdb/` are not auto-populated and the engine cmake fails with `No SOURCES given to target: sirius_extension`. Then `engine-build` + the cargo build above.
...
At SF100 the oracle is a multi-minute, tens-of-GB host-RAM job (seconds at SF1) — and it is the **only** correctness check in this entire workflow. Comparison bar: **row counts**, keys and ordering must be exact. On `x*(1-l_discount)` expressions, values drifted low by up to 0.39 % **at SF1** against a 0.25 % tolerance (the open decimal-lowering item, `notes/OPEN.md` #24; q03/q10 were its out-of-band `pass*` rows in the SF1 record — q10 does not even reach `pass` at SF100). The drift at 100x the rows has never been measured: establish the band from the oracle before accepting any value mismatch.
