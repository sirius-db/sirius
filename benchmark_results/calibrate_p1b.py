...
# Find K: smallest rows_per_group where P1b becomes slower (ratio > 1.0)
break_even_rpg = None
...
if break_even_rpg:
    K = break_even_rpg
    print(f"  → K = {K:,}  (P1b slower when rows/group > {K:,} and dedup_ratio=1.0)")
else:
    K = 1000  # safe default
...
N_LARGE = min(10_000_000, 50_000_000)  # use 10M to keep calibration fast
G_FIXED = 1_000
...
print(f"    - input_rows > {K} × estimated_output_groups  (large input)")
