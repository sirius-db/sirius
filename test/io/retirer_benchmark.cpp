...
struct fixture {
  fixture()
  {
    ...
    upstream = std::make_unique<cucascade::memory::numa_region_pinned_host_memory_resource>(0, true);
    pinned_mr =
...
int main(int argc, char** argv)
{
  ...
  // each other.  Anything above this is what retirement costs.
  {
    ...
    std::printf("DMA floor (copies only, no retirement): %.3f ms  %.2f GiB/s\n",
                a.wall_s * 1e3,
                a.gib_s);
  }
  ...
  if (sweep) {
    ...
    for (std::size_t n : {std::size_t{1}, std::size_t{2}, std::size_t{4}, std::size_t{8}}) {
      ...
      std::printf("  %zu stream%-2s %8.2f ms  %6.2f GiB/s\n",
                  n,
                  n == 1 ? " " : "s",
                  a.wall_s * 1e3,
                  a.gib_s);
  ...
  for (auto const& cfg : configs) {
    ...
    print_table(
      cfg.title, summarize("retirer", retirer_runs), summarize("events", event_runs));
  }
