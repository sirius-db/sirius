...
//===----------------------------------------------------------------------===//
// 2b. BUILD_PROBE probe split
//
// A BUILD_PROBE join folds its build into ONE hash table and streams the probe through it, so
// probe batches are mutually independent. Coalescing the whole probe into one batch would give the
// join exactly one task; set_probe_split_parts asks this CONCAT for N batches instead.
//===----------------------------------------------------------------------===//

TEST_CASE("sirius_physical_concat probe split groups pulls into ~parts batches",
          "[physical_concat][probe_split]")
{
  auto* space = get_shared_mem_space();
  REQUIRE(space != nullptr);

  // A budget far larger than the data: by default every batch coalesces into a single group (the
  // regression -- one probe batch, one join task, one thread).
  constexpr uint64_t threshold = 1024ull * 1024;  // 1 MB
  ...
  sirius_physical_concat concat_op(
    ...
    /*is_build=*/false,
    threshold);
  concat_op.operator_id = 100;  // the split path logs its decision, which needs an assigned id

  auto repo = std::make_unique<cucascade::shared_data_repository>();
  // 8 batches x 1000 int32 = 4000 bytes each, 32000 bytes total.
  ...
  port->type          = MemoryBarrierType::FULL;
  port->repo          = repo.get();
  port->src_pipeline  = nullptr;  // no source pipeline -> no further input can arrive
  port->dest_pipeline = nullptr;
  concat_op.add_port("input", std::move(port));

  // Baseline: without the split the whole side comes back in ONE pull.
  SECTION("without the split the probe coalesces into a single group")
  {
    auto result = concat_op.get_next_task_input_data();
    REQUIRE(result != nullptr);
    REQUIRE(dynamic_cast<const pipelineable_operator_data&>(*result).get_data_batches().size() ==
  ...
  SECTION("with the split the probe comes back as `parts` groups")
  {
    // 32000 total / 4 parts = 8000 bytes per group = 2 batches per group.
    concat_op.set_probe_split_parts(4, /*min_batch_bytes=*/1);

    std::vector<std::size_t> group_sizes;
    while (auto result = concat_op.get_next_task_input_data()) {
      group_sizes.push_back(
        dynamic_cast<const pipelineable_operator_data&>(*result).get_data_batches().size());
    }
    REQUIRE(group_sizes.size() == 4);
    std::size_t total = 0;
    for (auto s : group_sizes) {
      REQUIRE(s == 2);
      total += s;
    }
    REQUIRE(total == static_cast<std::size_t>(num_batches));  // nothing lost or duplicated
  }
}

TEST_CASE("sirius_physical_concat probe split slices one oversized batch into row-disjoint pieces",
          "[physical_concat][probe_split]")
{
  auto* space = get_shared_mem_space();
  REQUIRE(space != nullptr);

  // The case grouping alone cannot fix: the entire probe arrived as ONE upstream batch, so there
  // is nothing to group -- execute() must slice it.
  constexpr uint64_t threshold         = 1024ull * 1024;  // 1 MB
  constexpr std::size_t rows_per_batch = 1000;            // 4000 bytes
  ...
  sirius_physical_concat concat_op(
    ...
    /*is_build=*/false,
    threshold);
  concat_op.operator_id = 101;  // the split path logs its decision, which needs an assigned id
  ...
  // 4000 bytes / 4 parts = 1000 bytes per batch; the single 4000-byte batch exceeds it.
  concat_op.set_probe_split_parts(4, /*min_batch_bytes=*/1);

  auto pulled = concat_op.get_next_task_input_data();
  REQUIRE(pulled != nullptr);
  REQUIRE(dynamic_cast<const pipelineable_operator_data&>(*pulled).get_data_batches().size() == 1);

  auto outputs = concat_op.execute(*pulled, default_stream());
  auto const& out_batches =
    dynamic_cast<const pipelineable_operator_data&>(*outputs).get_data_batches();
  REQUIRE(out_batches.size() == 4);

  // Row-disjoint and order-preserving: concatenating the slices reproduces the input exactly, so
  // every probe row is joined exactly once (the per-probe-row soundness argument).
  std::vector<int32_t> seen;
  for (auto const& out : out_batches) {
    auto view = sirius::get_cudf_table_view(*out);
    REQUIRE(view.num_columns() == 1);
    auto host = copy_column_to_host<int32_t>(view.column(0));
    seen.insert(seen.end(), host.begin(), host.end());
  }
  REQUIRE(seen == values);
}

TEST_CASE("sirius_physical_concat probe split is refused on a build-side concat",
          "[physical_concat][probe_split]")
{
  auto* space = get_shared_mem_space();
  REQUIRE(space != nullptr);
  ...
  constexpr uint64_t threshold = 1024ull * 1024;  // 1 MB
  // LEFT + is_build=true -> _concat_all: the build MUST stay a single folded batch for BUILD_PROBE.
  auto fixture = create_test_hash_join(duckdb::JoinType::LEFT, {duckdb::LogicalType::INTEGER});
  sirius_physical_concat concat_op(
    sirius::from_duckdb_vec(duckdb::vector<duckdb::LogicalType>{duckdb::LogicalType::INTEGER}),
    1000,
  ...
  concat_op.set_probe_split_parts(4, /*min_batch_bytes=*/1);  // must be a no-op
