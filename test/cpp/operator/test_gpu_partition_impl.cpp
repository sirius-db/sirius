...
sirius::unique_ptr<data_batch_view> create_batch_with_random_data(
        const int num_rows, const sirius::vector<cudf::data_type>& column_types,
...
void copy_data_to_host_by_rows(cudf::table_view table, sirius::vector<sirius::vector<int64_t>>& h_rows) {
...
void validate_hash_partition(const sirius::data_batch_view& input_view,
    ...
    for (const auto& output_batch: output_batches) {
        output_table_views.push_back(output_batch->get_data()->cast<gpu_table_representation>().get_table().view());
    }
...
TEST_CASE("Hash partition basic", "[operator][hash_partition]") {
    ...
    auto output_batches = gpu_partition_impl::hash_partition(
        *input_view, partition_key_idx, num_partitions, cudf::get_default_stream(),
...
TEST_CASE("Hash partition with all the same partitioning keys", "[operator][hash_partition]") {
    ...
    sirius::vector<int> partition_key_idx = {0, 1};
    sirius::vector<std::optional<std::pair<int, int>>> ranges = {
        std::optional<std::pair<int, int>>({0, 0}), std::optional<std::pair<int, int>>({1, 1}),
