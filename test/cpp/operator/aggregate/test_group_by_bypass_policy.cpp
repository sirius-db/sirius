...
candidate_input good_candidate()
{
  ...
  in.downstream_materializes_copy = false;
          ...
          "[group_by_bypass][policy]")
{
  ...
  auto expect_auto_preserved = [](candidate_input in, decision_reason expected) {
    ...
    CHECK_FALSE(d.prototype_activated);
  };
...
TEST_CASE("group-by bypass model charges the terms it claims to", "[group_by_bypass][policy]")
{
  ...
  CHECK(base.additional_needed == base.concat_bytes + base.hash_set_bytes + base.gather_map_bytes +
                                    ...
                                    base.downstream_bytes + base.materialization_bytes);
  // v1 requires resident input, so there is nothing to materialize and the executor's own
  // materialization term stays the only source of that cost.
  CHECK(base.materialization_bytes == 0);
  ...
  SECTION("a materializing downstream is charged an extra retained copy")
  {
    auto projected                         = in;
    projected.downstream_materializes_copy = true;
    auto const with_copy                   = model_additional_bytes(projected);
    CHECK(with_copy.downstream_bytes == base.output_bytes);
    CHECK(with_copy.additional_needed == base.additional_needed + base.output_bytes);
  }
