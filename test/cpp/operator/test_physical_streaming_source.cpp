          ...
          "[streaming_source][pipeline_completion]")
{
  ...
  // Destroying the operator tears down its lifecycle (and any armed waker) with it; the
  // repository is independently owned and keeps the queued batch.
  op.reset();
  REQUIRE(repo->total_size() == 1);
}
          ...
          "[streaming_source][pipeline_completion]")
{
  ...
  // Starve: the hint parks and arms the one-shot waker.
  auto hint = op->get_next_task_hint();
  REQUIRE(hint.has_value());
  REQUIRE(hint->hint == TaskCreationHint::WAITING_FOR_INPUT_DATA);
  ...
  // One-shot: a second push without a new WAITING hint must not re-fire it.
  push_batch(*op, make_numeric_batch<int32_t>(*gpu_space, {2}, cudf::type_id::INT32));
  REQUIRE(creator.schedule_count(op.get()) == 1);

  // Drain, starve again, and the source re-arms.
