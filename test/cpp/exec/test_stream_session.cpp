          ...
          "[stream_session]")
{
  ...
  REQUIRE_FALSE(a.source->lifecycle().terminal());  // still waiting on sender 1
  REQUIRE_FALSE(b.source->lifecycle().terminal());  // untouched

  session.close_input(20, 0);
  REQUIRE(b.source->lifecycle().terminal());
  REQUIRE_FALSE(a.source->lifecycle().terminal());
}
