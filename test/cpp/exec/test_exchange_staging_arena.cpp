...
// ============================================================================
// ARENA-14: the FFI Context forwards lease/release and refuses an unset arena
// ============================================================================

TEST_CASE("ARENA-14: Context FFI throws when unset and leases when configured", "[staging_arena]")
{
  namespace fs = std::filesystem;
  auto const config =
    (fs::path(std::source_location::current().file_name()).parent_path().parent_path() / "scan" /
     "memory.yaml")
      .string();
  const auto* var = exchange_staging_arena::kCapacityEnvVar;
  struct restore_env {
    ...
    std::string old;
    bool had{false};
    explicit restore_env(const char* n) : name(n)
    {
  ...
  setenv(var, "4MiB", 1);
  auto ctx = sirius::ffi::make_context_from_config(config);
  REQUIRE(ctx->staging_capacity() == 4 * kMiB);
  REQUIRE(ctx->staging_base() != 0);
  auto offset = ctx->staging_lease(1024);
  REQUIRE(offset % exchange_staging_arena::kAlignment == 0);
  auto handle = ctx->staging_arena_handle();
  REQUIRE(handle != nullptr);
  REQUIRE(handle->outstanding() == 1);
  REQUIRE(handle->base() == ctx->staging_base());
  handle->release(offset);
