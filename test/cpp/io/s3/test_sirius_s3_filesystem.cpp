...
bool skip_if_no_s3_env(std::optional<s3_test_env> const& env)
{
  ...
  SUCCEED("SIRIUS_TEST_S3_* not set; skipping live S3 FileSystem test");
  return true;
}
...
class s3_filesystem_config_env_guard {
 public:
  explicit s3_filesystem_config_env_guard(s3_test_env const& env)
  {
    ...
    dir_              = fs::temp_directory_path() / ("sirius_s3_filesystem_" + unique);
  ...
  ~s3_filesystem_config_env_guard()
  {
...
class s3_filesystem_fixture {
 public:
  explicit s3_filesystem_fixture(s3_test_env const& env) : config_env(env), db(nullptr), con(db)
  {
  ...
  s3_filesystem_config_env_guard config_env;
  duckdb::DuckDB db;
...
bool extension_loaded(duckdb::Connection& con, std::string_view name)
{
  auto result = require_query_ok(con,
                                 "SELECT count(*) FROM duckdb_extensions() "
                                 "WHERE extension_name = " +
                                   sql_quote(name) + " AND loaded");
...
sirius::io::s3::s3_ioctx& require_s3_ioctx(s3_filesystem_fixture& fixture, std::string const& uri)
{
  auto sirius_ctx =
    fixture.con.context->registered_state->Get<duckdb::SiriusContext>("sirius_state");
  REQUIRE(sirius_ctx);
  // Resolve through create_datasource (the post-cleanup seam); the backend
...
TEST_CASE("sirius_s3_filesystem claims only valid S3 object paths", "[s3][filesystem]")
{
...
TEST_CASE("sirius_s3_filesystem rejects glob patterns but accepts exact keys", "[s3][filesystem]")
{
  sirius::io::s3::sirius_s3_filesystem fs;
          ...
          "[.][s3][integration][filesystem]")
{
  ...
  REQUIRE(size > 16);
  CHECK(static_cast<std::size_t>(size) == io_object->size());
  ...
  auto const direct_count =
    s3_ctx.host_read_io(*io_object, offset, direct_bytes.size(), direct_bytes.data());
          ...
          "[.][s3][integration][filesystem][sql]")
{
  ...
  auto const uri = s3_uri(env->bucket, "parquet/nation.parquet");
  auto result    = fixture.con.Query("SELECT count(*) FROM read_parquet(" + sql_quote(uri) + ")");
  REQUIRE(result);
          ...
          "[.][s3][integration][filesystem][sql][gpu_execution]")
{
  ...
  s3_filesystem_fixture fixture(*env);
  CHECK_FALSE(extension_loaded(fixture.con, "httpfs"));
  ...
  auto local_result =
    require_query_ok(local_con,
                     "SELECT n_nationkey, n_name, n_regionkey "
                     "FROM read_parquet(" +
                       sql_quote(local_parquet_path("nation").string()) + ") ORDER BY n_nationkey");
