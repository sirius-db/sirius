...
TEST_CASE("DISK->GPU round-trip conversion via converter registry", "[gpu_pipeline_disk]")
{
  ...
  // --- GPU -> DISK ---
  { auto mut = batch->to_mutable(); REQUIRE_NOTHROW(mut.convert_to<cucascade::disk_data_representation>(registry, disk_space, stream)); }
  ...
  { auto mut = batch->to_mutable(); REQUIRE_NOTHROW(mut.convert_to<cucascade::gpu_table_representation>(registry, gpu_space, stream)); }
...
TEST_CASE("DISK->GPU conversion preserves data correctness", "[gpu_pipeline_disk]")
{
  ...
  const size_t size_before = [&batch]() { auto ro = batch->to_read_only(); return ro.get_data()->get_size_in_bytes(); }();
  ...
  const size_t size_after = [&batch]() { auto ro = batch->to_read_only(); return ro.get_data()->get_size_in_bytes(); }();
