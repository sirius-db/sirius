...
/// Sets SIRIUS_DISABLE=1 for the fixture's parquet generation, then restores
/// whatever was there before — including the harness's own "1".
///
...
/// tests; and a failing REQUIRE throws, which would skip the unset entirely.
/// Restoring in a destructor fixes both.
...
class ParquetNullPredicateFixture : public sirius::test::GpuExecutionFixture {
 public:
  ParquetNullPredicateFixture()
  {
    ...
    dir_ = fs::temp_directory_path() / ("sirius_null_pred_" + std::to_string(::getpid()) + "_" +
                                        std::to_string(ctr.fetch_add(1)));
    fs::create_directories(dir_);
    auto const pq_path = (dir_ / "null_pred.parquet").string();
...
class ParquetHiveNullPredicateFixture : public sirius::test::GpuExecutionFixture {
 public:
  ...
  {
    static std::atomic<unsigned> ctr{0};
    dir_ = fs::temp_directory_path() / ("sirius_null_pred_hive_" + std::to_string(::getpid()) +
                                        "_" + std::to_string(ctr.fetch_add(1)));
    fs::create_directories(dir_);
    auto const root = (dir_ / "hive").string();
