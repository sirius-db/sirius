  ...
  std::shared_ptr<cucascade::data_batch> const& batch)
{
  ...
  batch->convert_to<cucascade::host_data_representation>(
    registry, host_space, cudf::get_default_stream());
...
{
  using column_metadata = sirius::column_metadata;
  ...
  cols.reserve(3);
  cols.push_back(int_builder.make_column_metadata(num_rows));
  cols.push_back(str_builder.make_column_metadata(num_rows));
  cols.push_back(big_builder.make_column_metadata(num_rows));
          ...
          "[memory][host_table_utils]")
{
  ...
  std::vector<column_metadata> cols;
  cols.reserve(2);
  cols.push_back(int_builder.make_column_metadata(rows_fit));
  cols.push_back(str_builder.make_column_metadata(rows_fit));
...
TEST_CASE("host_table_utils - metadata offsets match packed data", "[memory][host_table_utils]")
{
  ...
  auto const& columns = host_alloc->columns;
  REQUIRE(columns.size() == column_types.size());

  REQUIRE(columns[0].null_count == 0);
  REQUIRE(!columns[0].has_null_mask);
  REQUIRE(columns[1].null_count == static_cast<cudf::size_type>(int64_nulls.size()));
  REQUIRE(columns[1].has_null_mask);
  REQUIRE(columns[2].null_count == static_cast<cudf::size_type>(string_nulls.size()));
  REQUIRE(columns[2].has_null_mask);

  sirius::memory::multiple_blocks_allocation_accessor<int32_t> int32_accessor;
  int32_accessor.initialize(columns[0].data_offset, allocation);
  ...
  int64_accessor.initialize(columns[1].data_offset, allocation);
  ...
  if (!expected_int64_mask.empty()) {
    ...
    mask_accessor.initialize(columns[1].null_mask_offset, allocation);
  ...
  auto const& string_col = columns[2];
