...
{
  auto const pair = make_defer_pair(customer_schema(), {1, 2, 3}, origins(3));
  REQUIRE(pair.valid());
...
TEST_CASE("the restored types are the producer's original ones", "[late_mat][directive]")
{
  ...
  auto const pair = make_defer_pair(customer_schema(), {1, 3}, origins(2));
  REQUIRE(pair.valid());
...
{
  auto const pair = make_defer_pair(customer_schema(), {2, 3}, origins(2));
  REQUIRE(pair.valid());
...
{
  auto const pair = make_defer_pair(customer_schema(), {1, 2}, origins(2));
...
TEST_CASE("unbuildable requests produce no pair at all", "[late_mat][directive]")
{
  ...
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {}, {}).valid());
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {1, 2}, origins(1)).valid());
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {1, 99}, origins(2)).valid());
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {3, 1}, origins(2)).valid());  // unordered
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {1, 1}, origins(2)).valid());  // repeated
}
...
TEST_CASE("a pair with an empty origin is refused", "[late_mat][directive]")
{
  ...
  REQUIRE_FALSE(make_defer_pair(customer_schema(), {1, 2}, bad).valid());
}
