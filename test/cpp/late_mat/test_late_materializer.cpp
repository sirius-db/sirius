...
template <typename T>
std::unique_ptr<cudf::column> make_numeric_column_from(std::vector<T> const& host, cudf::type_id id)
{
...
std::vector<std::string> download_strings(cudf::column_view const& col)
{
  ...
  cudaMemcpy(offsets.data(),
             scv.offsets().data<cudf::size_type>(),
             offsets.size() * sizeof(cudf::size_type),
             cudaMemcpyDeviceToHost);
  std::vector<char> chars(static_cast<std::size_t>(offsets.back()));
  if (!chars.empty()) {
    cudaMemcpy(chars.data(), scv.chars_begin(test_stream()), chars.size(), cudaMemcpyDeviceToHost);
  }
...
TEST_CASE("late_mat pin_entry_handle fails closed across lifecycle", "[late_mat]")
{
  ...
  SECTION("wrong generation never resolves") { CHECK(handle.resolve(2) == nullptr); }
...
TEST_CASE("late_mat prepare_selection edge selections", "[late_mat]")
{
  ...
  SECTION(">2^31-1 ids refuse before any device work")
  {
    ...
    row_id_list ids{static_cast<std::uint64_t const*>(tiny.data()), (std::int64_t{1} << 31), false};
    CHECK_THROWS_AS(sirius::late_mat::prepare_selection(fx.layout, ids, stream, mr),
                    std::runtime_error);
  }
...
TEST_CASE("late_mat prepare_selection_from_batch forms", "[late_mat]")
{
  ...
  SECTION("dense with a filled range must agree with the layout")
  {
    ...
    CHECK_THROWS_AS(sirius::late_mat::prepare_selection_from_batch(fx.layout, sels, stream, mr),
                    std::runtime_error);
  }
  ...
  SECTION("mask form: hand-built wave-1 geometry over batch 0")
  {
    ...
    for (std::uint32_t r : {0u, 31u, 999u})
      words[r / 32] |= 1u << (r % 32);
    ...
    mask_sel.mask_words =
      std::make_shared<rmm::device_buffer>(words.data(), words.size() * 4, stream);
    mask_sel.chunk_offsets =
      std::make_shared<rmm::device_buffer>(choffs.data(), choffs.size() * 4, stream);
    ...
    cudaMemcpy(got.data(), sel->batches[0].local_indices.data(), 3 * 4, cudaMemcpyDeviceToHost);
  ...
  SECTION("id_list form: batch-local ascending int32")
  {
    ...
    idl.num_ids = 3;
    idl.row_ids = std::make_shared<rmm::device_buffer>(local.data(), local.size() * 4, stream);
    std::vector<row_selection> sels(3);
    sels[2] = idl;
    // Batches 0/1 dense.
    auto sel = sirius::late_mat::prepare_selection_from_batch(fx.layout, sels, stream, mr);
...
TEST_CASE("late_mat materialize uncompressed vs host reference", "[late_mat]")
{
  ...
  auto host_gather = [&](std::vector<std::uint64_t> const& host_ids) {
    ...
    for (auto id : host_ids)
      out.push_back(fx.host_values[static_cast<std::size_t>(id)]);
    return out;
  };
  ...
  {
    for (std::uint64_t id :
         {std::uint64_t{0}, std::uint64_t{999}, std::uint64_t{1000}, std::uint64_t{2024}}) {
...
TEST_CASE("late_mat materialize strings origin", "[late_mat]")
{
  ...
  for (auto id : ids)
    expect.push_back(all[static_cast<std::size_t>(id)]);
...
TEST_CASE("late_mat materialize refusal paths", "[late_mat]")
{
  ...
  SECTION("generation mismatch refuses (re-pin between prepare and materialize)")
  {
    ...
    CHECK_THROWS_AS(sirius::late_mat::materialize(stale, *sel, stream, mr), std::runtime_error);
  }
  SECTION("origin/selection batch count mismatch refuses")
  {
    pinned_column_view fewer = fx.view;
    fewer.batches.pop_back();
    CHECK_THROWS_AS(sirius::late_mat::materialize(fewer, *sel, stream, mr), std::runtime_error);
  }
  SECTION("origin batch row-count drift refuses")
  {
    pinned_column_view drift = fx.view;
    drift.batches[0].num_rows -= 1;
    CHECK_THROWS_AS(sirius::late_mat::materialize(drift, *sel, stream, mr), std::runtime_error);
  }
  SECTION("nullable source column refuses (v1 non-null contract)")
  {
    ...
    CHECK_THROWS_AS(sirius::late_mat::materialize(withnull, *sel, stream, mr), std::runtime_error);
  }
}
