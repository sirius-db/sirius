...
"""TPC-H query-stream orderings (spec Appendix A), used by tpch_power_throughput.py.

Stream 0 is the power-run order; streams 1..N are the throughput query streams.
Extracted from the reference generator so no qgen invocation is needed at run time:

    cd test_datasets/tpch-dbgen
    DSS_QUERY=queries ./qgen -c -p <stream> -s 1 | grep -oE '\\(Q[0-9]+\\)'
"""
...
# Spec Table 11 (clause 5.3.4), minimum query streams per scale factor. Running
# more than the minimum is allowed, running fewer is not, and SF100000 is the
# largest valid test database, so anything beyond the table must pass --streams.
MIN_STREAMS_BY_SF = (
    (1, 2),
    (10, 3),
    (30, 4),
...
def stream_order(stream: int) -> tuple:
    """Ordering for `stream`, wrapping past the 41 spec-defined streams."""
    return STREAM_PERMUTATIONS[stream % len(STREAM_PERMUTATIONS)]
...
def default_streams(sf: float) -> int:
    ...
    if sf > largest_sf:
        raise SystemExit(
            f"No default stream count for SF{sf:g}: SF{largest_sf} "
            f"({largest_streams} streams) is the largest valid scale factor. "
            "Pass --streams explicitly, since defaulting lower would run fewer "
            "streams than the minimum."
        )
    streams = 2
...
assert all(sorted(v) == list(range(1, 23)) for v in STREAM_PERMUTATIONS.values())
