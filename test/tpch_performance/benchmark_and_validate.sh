...
# True for any data source backed by a .duckdb file (the old CPU duckdb scan and the
# GPU-native duckdb scan share the same file, runner, and file-resolution machinery).
...
for engine in $ENGINES; do
    ...
    if is_duckdb_source; then
        ...
        # duckdb-native routes seq_scan to the GPU-native scan operator. The flag is
        # passed for both engines; run_tpch_duckdb.sh guards the SET to the sirius
        # engine, so the duckdb engine stays an unaffected CPU baseline for validation.
        if [ "$DATA_SOURCE" = "duckdb-native" ]; then
            EXTRA_ARGS+=(--gpu-native-scan)
        fi
