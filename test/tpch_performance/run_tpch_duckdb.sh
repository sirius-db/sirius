...
if [ $# -lt 3 ]; then
    ...
    echo "  --gpu-native-scan Re-assert GPU-native scan (redundant: it is the sirius default now)"
    exit 1
fi
...
if [ "$ENGINE" = "sirius" ] && [ "$GPU_NATIVE_SCAN" = true ]; then
    echo "GPU-native scan: enabled (SET enable_gpu_duckdb_native_scan = true)"
fi
if [ "$DROP_OS_CACHE" = true ]; then
...
run_single_session() {
    ...
    # The GPU-native DuckDB scan is the engine default, so the sirius engine already routes
    # seq_scan to it without any SET. SIRIUS_NATIVE_SCAN_VERIFY=1 raises the log level so the
    # native-scan markers appear; --gpu-native-scan re-asserts the default explicitly. Emitted
    # once before the first query marker so the SET Run Time stays outside every timing window.
    if [ "$ENGINE" = "sirius" ]; then
        if [ "${SIRIUS_NATIVE_SCAN_VERIFY:-0}" = 1 ]; then
            echo "SET sirius_log_level = 'debug';" >> "$TEMP_SQL"
        fi
        if [ "$GPU_NATIVE_SCAN" = true ]; then
            echo "SET enable_gpu_duckdb_native_scan = true;" >> "$TEMP_SQL"
        fi
    fi
    ...
    for q in "${VALID_QUERIES[@]}"; do
        ...
        TEMP_SQL=$(mktemp /tmp/tpch_q${q}_XXXXXX.sql)
        {
            ...
            if [ "$ENGINE" = "sirius" ]; then
                ...
                if [ "$GPU_NATIVE_SCAN" = true ]; then
                    printf "SET enable_gpu_duckdb_native_scan = true;\n"
                fi
            fi
            printf ".timer on\n"
            for ((iter = 0; iter < NUM_ITERATIONS; iter++)); do
