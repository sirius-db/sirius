...
```bash
# Full build (release preset, parallelism handled by ninja)
pixi run build

# Build a specific preset (debug, release, relwithdebinfo, clang-debug, clang-release, clang-relwithdebinfo)
pixi run build debug

# If build consumes too much memory, reduce parallelism
CMAKE_BUILD_PARALLEL_LEVEL=8 pixi run build

# After build errors, clean and rebuild
