...
- **Label-encoded group keys:** when a COLLECT_SET aggregation is present, the input is large (≥ 1M rows), the group key is multi-column and non-nested, and an HLL estimate puts group cardinality below 1% of rows, the key table is collapsed into a single dense INT32 label so cuDF's `stable_sorted_order` takes its single-column radix path; original keys are recovered by a gather at group cardinality. The label contract is `cudf::encode`'s; by default the labels are produced with `cudf::distinct` + `cudf::key_remapping` (byte-identical, faster), with `cudf::encode` itself as the fallback when the remap ids are not dense or when `enable_aggregate_label_remap` is off. This short-circuits the STRING dictionary-encode path and falls back silently to the plain multi-column sort on failure.
