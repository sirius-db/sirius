...
| `BUILD_PROBE` | Eligibility measured at one partition per GPU, per-partition build side (< `max_build_hash_table_bytes`) foldable to one batch. The INV-FOLD floor may raise the final count above `num_gpus`, in which case several partitions share a GPU | `cudf::hash_join`, `cudf::distinct_hash_join`, or `cudf::filtered_join` — built once per partition, probed many times |
...
- **Adaptive count:** `measure_input()` reports the resident input's bytes and its row count — exact on every tier but `cudf::pack`'d host batches, and flagged when it is not — the downstream consumer's `get_partition_strategy` turns both into N (`hash_partition_bytes` for the byte target, the INV-FOLD row floor for the folding side)
...
`k_fold_row_limit` is one below `cudf::size_type`'s maximum, not equal to it: `cudf::concatenate` caps rows at that maximum but also caps *offsets*, and a STRING or LIST column carries one offset more than it has rows. The limit is therefore what cuDF accepts for the widest column kind a fold may contain.

Under `_concat_batch_bytes` this holds incidentally, because that target is small. Under `_concat_all` nothing in the CONCAT can shrink a group — `refresh_cross_schedule` requires exactly one folded batch per partition — so the invariant rests entirely on the **partition count**. `compute_hash_join_partition_strategy` establishes it with `fold_partition_count`: one partition whenever the measured folding side already fits `max_fold_rows`, since that fold *is* the measurement and there is no distribution to be skewed, and only once a split is genuinely required is the count taken against the halved `fold_row_target` so hash skew has headroom. A build that cannot fit one fold is refused broadcast (a replica is not split, so raising the count cannot shrink it), and a MARK join in that position throws, its count being pinned at one partition or at broadcast.

The refusal and the throw both require the measurement to be **exact**. A best-effort figure — today only a `cudf::pack`'d host batch, or a batch that vanished mid-measurement — may raise the partition count but may never fail a query or withhold a broadcast. **Residual R5:** raising the count is itself how a single-partition dynamic filter is lost, so an over-estimated inexact bound can cost the filter. Gating the floor on exactness would be worse: an inexact bound cannot tell a real overflow from a phantom one, and an unfloored overflowing fold is a query failure rather than a lost optimization.

That floor bounds only the side the sizing PARTITION measured. A side that folds but is never measured — the probe of a right-family join under build-driven sizing (residual R1'), or a FULL OUTER join's probe (residual R3) — has no floor and is bounded solely by `check_fold_row_limit` inside `gpu_merge_impl::concat`, which refuses the fold before cuDF can. That refusal is a distinct exception type (`fold_row_limit_exceeded`), so `sirius_physical_concat::execute` can name the CONCAT, its partition, its side and the join it feeds and stamp the stable `[fold_limit]` marker on an INV-FOLD violation and on nothing else — a fold also fails on device OOM and on a batch that is not GPU-resident, and those are logged without the marker. Either way the exception rethrows into the normal CPU-fallback path. See `src/include/op/fold_limits.hpp` and the `max_concat_fold_rows` setting.

**Residual R4 — MERGE_GROUP_BY.** `gpu_merge_impl::merge_grouped_aggregate` and `merge_ungrouped_aggregate` concatenate a whole partition too, and carry the same guard, but no floor: `sirius_physical_grouped_aggregate_merge::get_partition_strategy` sizes by bytes alone and ignores the row measurement the PARTITION supplies. Reaching it needs more than 2^31 partial-group rows in one partition. Those two guards also run at the fixed cuDF limit rather than at `max_concat_fold_rows`, so unlike the CONCAT fold they are not reachable at test data volumes. Closing R4 properly means giving MERGE_GROUP_BY the same row-aware floor and the same configurable limit the hash join has.
