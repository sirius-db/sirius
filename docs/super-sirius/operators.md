...
**Broadcast small build tables.** On multi-GPU, when the build side is small (`< small_table_bytes`), instead of routing the whole build to one GPU the PARTITION operator proposes `num_gpus` partitions and *replicates* the small build table to every GPU (the `_broadcast` flag), so each GPU builds its own hash table and joins its local probe rows. The build sink deposits the build batch into every slot; the probe sink routes each batch to the slot for its current GPU (`slot_for_device`). Once the probe side finishes, slots that received build data but no probe rows are reclaimed (`reclaim_build_only_slots_if_probe_complete`, broadcast and non-broadcast alike): each is discarded, except that when the operator would otherwise finish with zero tasks ever issued, one slot is kept and issued as an orphan build task with a synthesized empty probe batch so a 0-row output batch flows (pure decision: `plan_build_probe_reclaim`). The pure candidate decision is `make_broadcast_partition_decision()`; right-family / mixed joins reject BUILD_PROBE and fall back to the normal partition count.
...
| MARK | build resident (via BUILD_PROBE; `_concat_all = false`) | probe (BUILD_PROBE) | n/a — forced BUILD_PROBE | ❌ clamped to 1 (single-GPU) | ✅ forced on multi-GPU | BUILD_PROBE (STANDARD only when the build side finishes empty) |
