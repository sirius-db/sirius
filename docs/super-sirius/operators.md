...
**Empty-opposite side (orphan tasks).** When one side is a genuinely empty table, its concat emits *zero* batches. The surviving side's batches then have nothing to pair with, so once both producers finish, `next_cross_schedule_orphan` emits each survivor as a single-side "orphan" task (`join_present_side` on `partitioned_operator_data`). `execute()` synthesizes an empty opposite batch (`make_empty_join_side_table` from the absent child's output types) and runs the normal join dispatch, so cuDF NULL-pads the survivor for LEFT/RIGHT/OUTER/ANTI and yields zero rows for INNER/SEMI. This both drains the repository (so the pipeline completes instead of hanging) and emits the correct rows. FULL OUTER and RIGHT-family joins are the reachable cases (they always run STANDARD); small-build INNER/LEFT/SEMI/ANTI take BUILD_PROBE, which handles an empty side separately.
