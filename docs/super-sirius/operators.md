...
- **Status and capacity:** experimental and disabled by default. The current FULL/FULL design drains both complete inputs in one task, so both inputs, the output, and workspace must fit the memory available to one GPU task. The default normal plan remains a partitioned HASH_JOIN plus HASH_GROUP_BY.
...
- **Configuration:** opt in at startup with the strict boolean YAML key `sirius.operator_params.enable_dense_count_join: true`. `dense_count_join_max_bytes` remains engine-owned and is rejected in YAML. Test builds can expose session overrides for both fields with `SIRIUS_ENABLE_TEST_OPTIONS=1`.
