...
`update_join_exec_mode()` selects BUILD_PROBE when `num_partitions <= num_gpus` (one hash table per partition, at most one partition per GPU — this reduces to the historical single-partition rule when `num_gpus == 1`), each partition's average build side fits `max_build_hash_table_bytes` and folds to a single batch, and the join is neither RIGHT-family (`RIGHT`, `RIGHT_SEMI`, `RIGHT_ANTI`) nor `MIXED_JOIN`. INNER, LEFT, OUTER, MARK, SEMI, and ANTI joins are all eligible (SEMI/ANTI/MARK build a persistent `cudf::filtered_join` on the right and stream left probe batches). The pure eligibility gate is `build_probe_mode_eligible()`.
