...
**Mechanism:** the key table collapses into one dense INT32 label; the groupby sorts on that label, and original keys are recovered with a gather at group cardinality. The label contract is `cudf::encode`'s: distinct key rows in sorted order, so label ordering equals lexicographic key ordering; NULL key tuples get their own label (`null_policy::INCLUDE`). By default the labels are produced by `compute_group_labels_via_remap` — `cudf::distinct` + a `cudf::key_remapping` hash probe with a group-cardinality lexicographic fixup — which is byte-identical to `cudf::encode` but replaces its per-input-row binary search with one hash probe per row (~5x faster label production). The function falls back internally to `cudf::encode` when the remap ids are not a dense 0..n-1 permutation, and `enable_aggregate_label_remap = false` restores `cudf::encode` outright. Gated on: a COLLECT_SET aggregation being present, ≥ 1M input rows, a multi-column non-nested key, and an HLL estimate putting group cardinality below 1% of rows (otherwise the internal distinct+sort costs as much as the sort it replaces). Falls back silently to the plain multi-column sort if encoding throws; short-circuits the STRING dictionary-encode path when active.

**Code path:** `src/op/aggregate/gpu_aggregate_impl.cpp` — `local_grouped_agg()`, label path (`use_label_keys`)

**Config:** `enable_aggregate_label_remap` (default true; thresholds hard-coded). TPC-H SF1000 GB300: q16 0.490 s → 0.298 s.
