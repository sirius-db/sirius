...
Multi-GPU parquet reads, local and `s3://`, stay **off kvikio**. The registry still carries a kvikio catch-all for paths no explicit backend claims; the config below keeps local parquet from falling through to it. The reasoning: cudf's bundled `file_source` factory uses kvikio, which binds the file handle to whichever CUDA context was active at construction time. In multi-GPU execution that's a hidden source of corruption — a file_handle bound to GPU 0 will silently funnel reads through GPU 0 even when the consumer is on GPU 1. `sirius_config::validate_sirius_datasource_for_multi_gpu()` therefore rejects `scan_manager_config::use_sirius_datasource = false` whenever more than one GPU memory space is configured. It does not silently change the configured backend.
...
| Multi-GPU parquet reads resolve to `sirius_datasource`, not kvikio | `sirius_config::validate_sirius_datasource_for_multi_gpu()` rejects `use_sirius_datasource=false` when >1 GPU is configured | Any file-path datasource via cudf silently uses kvikio, which binds to a single CUDA context |
