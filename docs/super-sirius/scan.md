...
2. **Reader-level filter pushdown:** the cuDF AST is set on `parquet_reader_options` via `set_filter()`, so cuDF applies the filter inside `read_parquet`. When the reader applies the row filter, `materialize_table` reports `ROW_FILTERED` (or `ROW_FILTERED_AND_PROJECTED` once hive partitions are assembled), so the scan operator skips a redundant post-decode filter. Runtime dynamic filters AND-merge into the same per-split `set_filter`; that merge — never the static predicate — is runtime-gated per scan by the reader pruning gate ([dynamic-filters-top-n.md](dynamic-filters-top-n.md)), which stops it on scans where it prunes no row groups.
