...
Before a query runs, `sirius_scan_manager::prepare_for_query` registers a per-query state entry (`query_scan_manager_state`, keyed by the query's id — concurrent queries prepare, run and tear down independently) and walks the plan's `GPU_SCAN` operators. For each it either (a) matches a pinned-table cache entry and serves the scan from cached batches, or (b) builds a `split_provider` over the operator's ingestible. A per-query sequencer (`load_balancing_scan_batch_coalescer`, one per state entry) drives metadata production, coalesces the output into right-sized data batches, balances each batch onto a GPU, and pushes the resulting splits onto each operator's `split_connector`.
...
`sirius_scan_manager` prepares scan-side state before a query runs and drives metadata production for every `GPU_SCAN` source. It owns a configurable thread pool shared by every query, a single `io::sirius_ioctx` (uring on the fast path, kvikio as the universal fallback — multi-GPU requires the uring path), an optional prefetching cache built on that ioctx, and the registry of pinned-table entries. Per-query scan state — the providers, the sequencer, and a `scoped_dispatcher` scoping that query's tasks on the shared pool — lives in a `query_scan_manager_state` entry keyed by query id. `scan_manager_config::max_concurrent_queries` bounds how many queries may hold scan state at once **and** sizes both the shared pool (each live query parks one sequencer worker) and `SiriusContext`'s admission slot pool. The manager runs alongside the GPU pipeline executors and is independent of the data-repository machinery used between intermediate operators.
...
2. **Per-query preparation.** `prepare_for_query(query, pruning, allocated_gpu_ids)` registers a NEW `query_scan_manager_state` entry keyed by the query's id (other queries' entries are untouched; a leftover entry for the same id is warned about and replaced), builds a `round_robin_strategy` over the query's admitted GPU ids and a fresh `load_balancing_scan_batch_coalescer` inside it, then walks the query's `GPU_SCAN` operators in order. It also mints the query's prefetch-cache epoch (`query_epoch_tracker`) so a new query's prepare cannot demote live peers' prefetched chunks. The admitted set is passed in by `SiriusContext::create_query`, which reads it back off `task_creator` after `sirius_engine::initialize_internal` installed it — so scan placement, partition pins and `pipeline_build_context` all draw on one list (see [configuration.md](configuration.md) for `topology.gpus_per_query`). For each it `register_pipeline`s a sequencer slot (which builds the ingestible's `batch_coalescer` and captures the operator's connector). Then either:
...
3. **Execution.** `start_metadata_processing` spawns the query's sequencer worker on the query's own `scoped_dispatcher` (a stop-scoped view of the shared pool), then calls `split_provider::run` for each non-cached operator. `run` iterates `has_more_splits` / `next_split_provider` and hands each claimed metadata task to that dispatcher; each task enqueues its `scan_info` onto the operator's sequencer slot queue. The sequencer worker walks slots in registration order, coalescing each slot's metadata into data-batch splits, balancing each onto a GPU, firing `fadvise`/`opportunistic` prefetch, and pushing a `scan_operator_input` onto the operator's connector. Consumers (the scan operators) block in `split_connector::get_next_split` until splits arrive.
4. **Teardown.** The sequencer closes each connector when its slot is drained (forwarding any worker-captured exception, first-writer-wins). Once closed and drained, the operator's `all_ports_empty()` returns true and `get_next_task_hint()` returns `nullopt`. `reset(query_id)` drops exactly this query's state entry: it stops and joins the query's scan work via its `scoped_dispatcher` (blocking — the wait happens outside the state mutex so another connection's `prepare_for_query` is never parked behind it), destroys its providers and coalescer, and retires the query's prefetch-cache epoch (`finish_query`). Other queries' scans are untouched.
...
`scan_manager_config` (`config.hpp`) tunes the thread pool, `max_concurrent_queries` (bounds live per-query scan states, sizes the shared pool, and sizes the admission slot pool — see [Concurrency Model](concurrency-model.md#admission)), the IO backend toggle (`use_sirius_datasource`), uring/REST reactor counts, the prefetching cache, and object-store credentials.
