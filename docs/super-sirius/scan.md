...
`no_history_peak_memory_estimate()` uses the larger of stored and working-set bytes for resident inputs that need no carrier conversion. When the cached input marker or planned physical override requires conversion, it adds the working set to the stored bytes multiplied by the named maximum carrier expansion (8), with saturating arithmetic; this accounts separately for the live source and destination. For fresh reads it reserves 8x the projected-column estimate plus decoded filter-only column buffers. The projected-column estimate remains the execution-history basis, and history-based reservations are clamped to the known decoded column-buffer footprint.
