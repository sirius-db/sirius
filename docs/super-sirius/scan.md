...
For the GPU tier, `insert_pinned_entry` merges into an existing entry when the row count *and* the chunk shape match (adding only columns not already cached) — `pin_gpu_merge_compatible` gates that — and replaces it otherwise. A same-total re-pin whose per-chunk placement or boundaries drifted (e.g. after delta promotion grew the entry, or a checkpoint re-chunked the disk image) therefore refreshes cleanly by replacing rather than failing the pin. The HOST tier always replaces, since each host chunk already holds every column.
...
States the delta cannot represent decline at plan time into the transparent CPU fallback: the querying transaction's own uncommitted rows (`LocalStorage`), rowid-only projections, `ARRAY` projections while a delta exists, and string columns whose statistics no longer fit the GPU string-decode limits. In-place `UPDATE`s after the pin (DuckDB update chains) are not yet detected — affected rows serve their pre-update values, for base and delta rows alike. Delta splits carry no zone maps, so filter pruning never drops them.

### Delta promotion

The insert delta's row *bytes* are immutable — appends only add at the tail, deletes are tombstones, `UPDATE` is refused — while only *visibility* changes per query. **Delta promotion** (`enable_delta_promotion`, on by default) exploits that: after a query decodes a delta bundle, its whole **closed** row groups are cached into the pinned entry as new base chunks, so later queries serve them from the base and re-decode only the residual open tail. Visibility is unchanged — the same per-query mask job now covers the promoted chunks, and `GetSelVector`'s insert-side check hides a promoted row from any snapshot older than its insert exactly as it hides too-new base inserts.

- **What promotes.** Whole row groups only, and only *closed* ones (not the table's last row group, or a sealed persistent bulk-flushed tail — `duckdb_insert_delta`'s `closed` flag). Rows must be all-visible at the promoting snapshot (all-visible ⟹ committed ⟹ never reverted). Promotion advances a contiguous ratchet from `n_cache`: a gap stops it, so the base stays one unbroken rowid prefix. Indexed tables whose delta starts mid-row-group (`k_offset > 0`) disable promotion for the pin.
- **Capture → apply.** The decode hook in `gpu_ingestible::materialize_table` photocopies the decoded (unmasked) columns into a thread-safe `promotion_sink` — never mutating the entry while providers read it — and the query serves its own result untouched. At `QueryEnd`, after `reset()` has destroyed the query's providers and while the query-lifecycle lock is still held, `apply_pending_promotions` appends the ratchet's contiguous run to the entry (chunks, memory spaces / host chunks, per-chunk counts, `num_rows`) in lock-step. Memory is reservation-gated: a failed reservation, a non-contiguous slice, or any error just leaves those rows on the per-query delta path (promotion is a pure optimization, never a correctness dependency). The carrier operator (the first matched op with non-empty output) decodes the full pinned column set so promoted chunks are full-width; `post_filter_and_project` trims the extras from that query's own output.
- **Files:** `src/scan_manager/delta_promotion.{hpp,cpp}` (sink, ratchet, capture hook, apply), with the closed/promotable marks in `duckdb_insert_delta.{hpp,cpp}` and `insert_delta_job.{hpp,cpp}` and the QueryEnd seam in `sirius_context.cpp`.

### Introspection

`SELECT * FROM sirius_pinned_tables()` reports one row per pinned entry: identity (name, format, database/schema/table, tier), shape (column/chunk counts, `base_rows`), `is_valid`, and — for duckdb pins — MVCC state (`v_base`, `promoted_chunks`/`promoted_rows`, `promotion_status`, `stale`) plus best-effort live deltas resolved through the catalog (`delta_insert_rows`, `delta_delete_rows`, `dirty_chunks`). Parquet pins NULL the mvcc columns; a dropped or altered table NULLs the live columns. Built by `collect_pinned_table_reports` (`src/scan_manager/pinned_table_report.{hpp,cpp}`).

**Schema drift.** A pin is a positional snapshot of one `DataTable`. Every structural `ALTER` (add / drop / retype a column) and `DROP` replaces the table's `DataTable` object, so `pin_table` captures a `weak_ptr<DataTable>` and the plan-time guard declines (clean CPU fallback) when the live table's `DataTable` differs — the `stale` column surfaces the same check. A column or table `RENAME` reuses the `DataTable`, so the positional cache keeps serving. The compaction/refresh recipe is `unpin → CHECKPOINT → re-pin`; a re-pin whose row count matches but whose chunk shape drifted (after promotion grew the entry, or a checkpoint re-chunked the image) replaces the entry rather than failing.
