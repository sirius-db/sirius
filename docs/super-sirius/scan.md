...
- **Updates** — DuckDB update chains version values in place, which row-visibility masks cannot represent. Before serving, the scan manager checks the union of materialized columns once per pinned entry across the full live physical row range, including the insert delta. If a scanned column has an update chain, transparent execution falls back to DuckDB CPU in the same transaction and snapshot. With `enable_duckdb_fallback = false`, the query returns an explicit error instead of stale values. Updates to columns the query does not materialize do not force fallback.

States the delta cannot represent decline at plan time into the transparent CPU fallback: the querying transaction's own uncommitted rows (`LocalStorage`), rowid-only projections, `ARRAY` projections while a delta exists, and string columns whose statistics no longer fit the GPU string-decode limits. Update-chain detection is deliberately conservative: even a version not visible to the query's snapshot causes fallback when its column is scanned. To restore GPU cache serving after an update, first `CALL unpin_table(...)`, then `CHECKPOINT`, then pin the table again. A manual checkpoint while a pin is live changes the database checkpoint generation; the next query refuses that pin and falls back or errors until it is unpinned and pinned again. The delta is re-captured and re-staged on every query, so a pinned table under sustained insert churn pays that cost until the next checkpoint + re-pin; delta splits also carry no zone maps, so filter pruning never drops them.
