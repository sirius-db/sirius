...
Adding a key family means one new value of `membership_key_family`, one arm in `classify_membership_key` / `membership_probe_compatible`, and one probe adapter plus its arm in `dispatch_probe_adapter` (`src/include/cuda/dynamic_filter_probe.cuh`); a family whose build side is not read through an integer carrier also adds an arm to `with_build_key_iterator` (the string family materializes its fingerprints there). The three filters do not change.
