...
**Application in the target scan.** A dynamic join filter is *redundant with the join that produced it*: the join discards every non-matching probe row, so the filter is a conjunctive superset and never has to be exact. The zone map rides the parquet reader's filter: `merge_dynamic_filters_into_ast` AND-merges it onto the reader root and `materialize_metadata_to_table` installs it via `reader_options::set_filter`, so cuDF can prune row groups by statistics and evaluates the predicate during decode. That merge sits behind the per-scan reader pruning gate (row-groups-pruned signal; see [dynamic-filters-top-n.md](dynamic-filters-top-n.md)), which stops it on scans where it demonstrably prunes nothing. Membership filters (IN-list / Bloom) are not AST-lowerable and ride the **post-decode** path (`sirius_physical_dynamic_filter` → `apply_dynamic_filters_to_view`, `membership_masks_only` mode). On the duckdb-native scan, which has no reader filter, the operator instead runs in `include_ast_row_masks` mode so zone maps apply row-wise there too. The shared `dynamic_filter_gate` measures scan-level and per-filter usefulness before allowing that post-decode cost to repeat. The static filter is untouched and remains authoritative. On a pinned-cache-served parquet scan no reader runs; the wrapper is promoted to `include_ast_row_masks` at prepare time and the zone map applies as a post-decode row mask behind the same gate ("Pinned-cache-served scans" in [dynamic-filters-top-n.md](dynamic-filters-top-n.md)).
>
