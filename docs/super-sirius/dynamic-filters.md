...
- **Sideways information passing (SIP)** — a hash-join build applies its membership filter inside its own probe subtree, as deep as the key stays a faithful pass-through, so intervening operators also skip rows the join would discard. Ships on by default under the same switch as scan pushdown.
...
- `enable_dynamic_filter` (bool, default **true**) — the feature switch: when off, discovery never runs, so no channel exists anywhere and there is zero overhead. On, each eligible build's membership filters wire through both routes — scan binds and join-edge endpoints (including on an intervening join's build input).
...
**Rollback levers, in order of blast radius.** `SET dynamic_filter_domain_coverage_threshold = 2.0` restores pre-gate publication behavior without disabling dynamic filtering. `SET enable_dynamic_filter = false` disables the whole subsystem: discovery never runs, so neither producer nor consumer wires anything. Each is a setting that already exists; reversing this subsystem introduces no new knob.
...
The derived classification is a plausibility heuristic, not a selectivity proof. Its failure modes (an unfiltered correlation domain; a user-hinted bare-copy `MATERIALIZED` CTE) wire a filter that keeps every row; the scan-level keep-ratio gate and the per-filter permanent skip disable such a filter after one measured split, and results never change because the producing join stays authoritative. The asymmetry is deliberate: on derived-only evidence (the mirror armed no scan bind), a key whose trace bottoms at a GPU scan still takes the join-edge route (endpoint above the scan), never a scan bind — the scan route mirrors DuckDB exactly.

**Flag:** none of its own — the join-edge route ships under `enable_dynamic_filter` (default on). The temporary `enable_dynamic_filter_sip` rollout flag was collapsed into it per the decision recorded on [#1010](https://github.com/sirius-db/sirius/issues/1010).
