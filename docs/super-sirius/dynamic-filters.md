...
Phases 1 and 2 are implemented under `enable_dynamic_filter`; zone maps and multi-partition Bloom
construction remain opt-in. Phases 3 and 4 are design only.
...
| Producer kind | hash-join build (any eligible join mode); whole builds use one-shot publication, partitioned builds may accumulate a Bloom | (unchanged) | + agg, sort, filter | (unchanged) |
| Coordination | one-shot build-port publication or exact-ID pre-scatter accumulation; consumers remain opportunistic | deterministic under the active task strategy; opportunistic under lookahead | (unchanged) | streaming / incremental refinement |
...
`dynamic filter NOT published` diagnostic naming the join mode and increments
`publications_skipped_build_not_whole`. Both fire **once per join**.
...
or unsupported key, a domain-covering key, or a non-selective zone range. That successful no-op is
still `FINISHED`. Consumers need no readiness protocol: they test the channel and local device
availability and pass the batch through when nothing useful can apply. The one-shot filter kinds
retain PR #1277's best-effort per-target replica behavior. The accumulated path is deliberately
...
Destination replicas explicitly reserve their allocator-accounted bit-array bytes before construction. The source Bloom in the one-shot path and per-device accumulator partials instead allocate from each GPU space's reservation-aware default allocator without a separate up-front reservation. They remain accounted and subject to CuCascade's allocation limits; the Bloom cap below is an earlier policy gate, not a reservation.
...
3. Otherwise select the Bloom representation when its key type is supported; construction still requires the complete Bloom candidate set to pass the per-GPU size gate.
...
**Bloom memory gate.** `max_dynamic_filter_bloom_bytes_per_gpu` (default 256 MiB) caps the allocator-accounted Bloom bit arrays produced by one join on each GPU. One Bloom uses 16 bits per global build row in 256-bit blocks, rounded to CUDA allocation alignment; the gate sums that footprint across every Bloom candidate key with overflow-safe arithmetic. Equality is allowed for representable footprints. If the complete set exceeds the cap or its aligned footprint is not representable, all of that join's Bloom candidates are skipped before allocation, avoiding admitted-key-order bias; exact IN-lists and zone maps remain eligible. The cap is per GPU and is therefore neither multiplied by replica count nor divided by partition count. It excludes host object overhead and the accumulator's transient reduction scratch. A skip is a successful fail-open publication decision, not a query or publication failure.
...
The **hash-IN-list-to-Bloom** cutover is hardware-adaptive — it replaced an earlier fixed
row-count threshold. The publisher estimates the hash-set footprint against the smallest L2 cache
among the planned probe GPUs: it keeps the exact hash IN-list while the estimate fits and
otherwise uses the more compact Bloom representation. No hardware-specific cardinality threshold
is embedded in the policy.
...
- `enable_dynamic_filter` (bool, default **true**) — enable target discovery for probe scans and join-edge endpoints. When off, no dynamic-filter channels are created.
- `enable_dynamic_filter_multi_partition` (bool, default **false**, requires `enable_dynamic_filter`) — accumulate one globally complete Bloom per eligible INT32/INT64 key across a non-broadcast hash-partitioned build. When off, PR #1277's one-shot behavior is unchanged.
- `max_dynamic_filter_bloom_bytes_per_gpu` (bytes, default **256 MiB**) — cap one join's allocator-accounted Bloom bit arrays on each GPU across all Bloom keys. `0` disables Bloom construction while leaving exact IN-lists and zone maps eligible.
- `enable_dynamic_zone_map_filter` (bool, default **false**, requires `enable_dynamic_filter`) — additionally emit build-key min/max filters. Parquet scans use them for row-group pruning; duckdb-native scans apply them post-decode.
...
| `test/cpp/integration/test_gpu_execution_dynamic_filter_sip.cpp` | Derived-build and join-edge placements preserve results; a forced multi-partition build retains the default-off skip, publishes a complete Bloom when the subordinate switch is on, and remains disabled when the master switch is off |
