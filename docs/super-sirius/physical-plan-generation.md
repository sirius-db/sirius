...
**What the count has to satisfy.** Beyond the byte target (`hash_partition_bytes`), the count is what establishes [INV-FOLD](operators.md#inv-fold-every-group-becomes-one-cudf-table) for whichever side the join folds into one batch per partition. `compute_hash_join_partition_strategy` therefore floors the count at `fold_partition_count(measured_rows, max_fold_rows)` when the measured side is a folding side: one partition while the measurement fits a single fold, and only past that a split taken against the halved `fold_row_target` so hash skew has headroom. A build that cannot fit one fold is refused broadcast (a replica is not split, so raising the count cannot shrink its fold), and a MARK join in the same position throws, its count being pinned at one partition or at broadcast. Both the refusal and the throw require an exact measurement; an inexact one may only add partitions (residual R5, see [operators.md](operators.md#inv-fold-every-group-becomes-one-cudf-table)).
