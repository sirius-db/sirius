...
`round(x [, places])` maps to `function_id::round` for DuckDB's floating overload only — `round(FLOAT | DOUBLE [, INTEGER places])` with a constant `places`; `from_duckdb` declines the DECIMAL and integer overloads so they keep the CPU path. It is materialize-only (`src/expression_evaluator/specializations/function.cpp`) and shares its kernel with the semantic FP64 → DECIMAL cast (`round_to_scale_like_duckdb`, `src/expression_evaluator/specializations/round_to_scale.cu`): DuckDB's `round(x * 10^places) / 10^places` on the whole value, half away from zero, computed in double, with `round(x / 10^-places) * 10^-places` for negative places. The StarRocks translator emits it over every FE-declared DECIMAL aggregate it lowered to FP64, so two independent FP64 sums of the same decimals finalize to the same double (TPC-H q15 compares such a sum against the max of another evaluation of the same CTE).
