...
sirius:
  ...
  operator_params:
    ...
    max_dynamic_filter_bloom_bytes_per_gpu: 268435456  # 256 MiB accumulated-Bloom budget per join on each GPU (one-shot is uncapped)
...
| `max_dynamic_filter_bloom_bytes_per_gpu` | 256 MiB | Per-join allocator-accounted budget for the multi-partition accumulated Bloom on each GPU, summed across its Bloom keys; the full candidate set is skipped when it exceeds the cap. One-shot publication is not budget-gated. `0` disables accumulated Bloom construction only. |
