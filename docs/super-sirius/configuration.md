...
| `enable_clustered_merge_bypass` | true | Skip the PARTITION/MERGE_GROUP_BY re-hash of grouped-aggregate partials when a runtime range proof (per-partial leading-key min/max, adjacency-only overlap) shows the partials are range-disjoint (clustered input, e.g. lineitem grouped by `l_orderkey`). The merge pushes the downstream HAVING filter to each partial and re-groups only the boundary windows with the normal merge combine. Every branch is exact; the gate only chooses between exact strategies. Requires a FILTER directly above the merge, single GPU, and decomposable combines (SUM/MIN/MAX/COUNT). |
| `clustered_bypass_max_overlap_fraction` | 0.05 | Maximum adjacent-batch key-range overlap (fraction of the smaller batch's key span, plus a small absolute floor) the clustered merge bypass tolerates before declaring the input un-clustered. Purely economic — correctness never depends on it. |
| `enable_sorted_groupby_hint` | true | Pass `sorted::YES` to the grouped-aggregate cudf groupby when a per-batch `cudf::is_sorted` check (explicit ASCENDING / `null_order::AFTER`, matching the groupby ctor) proves the keys pre-sorted, skipping both hashing and sorting on clustered batches. Unsorted input cannot take the sorted path. |
| `sorted_groupby_hint_min_rows` | 1 Mi rows | Minimum batch rows before the sorted-groupby-hint `is_sorted` check runs (the check costs one comparator pass over the key columns). |
...
| `enable_clustered_merge_bypass` | true | Range-proof-gated bypass of PARTITION/MERGE_GROUP_BY for clustered grouped-aggregate partials (HAVING pushed to partials + boundary fix-up regroup) |
| `clustered_bypass_max_overlap_fraction` | 0.05 | Adjacent key-range overlap tolerated by the clustered merge bypass |
| `enable_sorted_groupby_hint` | true | `cudf::is_sorted`-proven `sorted::YES` for grouped aggregates |
| `sorted_groupby_hint_min_rows` | 1 Mi rows | Minimum batch rows before the is_sorted check runs |
