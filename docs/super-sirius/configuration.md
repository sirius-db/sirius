...
| `enable_disjoint_groupby_passthrough` | false | YAML accepts this merge-local diagnostic switch. It preserves multiple read-only grouped-aggregate partial batches only when a strict runtime proof shows their non-null integral or timestamp leading keys are disjoint, the upstream repository has exactly one partition, and task bytes are at most `hash_partition_bytes`. Other key shapes use the normal merge. It saves only the merge re-hash: upstream partitioning still runs and no HAVING filter is pushed to partials. |
| `enable_sorted_groupby_hint` | false | YAML accepts this diagnostic switch. Eligible fixed-width keys that meet `sorted_groupby_hint_min_rows` are checked with `cudf::is_sorted`; only proven keys pass `sorted::YES`. It is disabled by default because the sorted path builds additional group/order metadata and may gather value columns, increasing transient GPU memory. |
| `sorted_groupby_hint_min_rows` | 1 Mi rows | YAML accepts this diagnostic threshold for the sorted-groupby `is_sorted` check; it must be at least 1. |
| `pin_table_natural_file_order` | true | YAML accepts this diagnostic switch for digit-aware multi-file parquet ordering while pinning. |
...
`enable_disjoint_groupby_passthrough`, `enable_sorted_groupby_hint`,
`sorted_groupby_hint_min_rows`, and `pin_table_natural_file_order` are accepted in YAML under
`sirius.operator_params`. They are exposed as DuckDB settings only when Sirius test options are
explicitly enabled, and are not part of the normal user session surface.
