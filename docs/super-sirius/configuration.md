...
| `enable_aggregate_label_remap` | true | Compute the COLLECT_SET (COUNT DISTINCT) group labels with `cudf::distinct` + `cudf::key_remapping` (hash probe) instead of `cudf::encode` (per-row binary search); labels are byte-identical, off = encode |
