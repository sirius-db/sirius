...
| `enable_dynamic_filter` | true | Enable runtime filters for eligible `BUILD_PROBE` joins. Targets may be probe scans or join-edge endpoints. An eligible build selects a raw exact IN-list for 1–12 supported build rows, otherwise a hash IN-list within the L2 budget or a Bloom. |
