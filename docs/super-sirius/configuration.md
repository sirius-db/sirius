...
| `enable_disjoint_groupby_passthrough` | false | YAML accepts this merge-local diagnostic switch. It preserves multiple read-only grouped-aggregate partial batches only when a strict runtime proof shows their leading keys are disjoint, upstream naturally selected exactly one partition, and task bytes are at most `hash_partition_bytes`. It saves only the merge re-hash: upstream partitioning still runs and no HAVING filter is pushed to partials. |
| `enable_sorted_groupby_hint` | true | YAML accepts this diagnostic switch. A per-batch `cudf::is_sorted` proof may pass `sorted::YES` to grouped aggregation; unsorted input cannot take that path. |
| `sorted_groupby_hint_min_rows` | 1 Mi rows | YAML accepts this diagnostic threshold for the sorted-groupby `is_sorted` check. |
