...
The owner can be any copy-constructible object that keeps the memory alive — e.g. a `read_only_data_batch` lock on a source batch (so the source stays alive and read-only-pinned for the view's lifetime), a `shared_ptr<cudf::table>`, or a composite of both. Producers of view-backed batches include the pinned-table scan path, the PROJECTION operator's zero-copy passthrough paths, and the PARTITION operator's hybrid zero-copy hash output (see [operators](operators.md)). Downstream code reads both forms through `get_table_view()`, and these representations record the producer's writer event so cross-stream or cross-device consumers can order their reads without materializing the view. `release_table()` also accepts a view-backed representation, but it enqueues the deep copy on the release stream and destroys the type-erased owner inline, so a caller holding the last owner must not use it to materialize across streams.
