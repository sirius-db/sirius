...
| F3 | The prefetch cache tracks LIVE query epochs instead of one newest-wins ticker. `query_epoch_tracker` (new, `io/cache/query_epoch_tracker.hpp`): `prepare_for_query` mints the query's epoch, the new `finish_query(query_id)` retires it at `scan_manager::reset` (default + routed ioctxs — retired even when the query registered no scan state, since the epoch is minted before that registration). Chunk scoring passes `min_live_epoch()` — the OLDEST live epoch — as the staleness bar to `eviction_tier`, so a new query's prepare no longer demotes live peers' prefetched-but-unread chunks to tier 0. Shared chunks get UNION accounting: `chunk_lifecycle::on_request(query_tick, min_live_tick)` resets counters only when the chunk's tick predates every live epoch; otherwise it accumulates inserts and advances the tick — a second query touching a shared chunk no longer erases the first's outstanding demand. Requests are stamped with the REQUESTING query's epoch (the coalescer passes `query_id` through `fadvise` → `insert`), not the newest ticker. When no query is live the bar parks at the newest epoch, the old between-queries behavior; single-query semantics are unchanged (with one live query, min live == newest). |
| F4 | The cache's preparation/prefetch/eviction queues round-robin across queries instead of strict process-wide FIFO. `fair_band_queue` (new, `io/cache/fair_band_queue.hpp`): FIFO within a band (band = the request's query epoch), rotation across non-empty bands at pop — query A's 20k-chunk flood no longer blocks query B's first prefetch (B's request surfaces within #bands pops). Single band = bit-identical FIFO. The thread pipeline stays single-threaded; only the ORDER is fair. The shared `_rate_limiter` budget is untouched — fair pop order alone interleaves its consumption. nullptr wake-up sentinels ride band 0 and take part in the rotation. Component tests in `test/cpp/io/cache/test_prefetch_cache_fairness.cpp` (GPU-free); the epoch plumbing is asserted end-to-end in `test_s3_routing_cutover.cpp`. |
...
| C4 | The GPU manager thread NEVER blocks on memory. It used to run the blocking `make_reservation` and the downgrade `.get()` itself while holding a reserved pool slot, so one query's memory-hungry task stalled every query's dispatch to that device (a fair F1 pop still stalled behind the blocked reserve). `process_task` now only resolves the completion handler, attributes the slot to the task's query, and dispatches; the reservation, the downgrade-on-shortfall dance and the execution all run in the worker (`prepare_and_execute`), where the pool's per-query accounting already covers them for the drains. At most ONE task per executor parks in a memory wait (`_memory_waiter_parked` — the historical one-blocking-reservation-per-device arbitration); overflow waiters re-queue through the executor's own queue after a 10 ms worker-held backoff (no tight spin, F1 rotation interleaves co-tenants between retries; `executor_metrics.tasks_requeued_on_memory_wait` counts them), so a hungry query cannot fill every worker slot with parked waits either. Retry-cap semantics untouched: re-queues never consume `gpu_reservation_max_retries` budget (that cap remains OOM-reschedule-only). Telemetry stays FSM-legal: the re-queue emits `routing` (the queued→routing→queued loop a scheduler→executor hop already takes), and `reserving` is only entered once the task commits to acquiring. Evidence: `test_gpu_pipeline_executor_memory_wait.cpp` parks a real reservation behind a test-held hold — pre-fix the shorts scheduled after it never dispatch (manager blocked); post-fix they complete in milliseconds while it waits, and a second hungry task re-queues instead of consuming the last worker slot. |
| D3 | `schedule_lookahead` rotates round-robin across ACCEPTING queries (the F1 rotation idea applied to warm-up) instead of hard-coding `begin()` — previously only the OLDEST query ever got lookahead and every newer query started cold; a query with nothing warmable (or one quiescing per the lifecycle registry) no longer pins the rotation, and the cursor mirrors the queue's fair-pop contract (upper_bound, stale ids harmless). The teardown-accounting hole is closed by ORDER, mirroring what the creation path gets from slot-attach: `drain_pending_tasks` now clears the lookahead queue under `lookahead_mutex` BEFORE its request drain, so a racing lookahead either lands its push ahead of the drain (dropped) or finds the queue empty (no-op) — the operator walk is always against a still-alive plan (the plan dies only after `drain_pending_tasks` returns), and no stale request can survive `reset(query_id)`. Previously (clear LAST) a racing push could land after the drain and hand a worker a raw operator pointer into a destroyed plan with nothing waiting on it. Evidence: `test_task_creator_lookahead.cpp` — rotation/skip/single-query cases fail pre-fix (begin() only), and a looped `schedule_lookahead`-vs-`reset(query_id)` race asserts the creation queue holds nothing for the dead query under every interleaving. |
...
- ~~**F3/F4** (prefetch-cache cross-query fairness)~~ — FIXED, see the table
  (live query epochs + union chunk accounting; per-query round-robin request
  queues).
- **F5–F9** — fairness/throughput; matter after correctness burn-in. F5
