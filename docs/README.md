...
To generate the clickbench dataset:
```
cd test_datasets
...
To load the Clickbench dataset to duckdb:
```
./build/release/duckdb {DATABASE_NAME}.duckdb
...
To run only one of tests do: 
```
build/release/test/unittest sirius_tpch_queries.test
```
