...
**`[integration][TPC-H]` reconciliation (post-verify, 2026-05-01):** 15-VALIDATION.md's first attempt at this suite hit the 1800s MCP cap with SIGTERM at Q11 parquet, which initially looked like the documented `[v1.3 SHIP BLOCKER — 13-04 PARTIAL]` cumulative-state hang. A reconciliation run (direct unsandboxed `pixi run sirius_unittest --abort '[integration][TPC-H]'`, 600s cap) passed 48/48 in 2:43 with 71608 assertions — Q11 parquet clean mid-run. The earlier timeout was a transient on this consumer 2 × RTX 6000 Ada host, not a real hang. 13-04's verdict was upgraded PARTIAL → PASS in `13-04-SUMMARY.md`, and 15-VALIDATION.md's `[integration][TPC-H]` section was rewritten to reflect this. No Plan 13-05 follow-up is required. Phase 15's source diff (vs Phase 14 HEAD `0ee3166`) remains zero behavioral changes on the production path.
