...
Companion to [`2NODE-REPLICATE.md`](2NODE-REPLICATE.md) (engine A, fabric arena — the working
two-host procedure). Historical engine-A runbook: [`notes/2026-08-11-2node-gb200/2NODE-RUNBOOK.md`](../../../notes/2026-08-11-2node-gb200/2NODE-RUNBOOK.md).
This document brings up the **engine B** control:
...
* `run_mode` stays **unset** (defaults to `shared_nothing`). Engine A requires
  `run_mode = shared_data` (REPLICATE / the archived runbook); copying that here breaks engine B.
  *(Correction to the archived runbook's stated reason — see `notes/2026-08-11-2node-gb200/2NODE-RUNBOOK-GAPS.md` §A.)*
