#!/usr/bin/env bash
# cluster8.sh variant that drives each CN from an explicit --sirius-config
# (.cn<i>/sirius-tuned.yaml) instead of --gpu-memory-limit/--host-memory-limit.
# The CN rejects --sirius-config together with those flags, and per STATUS.md the
# operator budgets -- the decisive lever for q08 -- can only be set this way.
...
for i in $(seq 0 $((NUM_CNS - 1))); do
    ...
    "$CN_BIN" \
        ...
        --http-port      "$((base + 3))" \
        --starlet-port   "$((base + 4))" \
        --sirius-config ".cn$i/sirius-tuned.yaml" \
        --engine-dir ".cn$i" &
    pids+=("$!")
