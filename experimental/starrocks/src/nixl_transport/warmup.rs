...
//!
//! Environment (all four live in the validated registry, `crate::tunable`, so a typo fails
//! bring-up instead of silently changing behaviour):
//!   * `SIRIUS_CN_NIXL_WARMUP=off` — kill switch; back to purely lazy sessions.
...
/// Operator-tunable warmup behaviour, resolved once from the registry.
struct Settings {
...
impl Settings {
    /// Reads the validated registry; `None` means the kill switch is set.
    ///
    /// Every knob comes from `Tunables`, so a typo in any of them fails CN bring-up
    /// (`Tunables::resolve` in `main`) instead of silently reverting to the default here — which
    /// used to look exactly like the knob having no effect.
    fn from_tunables() -> Option<Self> {
        let tunables = Tunables::get();
        if !tunables.warmup {
            return None;
        }
        Some(Self {
            peers: tunables.warmup_peers,
        ...
        for (host, brpc_port) in peers {
            ...
            match warm_one(requests, agent_name, local_md, &host, brpc_port) {
                ...
                Err(err) => {
                    ...
                    entry.0 += 1;
                    let backoff = retry_backoff(entry.0);
