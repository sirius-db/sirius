...
//!
//! The cycle only breaks when the PRPC reply timeout (60 s) fires, long after the FE gave up.
//! Reproduced on a fresh 4-CN cluster: run 1 (COLD) failed after 121 s with the FE's
//! `THRIFT_RPC_ERROR ... request time out ... 60000 (MILLISECONDS)`; run 2 (WARM, same cluster,
//! same SQL) took 751 ms and was correct. A TPC-H harness that discards run 0 as a warm-up pays
//! the hang exactly once, invisibly.
//!
...
//! The remainder of session set-up (`load_remote_md`, the bandwidth canary) still runs on the
...
impl Settings {
    ...
    fn from_env() -> Option<Self> {
        ...
        Some(Self {
            peers,
            budget: super::warmup_timeout(),
            expect: super::warmup_expect_peers(),
        })
    }
}
