...
/// sender fragment runs; the cross-node nixl transport implements the `Remote` arm.
#[derive(Debug)]
...
impl SenderDrains {
    ...
    /// claim on the parked fragment (`nixl_transport::TransportState::send_fragment` on success,
    /// the transport thread's failure arm otherwise); abandoning a sibling would only lose its
    /// outcome, never cancel it. (2) The FE must see a failure whichever destination produced
    /// it. Every failure is logged; the first is returned.
...
impl FragmentOutcome {
    ...
    /// the blocking alternative: a sender that could not reach a remote destination has already
    /// failed the query.
...
struct ServiceCore {
    ...
    /// `SiriusEngine` (via [`SiriusComputeNodeService::with_transport`]); tests use a stub.
...
impl SiriusComputeNodeService {
    ...
    ///
    /// The order is the whole point: dispatch puts the receiver on the worker thread while the
...
impl PInternalService for SiriusComputeNodeService {
    ...
    /// Leases bytes of this CN's staging arena for a peer's device WRITE. Served directly from
    ...
    async fn transmit_packed(
        ...
        // Blocking again: the canary path touches the arena handle, and a completed sender
...
impl SiriusComputeNodeService {
    ...
    /// a completed receiver to the dispatch worker. This is the receive half of the cross-node
    /// tier and needs no transport of its own — the sending CN's transport WRITEs the payload
    /// into this CN's arena and then calls this RPC.
...
impl ServiceCore {
    ...
    /// the next receivers when this fragment's own sink completed other sender sets.
    ...
    /// sender output is materialized. Returns the receivers whose sender sets this fragment
    /// completed — the caller decides where they execute (RPC handlers hand them to the dispatch
    /// worker; the worker itself runs them inline).
    fn process_fragment(
        &self,
    ...
    /// for its local receivers (transmitting it when the receiver is remote). `inputs` names the
    /// parked sender outputs this fragment consumes; `remote_inputs` the staged remote batches.
    /// A sender that completes a receiver's sender set returns that receiver for the caller to
    /// run or dispatch, alongside the remote drains the caller must join.
    fn execute_fragment_with_inputs(
        ...
        // (each destination drains its own copy from its own output stream); HASH_PARTITIONED
        // routes rows by the translated partition key columns.
        ...
        // thread is still draining this CN's remote destinations.
        //
        // ORDER IS UNCHANGED, and deliberately so: one FIFO request channel into ONE transport
        // thread that handles `SendFragment` inline means the drains run one at a time, in the
        // FE's destination order, with every frame of a destination issued by that single
        // thread. `seq`, eos, and the exactly-once `drop_parked` all stay inside the transport
        // thread's own drain loop, so the receiver's per-(exchange, sender) gap check
        // (`local_exchange::push_remote_frame`) sees a gap-free byte sequence.
    ...
    /// Staging arena `(base, capacity)`, cached after the first successful executor round-trip.
    ...
    /// next receivers when this one's own sink completed other sender sets.
...
impl SiriusComputeNodeService {
    ...
    /// StarRocks OK status. For these RPCs OK means the fragment was accepted and (for a result
    /// fragment) executed and buffered.
...
mod tests {
    ...
    /// Captures the fan-out shape of every sender run (outputs, broadcast, hash keys).
    #[derive(Debug, Default)]
    struct FanOutRecordingExecutor {
        runs: Mutex<Vec<(Vec<SenderSlot>, bool, Vec<usize>)>>,
    }
    ...
    #[test]
    fn two_local_senders_complete_the_receiver_sender_set() {
        // N-senders: the receiver expects two senders on one exchange; it runs only after BOTH
        // sender instances have parked, with each sender counted by its sender_id.
        let executor = Arc::new(CountingExecutor::default());
        let service = SiriusComputeNodeService::with_executor(executor.clone(), test_identity());
        let query_id = TUniqueId::new(21, 1);
        let receiver_id = TUniqueId::new(21, 2);
        ...
        for sender_ordinal in 0..2 {
            // Only the first sender must NOT complete the set.
            assert_eq!(
                executor.calls.load(Ordering::Relaxed),
                sender_ordinal as usize,
                "the receiver ran before its sender set completed"
            );
            let mut sender = fragment_params(Some(scan_plan(0, 0)), Some(desc_table()));
            sender.fragment.as_mut().unwrap().output_sink = Some(data_stream_sink(7));
            let mut sender_exec =
                exec_params(query_id.clone(), TUniqueId::new(21, 3 + sender_ordinal));
            sender_exec.sender_id = Some(sender_ordinal as i32);
            sender_exec.destinations = Some(vec![local_destination(receiver_id.clone())]);
            sender.params = Some(sender_exec);
            assert_exec_ok(&service, &sender);
        }

        // Two senders plus the dispatched receiver.
        wait_until("both senders and the receiver to execute", || {
            executor.calls.load(Ordering::Relaxed) == 3
        });
        let fetched = fetch_rows_eventually(&service, receiver_id.hi, receiver_id.lo);
        assert!(!fetched.attachment.is_empty());
    ...
    #[test]
    fn unpartitioned_fan_out_to_two_local_receivers_broadcasts_one_park() {
        // An UNPARTITIONED sink with two destinations parks ONCE with one output stream per
        // destination and the broadcast flag set — the shape the engine fans out on.
        let executor = Arc::new(FanOutRecordingExecutor::default());
        let service = SiriusComputeNodeService::with_executor(executor.clone(), test_identity());
        let query_id = TUniqueId::new(22, 1);
        let first_receiver = TUniqueId::new(22, 2);
        let second_receiver = TUniqueId::new(22, 3);

        for receiver_id in [&first_receiver, &second_receiver] {
            let mut receiver = fragment_params(Some(exchange_plan(7, 0)), Some(desc_table()));
            receiver.fragment.as_mut().unwrap().output_sink = Some(result_sink());
            let mut receiver_exec = exec_params(query_id.clone(), receiver_id.clone());
            receiver_exec.per_exch_num_senders.insert(7, 1);
        ...
        let mut sender = fragment_params(Some(scan_plan(0, 0)), Some(desc_table()));
        sender.fragment.as_mut().unwrap().output_sink = Some(data_stream_sink(7));
        let mut sender_exec = exec_params(query_id, TUniqueId::new(22, 4));
        sender_exec.sender_id = Some(0);
        sender_exec.destinations = Some(vec![
            local_destination(first_receiver.clone()),
            local_destination(second_receiver.clone()),
        ]);
        sender.params = Some(sender_exec);
        assert_exec_ok(&service, &sender);
        ...
        let sender_run = {
            ...
            runs.iter()
                .find(|(outputs, _, _)| !outputs.is_empty())
                .cloned()
                .expect("the sender parked output")
        };
        let (outputs, broadcast, hash_keys) = sender_run;
        assert_eq!(outputs.len(), 2, "one output stream per destination");
        assert_eq!(
            outputs[0].fragment_instance_id,
            FragmentInstanceId::from(&first_receiver),
            "stream 0 belongs to the FE's first destination"
        );
        assert_eq!(
            outputs[1].fragment_instance_id,
            FragmentInstanceId::from(&second_receiver)
        );
        assert!(broadcast, "UNPARTITIONED fan-out broadcasts");
        assert!(hash_keys.is_empty());

        // Both receivers eventually deliver rows.
        for receiver_id in [&first_receiver, &second_receiver] {
            let fetched = fetch_rows_eventually(&service, receiver_id.hi, receiver_id.lo);
            assert!(!fetched.attachment.is_empty());
        }
    ...
    fn local_receiver_is_dispatched_before_the_remote_drains_complete() {
        // A local receiver whose sender set this CN's own push just completed must not wait out
        // this CN's entire sequential remote drain before it can start. The fake transport holds
        // its drain open until the receiver has run — which can only happen if the sender's RPC
        // thread dispatched the receiver BEFORE joining the drain. Against a blocking
        // join-then-dispatch loop this deadlocks until the bounded wait fires and the RPC
        // reports the timeout.
    ...
    fn every_remote_destination_is_drained_even_after_one_fails() {
        ...
        // drop_parked that releases its claim on the parked fragment. A loop that returned on
        // the first error would never post the second destination at all.
        let (requests_tx, requests_rx) = mpsc::channel();
        let fake_transport = std::thread::spawn(move || {
    ...
    fn remote_drains_are_posted_once_per_destination_in_the_fes_order() {
        ...
        // (`local_exchange::push_remote_frame` fails a query on a seq gap per exchange+sender).
        // It holds because each destination is drained exactly once, by one thread, and the
        // drains are posted to a FIFO channel in the FE's destination order. Pin all three
        // properties.
        let (requests_tx, requests_rx) = mpsc::channel();
    ...
    /// as `remote_inputs` — the full receiver half of the cross-node tier minus the device WRITE.
    #[test]
    ...
    /// The transport tier has not landed; `exchange_nixl_md` names the gap instead of pretending.
    #[test]
    ...
    fn sender_rpc_returns_before_the_dispatched_receiver_executes() {
        ...
        // receiver open; under an inline design the sender RPC could not return until the
        // gate released.
