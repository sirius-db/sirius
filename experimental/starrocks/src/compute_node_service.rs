...
use crate::proto::starrocks::{
    ...
    PSlotDescriptor, StatusPb, p_internal_service_brpc::PInternalService,
};
...
enum DestinationRoute {
    ...
    /// This CN carries no cross-node transport yet: a remote destination is refused loudly
    /// before the sender runs.
...
impl SiriusComputeNodeService {
    ...
    /// identity. Production injects everything via [`with_executor`](Self::with_executor).
    #[cfg(test)]
...
impl ServiceCore {
    ...
    fn execute_fragment_with_inputs(
        ...
        // Route every destination BEFORE running: a remote destination (or a duplicate) must
        // fail before any GPU work happens. Destination i then drains the sender's output
        // stream i -- the FE's destination order, positionally.
        ...
        for destination in destinations {
            ...
            if let DestinationRoute::Remote { host, brpc_port } =
                self.route_destination(destination)?
            {
                return Err(format!(
                    "cross-node exchange to {host}:{brpc_port} needs the nixl transport \
                     tier, which is not active: this CN only exchanges between fragments of \
                     its own process, so every data-stream destination must be its own brpc \
                     endpoint"
                ));
            }
            slots.push(slot);
        ...
        // destination; only rendezvous bookkeeping leaves the engine thread.
        ...
        // Every destination is local (routing refused anything else), so its rendezvous is
        // immediate bookkeeping and fails fast.
        ...
        for input in ready.inputs {
            ...
            for source in input.sources {
                match source {
                    ...
                    SenderSource::Remote { sender_id, .. } => {
                        // Nothing on this CN records a remote sender (routing refuses remote
                        // destinations), so one here is a rendezvous bug, not a recoverable
                        // state.
                        return Err(format!(
                            "exchange node {} became ready with remote sender {sender_id}, \
                             which this CN has no cross-node transport to receive from",
                            input.node_id
                        ));
                    }
                }
