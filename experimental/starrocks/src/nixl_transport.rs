...
// ---------------------------------------------------------------------------------------------
// Inlined transport tunables.
//
// The demo carries a whole registry (`tunable.rs`) resolved at bring-up; this branch inlines
// only the accessors the transport tier needs, keeping the SAME environment names and defaults
// so launcher scripts work unchanged. Each knob resolves once, at first use; an unparsable or
// out-of-range value is LOUD (an error log naming the variable, the value, and the default
// taken) rather than a silent revert — but it does not fail bring-up, because nothing resolves
// the set up front here.

/// The variable's value, or `None` when it is unset OR set to the empty string. Empty counts as
/// unset on purpose: `export FOO=${FOO:-}` and an unset variable are the same operator intent.
...
/// Parses one unsigned-integer knob; out-of-range and unparsable values fall back to the
/// default LOUDLY. Pure so the policy is testable without touching the process environment.
fn parse_u64_knob(name: &str, raw: Option<&str>, default: u64, min: u64, max: u64) -> u64 {
    ...
    match raw.trim().parse::<u64>() {
        ...
        Ok(value) => {
            tracing::error!(
                ...
                max,
                default,
                "transport tunable out of range; using the default"
            );
            default
        }
        Err(_) => {
            tracing::error!(
                ...
                raw,
                default,
                "unparsable transport tunable; using the default"
            );
            default
        }
    }
...
/// Parses one finite-float knob with the same loud-fallback policy as [`parse_u64_knob`].
#[cfg(feature = "nixl-transport")]
fn parse_f64_knob(name: &str, raw: Option<&str>, default: f64, min: f64, max: f64) -> f64 {
    let Some(raw) = raw else {
        return default;
    };
    match raw.trim().parse::<f64>() {
        // NaN fails every comparison, so a bare range check would ADMIT it; require finite.
        ...
        Ok(_) | Err(_) => {
            tracing::error!(
                ...
                raw,
                default,
                "unparsable or out-of-range transport tunable; using the default"
            );
            default
        }
    }
...
/// Bound on a PRPC connect and on waiting for a reply (`SIRIUS_CN_RPC_TIMEOUT_SECS`, default
/// 60 s). Generous because a peer's `request_staging_lease` queues behind whatever fragment its
/// engine thread is currently running; a peer that exceeds this is treated as wedged and the
/// query fails loudly.
...
pub(crate) fn rpc_timeout() -> Duration {
    ...
    Duration::from_secs(*CELL.get_or_init(|| {
        let name = "SIRIUS_CN_RPC_TIMEOUT_SECS";
        parse_u64_knob(name, env_value(name).as_deref(), 60, 1, 3600)
    }))
}

/// Bound on waiting for one posted nixl WRITE to reach DONE
/// (`SIRIUS_CN_NIXL_XFER_TIMEOUT_SECS`, default 30 s). Covers the RDMA/NVLink transfer alone,
/// not any queueing behind a peer's engine thread, so it is much tighter than [`rpc_timeout`]:
/// it is the one signal that distinguishes a wedged fabric from a busy peer.
#[cfg(feature = "nixl-transport")]
fn xfer_timeout() -> Duration {
    static CELL: OnceLock<u64> = OnceLock::new();
    Duration::from_secs(*CELL.get_or_init(|| {
        let name = "SIRIUS_CN_NIXL_XFER_TIMEOUT_SECS";
        parse_u64_knob(name, env_value(name).as_deref(), 30, 1, 3600)
    }))
}

/// Bytes of the mandatory first-contact bandwidth canary (`SIRIUS_CN_NIXL_CANARY_BYTES`,
/// default 16 MiB): pool memory over cuda_ipc silently degrades ~220x with correct bytes, so a
/// slow link must be refused, not tolerated. Large enough that the measurement is bandwidth and
/// not latency; the lower bound keeps it that way.
#[cfg(feature = "nixl-transport")]
fn canary_bytes() -> u64 {
    static CELL: OnceLock<u64> = OnceLock::new();
    *CELL.get_or_init(|| {
        let name = "SIRIUS_CN_NIXL_CANARY_BYTES";
        parse_u64_knob(name, env_value(name).as_deref(), 16 << 20, 1 << 20, 1 << 30)
    })
}

/// Floor under which the link is declared degraded (`SIRIUS_CN_NIXL_CANARY_FLOOR_GBPS`, default
/// 2.0). The healthy same-host cuda_ipc path measured ~85-90 GB/s (A100) and 322-399 GB/s
/// (GB200 NV18); the degraded staged-copy path ~0.4 GB/s. `0` disables the check — that is an
/// escape hatch for a fabric whose healthy bandwidth is genuinely below the floor, and it is
/// warned about once because with it set the silent-degradation trap is once again silent.
#[cfg(feature = "nixl-transport")]
fn canary_floor_gbps() -> f64 {
    static CELL: OnceLock<f64> = OnceLock::new();
    *CELL.get_or_init(|| {
        let name = "SIRIUS_CN_NIXL_CANARY_FLOOR_GBPS";
        let floor = parse_f64_knob(name, env_value(name).as_deref(), 2.0, 0.0, 10_000.0);
        if floor == 0.0 {
            tracing::warn!(
                knob = name,
...
/// (`SIRIUS_CN_NIXL_WARMUP_TIMEOUT_SECS`, default 180 s). Best-effort: exhausting it is loud
/// but never fails bring-up, because a cold peer still works (slowly) through the lazy path.
#[cfg(feature = "nixl-transport")]
fn warmup_timeout() -> Duration {
    static CELL: OnceLock<u64> = OnceLock::new();
    Duration::from_secs(*CELL.get_or_init(|| {
        let name = "SIRIUS_CN_NIXL_WARMUP_TIMEOUT_SECS";
        parse_u64_knob(name, env_value(name).as_deref(), 180, 1, 3600)
    }))
}

/// Peer count that ends the warmup loop early once established
/// (`SIRIUS_CN_NIXL_WARMUP_EXPECT_PEERS`; the `0` sentinel means "keep trying every peer the FE
/// reports until the budget runs out").
#[cfg(feature = "nixl-transport")]
fn warmup_expect_peers() -> Option<usize> {
    static CELL: OnceLock<u64> = OnceLock::new();
    match *CELL.get_or_init(|| {
        let name = "SIRIUS_CN_NIXL_WARMUP_EXPECT_PEERS";
        parse_u64_knob(name, env_value(name).as_deref(), 0, 0, 4096)
    }) {
        0 => None,
...
impl NixlTransport {
    ...
    ///
    /// ORDERING (the invariant the receiver enforces): `local_exchange.rs` fails a query on a
    /// `seq` gap per (exchange key, sender ordinal). Both the counter and the eos frame live
    /// inside the transport thread's `send_fragment`, and the request channel is FIFO — so the
    /// wire sequence is byte-identical to a blocking per-destination loop's.
...
mod agent_tier {
    ...
    // CUDA driver API, for the single-GPU invariant below. Linked explicitly by `build.rs`
    // under the `nixl-transport` feature (the engine only re-exports its own FFI symbols).
    unsafe extern "C" {
        fn cuInit(flags: u32) -> i32;
        fn cuDeviceGetCount(count: *mut i32) -> i32;
    }

    /// The staging arena is registered with `NixlDescriptor::device_id() == 0`, which names
    /// VISIBLE device 0 — unambiguous only when this process sees exactly one GPU. A CN
    /// launched without `CUDA_VISIBLE_DEVICES` pinning could register the arena against a
    /// device it does not live on, and UCX would then move the bytes through the wrong GPU (or
    /// fall back to the staged host path the canary exists to catch). Refuse bring-up instead;
    /// `SIRIUS_CN_NIXL_ALLOW_MULTI_GPU=1` is the explicit override.
    pub(super) fn require_single_visible_gpu() -> Result<(), String> {
        if std::env::var_os("SIRIUS_CN_NIXL_ALLOW_MULTI_GPU").is_some() {
            warn!(
                "SIRIUS_CN_NIXL_ALLOW_MULTI_GPU is set: skipping the single-GPU check. nixl \
                 descriptors carry device id 0, so if the engine is not on visible device 0 the \
                 exchange will move bytes through the wrong GPU"
            );
            return Ok(());
        }
        // SAFETY: plain calls into the CUDA driver. `cuInit` is idempotent, and the engine has
        // already initialized CUDA in this process by the time the transport starts.
        let mut count: i32 = 0;
        let status = unsafe { cuInit(0) };
        if status != 0 {
            return Err(format!(
                "cuInit failed with CUDA error {status}; cannot verify the transport's \
                 single-visible-GPU invariant"
            ));
        }
        let status = unsafe { cuDeviceGetCount(&mut count) };
        if status != 0 {
            return Err(format!(
                "cuDeviceGetCount failed with CUDA error {status}; cannot verify the \
                 transport's single-visible-GPU invariant"
            ));
        }
        if count != 1 {
            return Err(format!(
                "this CN process sees {count} GPUs, but the nixl transport registers its \
                 staging arena with device id 0 — pin exactly one GPU per CN with \
                 CUDA_VISIBLE_DEVICES, or set SIRIUS_CN_NIXL_ALLOW_MULTI_GPU=1 to override"
            ));
        }
        Ok(())
    }
    ...
    impl NixlTransport {
        ...
        /// dir, arena, or a multi-GPU process surfaces here, before any cross-node query is
        /// accepted.
        ///
    ...
    /// (`CUDA_VISIBLE_DEVICES` pins the device per process; enforced by
    ...
    impl TransportState {
        ...
        /// The silent-degradation guard: nothing in nixl/UCX flags the ~220x staged-copy path
        ...
        fn bandwidth_canary(
            ...
            let result = (|| {
                ...
                // the number is in the record, but admit the link. `canary_floor_gbps` warned
                // once at resolution that the trap is unguarded, so this stays quiet here.
                if floor_gbps > 0.0 && gbps < floor_gbps {
                    return Err(format!(
                        "nixl link to {} measured {gbps:.2} GB/s, below the {floor_gbps} \
                         GB/s floor — the silent cuda_ipc degradation trap (non-cudaMalloc \
    ...
    mod tests {
        ...
        /// like the two CNs of a cluster), the metadata handshake loads, a cross-agent WRITE
        /// between two leases reaches DONE, and the measured bandwidth clears the canary
        /// floor. It does NOT verify the transferred bytes (Rust has no view into the device
        /// leases) — value verification is the end-to-end query, where real results are
        /// compared. nixl 1.3.2 refuses loading an agent's own metadata, so the "two processes"
        /// are two agents here.
        #[test]
        #[ignore = "GPU + libnixl smoke: source scripts/cn-env.sh, pin one GPU with CUDA_VISIBLE_DEVICES, run with --ignored"]
        fn nixl_cross_agent_write_between_arena_leases() {
            ...
            // The invariant the production bring-up enforces: this test is documented to run
            // with one pinned GPU, so the probe must agree (and proves the driver FFI links).
            require_single_visible_gpu()
                .expect("pin exactly one GPU with CUDA_VISIBLE_DEVICES for this smoke test");

            let executor: Arc<dyn FragmentExecutor> =
                Arc::new(SiriusEngine::start(None).expect("bring up sirius engine"));
            ...
            assert!(
                ...
                "cross-agent WRITE measured {gbps:.2} GB/s, below the {floor_gbps} GB/s \
                 canary floor — the silent-degradation trap"
            );
...
mod tests {
    ...
    /// The knob parser is where an operator typo would otherwise silently revert to a default;
    /// pin that the fallback is taken (loudly, but deterministically) and that in-range values
    /// win. Pure-input tests — the environment-reading accessors cache process-wide.
    #[test]
    fn u64_knob_takes_valid_values_and_falls_back_loudly_otherwise() {
        let knob = |raw| parse_u64_knob("SIRIUS_CN_TEST_KNOB", raw, 60, 1, 3600);
        assert_eq!(knob(None), 60, "unset means the default");
        assert_eq!(knob(Some("90")), 90);
        assert_eq!(knob(Some(" 90 ")), 90, "whitespace is tolerated");
        assert_eq!(knob(Some("6O")), 60, "a letter O is not a digit");
        assert_eq!(knob(Some("0")), 60, "below the minimum");
        assert_eq!(knob(Some("999999")), 60, "above the maximum");
    }

    #[cfg(feature = "nixl-transport")]
    #[test]
    fn f64_knob_accepts_the_zero_sentinel_and_rejects_nan() {
        let knob = |raw| parse_f64_knob("SIRIUS_CN_TEST_KNOB", raw, 2.0, 0.0, 10_000.0);
        assert_eq!(knob(None), 2.0);
        assert_eq!(knob(Some("50.5")), 50.5);
        assert_eq!(knob(Some("0")), 0.0, "0 is the documented disable sentinel");
        assert_eq!(knob(Some("NaN")), 2.0, "NaN would pass a naive range check");
        assert_eq!(knob(Some("-1")), 2.0, "a negative floor is meaningless");
    }

    /// An empty value counts as unset: `export FOO=${FOO:-}` and an unset variable are the same
    /// operator intent.
    #[test]
    fn empty_environment_values_read_as_unset() {
        // SAFETY: single-threaded touch of a name no other test reads.
        unsafe { std::env::set_var("SIRIUS_CN_TEST_EMPTY_KNOB", "") };
        assert_eq!(env_value("SIRIUS_CN_TEST_EMPTY_KNOB"), None);
        unsafe { std::env::remove_var("SIRIUS_CN_TEST_EMPTY_KNOB") };
    }
}
