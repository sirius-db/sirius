...
//! into a peer's exchange stall — a fragment wedged inside `run()` would starve a peer CN's
//! `request_staging_lease` for the PRPC timeout and fail the whole query — so leases must never
//! wait behind engine work.
...
impl SiriusEngine {
    ...
    /// (built-in engine defaults when `None`).
...
fn engine_thread(
    ...
    // the query. Bounded: an entry is dropped as soon as the slot is parked again.
...
/// failure that triggered the wipe — that is the query's REAL error; reporting only the generic
/// message would attribute an unrelated fragment's failure to the exchange.
...
fn run_fragment_inner<'ctx>(
    ...
    {
        // `result_to_arrow` drains the stream and drops the context-referencing wrapper here, on
        // the engine thread, returning owned batches whose buffers are released via their own
        // Arrow C release callbacks — independent of the context. So they are safe to send to,
        // and drop on, the caller's thread.
        fragment
            .result_to_arrow()
...
mod tests {
    ...
    /// contract, GPU-side, without any network. Requires a GPU.
    #[test]
    fn engine_pushes_staged_remote_batches() {
        ...
        // SAFETY: the GPU lock is held, so no other engine test touches the environment here.
    ...
    /// The regression guard for lease starvation: a peer's `request_staging_lease` lands while
    /// this CN's engine thread is busy running a fragment, and an engine-channel funnel would
    /// make that lease wait for the whole run (forever, for a wedged one). With the arena handle
    /// serving leases on the caller's thread, a lease taken while the engine thread is occupied
    /// must return immediately. Requires a GPU.
    #[test]
