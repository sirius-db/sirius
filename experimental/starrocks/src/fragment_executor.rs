...
///
/// The neutral wire shape of the cross-node exchange tier: on the sender it names a lease in the
/// *local* arena (filled by `export_packed`), on the receiver a lease in the *receiver's* arena
/// (filled by the transport's device WRITE). Whoever holds the arena releases the lease after
/// the bytes leave it.
