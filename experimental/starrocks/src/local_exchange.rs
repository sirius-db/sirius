...
//! parks native engine output as a [`SenderSlot`]; a remote sender's batches arrive as Arrow
//! IPC and sit in [`SenderSource::Remote`] until eos. The receiver becomes ready when every
//! expected sender is complete. There is no fusion: every leaf runs, then hops, then the root.
...
pub(crate) enum SenderSource {
    ...
    /// A remote sender whose Arrow batches arrived over the exchange hop. Counts toward
...
impl LocalExchange {
    ...
    /// Records one Arrow hop frame from a remote sender: a batch, eos, or both.
...
mod tests {
    ...
    fn batch(fill: i64) -> RecordBatch {
        ...
        RecordBatch::try_new(
            schema,
            vec![
                Arc::new(Int64Array::from(vec![Some(fill)])),
                Arc::new(Int64Array::from(vec![Some(fill + 1)])),
            ],
        )
        .unwrap()
    }
    ...
    fn remote_frames_accumulate_and_eos_completes_the_set() {
        ...
        assert!(
            exchange
                .push_remote_frame(key, 0, 0, false, names(), Some(batch(1)))
                .unwrap()
                .is_none()
        );
        assert!(
            exchange
                .push_remote_frame(key, 0, 1, false, names(), Some(batch(2)))
                .unwrap()
                .is_none()
        );
        assert!(
    ...
    #[test]
    fn retire_drops_a_pending_receiver_and_refuses_later_frames() {
        let exchange = LocalExchange::default();
        let key = key(9, 7);
        exchange
            .register_receiver(key.fragment_instance_id, vec![(7, 1)], params())
