...
#
#   TOOLS_DIR   (default: a `tools/` directory next to the repo root, falling back to
#                /opt/dlami/nvme/sirius-demo-build/tools where this box keeps them)
#
...
#
#   bash scripts/apply-starrocks-patches.sh
#
...
if [ -z "${TOOLS_DIR:-}" ]; then
    TOOLS_DIR=$(cd "$REPO_ROOT/.." && pwd)/tools
    # This clone has no sibling tools/ dir; the shared installs live in the demo build tree.
    [ -d "$TOOLS_DIR/nvda_nixl/lib" ] || TOOLS_DIR=/opt/dlami/nvme/sirius-demo-build/tools
fi
...
if [ ! -d "$NIXL_PREFIX/lib" ]; then
    echo "no nixl install at $NIXL_PREFIX -- set TOOLS_DIR (or NIXL_PREFIX) to a dir holding" >&2
    echo "nvda_nixl/ and ucx-install/ (this box: /opt/dlami/nvme/sirius-demo-build/tools)" >&2
...
#
# ORDER DIVERGES FROM THE DEMO ON PURPOSE: this branch's pixi env ships a conda UCX
# (libucp/libucs, pulled in by newer rapids deps) that the demo's env did not have. With
# $_pixi_env/lib ahead of $UCX_PREFIX/lib the nixl UCX plugin dlopens the conda UCX instead of
# the install it was built against, and the agent reports "backend 'UCX' not found" — so the
# nixl/UCX libdirs must come first. Nothing else overlaps: ucx-install/ and the nixl libdir
# carry only their own libraries.
