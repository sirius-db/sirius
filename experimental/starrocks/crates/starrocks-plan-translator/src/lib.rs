...
//! | `HASH_JOIN_NODE`     | `JoinRel` (inner/outer/left-semi; anti joins lower to outer joins + null filters) |
//! | `NESTLOOP_JOIN_NODE` | `JoinRel` (constant-key inner) + optional `FilterRel`, inner/cross only |
//! | `EXCHANGE_NODE`      | `ReadRel` (engine stream view, with an explicit same-node input stream) |
//!
