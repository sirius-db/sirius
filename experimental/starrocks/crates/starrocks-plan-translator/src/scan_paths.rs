...
pub(crate) struct ScanFilePaths {
    ...
    resolved: HashMap<i32, Vec<ScanFile>>,
}
        ...
        for (&node_id, node_paths) in &self.by_node {
            ...
            for path in node_paths {
                let ranges = files
                    ...
                    .map(Vec::as_slice)
                    .unwrap_or_default();
                // Ask "is any size missing" before "do the sizes agree": both questions are
                // about `file_size`, and checking agreement first reports a missing size as a
                // disagreement whenever the incomplete range is not the one that happened to
                // be inserted first.
            ...
            // Zero owned bytes must not fall back to a named-table read of something else.
            if scan_files.is_empty() {
                return Err(Self::unsupported(
                    node_id,
                    "scan instance owns only empty byte ranges",
                ));
            }
