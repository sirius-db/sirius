...
canary_re = re.compile(r"nixl bandwidth canary")
...
if not cross:
    raise SystemExit(
        f"packed hop did not cross CNs (shipped={shipped} received={received})"
    )
# Canary is first-contact on SendFragment. A CN that only receives never WRITEs.
missing_canary = [name for name in shipped if shipped[name] and not canaries[name]]
if missing_canary:
    raise SystemExit(
        f"nixl bandwidth canary missing from shipper(s) {missing_canary} "
        f"(shipped={shipped} canaries={canaries})"
    )
