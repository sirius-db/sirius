...
fn merge_fragment_plans(
    ...
    if !exchange_root_fragments.is_empty() && !intermediate_fragments.is_empty() {
        ...
        for params in &mut exchange_root_fragments {
            ...
            // are accessible during plan translation. Without this, projection
            // expressions from AGG nodes (e.g., SUM(x)/SUM(y)) can't be propagated
            // to the coordinator's output_exprs slots.
